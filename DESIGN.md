...
**Target presentation (BX-I1 — remaining gaps):** Optional **BX-I10A3** filter-toolbar / command surface unification; **BX-I10A4** course/document accent rails; **BX-I10A5** glass/elevation pass — each **separate-gated**, **not automatic**. Auth + PageHeader intro chrome shipped in **BX-I8C** (commit **`8008dc1`**). **Generated plan artifact**, **plan history**, **import toolbar**, and **plan flashcard study card** share the same command-column visual family.
...
| 2026-06-03 | **Phase BX-I10A2** (implementation) + **DOCS-BX-I10A2** — Material-only AI Generate gradient in `components.css` only (commit `b90108e`); scoped violet→blue gradient on material Generate CTA; global `.btn--primary` unchanged; §6.3 updated; **BX-I10A3**–**BX-I10A5** proposed, not started |
