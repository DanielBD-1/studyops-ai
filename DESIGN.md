...
**Shipped presentation (BX-I10A5, commit `38aa561`):** Targeted glass/elevation pass in **`components.css`** + **`layout.css`** only — navigable-card hover softened (**`--elevation-1`**, **`transform: none`**); quiet inline glass on tasks create/edit, flashcards library/create, Trello wizard/sync/results steps, course settings/create-material/materials empty; optional **`.trello-task-list__label`** polish + Focus session outer panel softening (timer/actions unchanged); **`prefers-reduced-motion`** disables new glass **`backdrop-filter`**; **no** JSX/API/behavior/copy changes; **BX-I10A1**–**BX-I10A4** + dashboard hero + material AI cockpit + material editor/library elevation **unchanged**; **BX-I10A5-SMOKE** **Pass with notes** at **~375px** / **1280px** on authenticated route matrix + **`/study-materials/:id`** spot check.

**Verified presentation (BX-I10A6-QA, review only):** Trello failed sync row live verification on **`/trello`** at **~375px** / **1280px** (**narrow responsive browser viewport**, **not** native mobile) — **Pass with notes**, **no** repo file changes. Failed row triggered with harmless fake credentials (**no** cards created); Step 5 **Sync results** uses danger/error tokens (**`--color-error`**, danger subtle/border) — distinct from course rose accent and success green; readable **FAILED** badge and error text; results zone **no** horizontal overflow; credential fields remain **`type="password"`**, cleared after sync.

**Target presentation (BX-I1 — remaining gaps):** Optional material editor/library elevation alignment; optional global shadow/token cleanup only if explicitly approved — each **separate-gated**, **not automatic**. Auth + PageHeader intro chrome shipped in **BX-I8C** (commit **`8008dc1`**). **Generated plan artifact**, **plan history**, **import toolbar**, and **plan flashcard study card** share the same command-column visual family.
...
| 2026-06-03 | **Phase DOCS-BX-I10A6-QA** — **BX-I10A6-QA** Trello failed-row live verification recorded (**Pass with notes**, review only, no repo changes); §6.3 updated; **ROADMAP-A1** and optional polish phases proposed, not started |
| 2026-06-03 | **Phase BX-I10A5** (implementation) + **DOCS-BX-I10A5** — Targeted glass/elevation pass in `components.css` + `layout.css` only (commit `38aa561`); softened navigable-card hover; quiet inline glass panels; optional Trello picker + Focus outer panel polish; §6.3 updated; **BX-I10A6-QA**–**ROADMAP-A1** proposed, not started |
