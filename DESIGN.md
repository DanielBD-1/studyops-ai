...
**Target presentation (BX-I1 — remaining gaps):** Optional **BX-I10A6-QA** Trello failed-row live verification; optional material editor/library elevation alignment; optional global shadow/token cleanup only if explicitly approved — each **separate-gated**, **not automatic**. Auth + PageHeader intro chrome shipped in **BX-I8C** (commit **`8008dc1`**). **Generated plan artifact**, **plan history**, **import toolbar**, and **plan flashcard study card** share the same command-column visual family.
