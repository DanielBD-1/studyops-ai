...
| **Backend fetch** | **`listAllTasks({ courseId?, status? })`** unchanged — **no** backend **`?materialId=`** |
...
| **Deadline heroes** | **`overdue-tasks`** / **`due-today-tasks`** kinds; primary CTA **`View pending tasks`** → **`/tasks?status=pending`**; **no** secondary course-scoped CTA |
| **At a glance** | **Tasks** band adds **Overdue** and **Due today**; positive counts link to existing pending task list (**`/tasks?status=pending`**) |
...
**Not in DASHBOARD-DEPTH-A2 (deferred):** Upcoming deadline window; overdue-only or due-today-only **`/tasks`** list filters or URL params; due-date sorting; stored user timezone persistence; calendar context; notifications; exam-date model; Gemini/Trello due-date assignment; AI scheduling; charts; streaks; goals.
...
- **Dashboard deadline intelligence (beyond DASHBOARD-DEPTH-A2 / A1 / P0)** — **shipped:** rolling **Last 7 days** focus snapshot (**A1**); optional per-task **`dueDate`** on create/edit + card display (**P0**); deadline-aware dashboard aggregates (**A2** — **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`deadlineReferenceDate`**; optional validated **`referenceDate`**; browser-local frontend date alignment; UTC fallback when omitted; recommendation priority including **overdue-tasks** / **due-today-tasks**; **At a glance** **Overdue** / **Due today**). **Deferred:** upcoming deadline window; overdue-only or due-today-only **`/tasks`** filters/**URL query params** (task URL params remain **`courseId`**, **`materialId`**, **`status`** only); due-date sorting; stored user timezone persistence; calendar context; reminders/notifications; exam dates; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates + A2 aggregates are **not** a complete deadline-planning system — further dashboard deadline phases require explicit approval.
