...
| **Priority order** | **no-courses → pending-tasks → due-flashcards → plan-gap → flashcards → add-tasks-or-plans → empty-workspace → caught-up** |
| **Pending tasks win** | When **`pendingTasks > 0`**, **`pending-tasks`** recommendation unchanged even if **`dueFlashcardsCount > 0`** |
...
| **Preserved** | Plan import does **not** set due dates; Gemini schema unchanged; Trello due dates unchanged; dashboard recommendations unchanged; list order **`created_at DESC`**; complete preserves due date |
...
**Not in DASHBOARD-DEPTH-P0 (deferred):** Dashboard overdue/due-today counts; deadline-aware hero; due-date sort/filter/URL params; reminders; exam dates; calendar integration; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates are **not** a complete deadline-planning system.
...
| **Recommendation rules** | Priority order: no courses → pending tasks (optional most-pending course from **`courseStats`** counts) → due flashcards when **`dueFlashcardsCount > 0`** (**FLASHCARD-REVIEW-A5c**) → plan gap (**`totalStudyMaterials - totalGeneratedPlans`**) → flashcards (when none due) → add tasks/plans → empty workspace → caught up. At BX-I3 ship, **`pending-tasks`** CTA was bare **`/tasks`** — **DASHBOARD-DEPTH-A1** superseded with **`/tasks?status=pending`** and course-scoped pending link. |
...
| `/dashboard` | Yes | **Student dashboard cockpit** — real user-owned stats from **`GET /api/dashboard/stats`** (**5B** + **5C** + **5C.1** + **DASHBOARD-DEPTH-A1** last-7-days focus snapshot + **BX-I3** decision layout + **FLASHCARD-REVIEW-A5c** due-flashcard-aware hero when **`dueFlashcardsCount > 0`** + **BX-I4** course workload row accents + **BX-I6B** command-center presentation + **BX-I7C** desktop grid at **≥1280px**); read-only; rule-based **“What should I study next?”** flagship hero; pending-task CTAs **`/tasks?status=pending`** and **`/tasks?courseId=<id>&status=pending`** (**A1**); Study pulse cockpit band (**Last 7 days** focus metrics + Pending/Completed/Total + progress bars); course workload command deck; tertiary **At a glance** stat bands (**Due now** + **Ready for review** deep link — **A4c** + **A5a**); fetch on mount + **Try again** + **Refresh stats**; silent refresh when mounted after stat-changing actions elsewhere; presentation **8C-2A** + **B2** cockpit width + **B3** stat tiles + **BX-I3** + **BX-I6B** + **BX-I7C** (**`layout.css`** grid only) |
...
- **Dashboard deadline intelligence (beyond DASHBOARD-DEPTH-A1 / P0)** — **shipped:** rolling **Last 7 days** focus snapshot (**A1**); optional per-task **`dueDate`** on create/edit + card display (**P0**). **Deferred:** dashboard overdue/due-today task counts; deadline-aware hero recommendations; due-date sorting/filtering/**URL query params** (task URL params remain **`courseId`**, **`materialId`**, **`status`** only); reminders/notifications; exam dates; calendar integration; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates are **not** a complete deadline-planning system — next deadline-aware dashboard phase requires explicit approval.
