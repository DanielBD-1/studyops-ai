...
**Frontend-only** — material detail **View linked tasks** CTA and **`/tasks`** initial-load query filter initialization from **`courseId`** / **`materialId`** URL params. **No** backend, API, database, migration, document-service, or package changes.

| Aspect | Detail |
|--------|--------|
| **Material detail CTA** | **`MaterialRelatedTasksSection`** — primary **`Link`** **View linked tasks** to **`/tasks?courseId=<courseId>&materialId=<materialId>`** via **`buildTasksPageMaterialLink`** in **`task-nav-query.js`**; secondary **View tasks on course** → **`/courses/:courseId`** unchanged; counts, preview rows, loading/error/empty behavior unchanged |
| **Query helpers** | **`parseTasksPageSearchParams`**, **`resolveInitialTaskFilters`**, **`buildTasksPageMaterialLink`** — pure, unit-tested; parse/validate UUID **`courseId`** and **`materialId`** only; **no** **`status`** param support |
| **`/tasks` init** | **`GlobalTasksSection`** — **`useSearchParams`** read-only; one-time init refs (**`courseQueryInitDone`**, **`materialQueryInitDone`**); waits for **`listCourses()`** before course init; waits for **`listMaterials(courseFilter)`** before material init; invalid/missing/unknown params silent fallback; **`statusFilter`** remains **all** |
| **Material filtering** | Client-side over already-loaded tasks (**TASK-MATERIAL-FILTERS-A2**); **`listAllTasks`** unchanged — **no** backend **`?materialId=`** |
| **URL sync** | Manual course/material/status filter changes **do not** call **`setSearchParams`** or **`navigate`** — **not** full URL-persisted filters |
| **Focus return** | **`focusReturnTo`** on **`TaskCard`** preserves full **`/tasks`** query string when present |

**Not in TASK-MATERIAL-NAV-B1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; **URL-persisted** task filters (sync filters to URL on every manual change); **`status`** query param on **`/tasks`**; filter for tasks **without** a material link; Study Rooms / collaboration; PDF; SRS; dashboard charts.
...
| `/tasks` | Yes | **All study tasks** — course + status + **study material** filters (material filter only when a specific course is selected — **TASK-MATERIAL-FILTERS-A2**), create, edit pending, complete, delete, **Start Focus**; optional initial-load deep link **`?courseId=&materialId=`** from material detail **View linked tasks** (**TASK-MATERIAL-NAV-B1** — one-time filter init only; manual changes do **not** update URL; **not** full URL-persisted filters); **`TaskCard`** material **`Link`** to **`/study-materials/:materialId`** when `task.materialId` set (**TASK-MATERIAL-LINKS-A1**); presentation upgraded **8C-3A** + **B4-A** task command surface / command band, filter toolbar framing, semantic **`ul > li`** task list, native status filter **`aria-pressed`** (**B4-A-F1**), scoped empty/error/loading wrappers + **BX-I7E1** desktop panels at **≥1280px** (horizontal command-controls band, **2-column** populated task list, create FormCard full-width, inline edit intended full width via **`:has(.task-workspace__edit-card)`** — **`layout.css`** only) |
