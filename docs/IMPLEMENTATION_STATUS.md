...
**User-facing connect:** shipped in **TRELLO-OAUTH-A4-FRONTEND** (see below). Boards/lists/sync still use manual credentials until **A5**.

---

## Implemented — Trello OAuth frontend connect UI (Phase TRELLO-OAUTH-A4-FRONTEND)

**Frontend only** — Trello **account connection** UX on **`/trello`** and protected OAuth callback. Builds on **A2** encrypted storage, **A3** backend connect routes, and **A4-STATE** signed state validation. **Stored-token sync is not implemented:** **`POST /api/trello/boards`**, **`/lists`**, and **`/sync`** still require manual `{ apiKey, token }` in the POST body (ADR 004). **A5** is still required before boards/lists/sync use the encrypted token in `trello_connections`.
...
**Not in A4-FRONTEND (deferred — A5+):**

- Refactor boards/lists/sync handlers or frontend sync form to use **stored** token from `trello_connections`
- Remove manual apiKey/token fields from sync section (requires **A5** + Security Review)
- Board/list persistence; Trello card update/delete; force re-sync
