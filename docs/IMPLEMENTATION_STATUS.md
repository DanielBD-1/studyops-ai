...
**Still deferred (Trello):** **A4 frontend** OAuth callback (fragment handling, Connect UI on **`/trello`**); **A5** using stored credentials in sync/boards/lists; board/list **persistence**; Trello card update/delete; force re-sync; advanced sync management beyond manual MVP. **A2 foundation complete** — encrypted `trello_connections` in repo. **A3 + A4-STATE backend complete** — connect/authorize HTTP APIs + signed state reviewed; **not live for users** until **A4 frontend** ships.
...
- Trello **OAuth frontend** (**A4:** callback, state/nonce, Connect UI); **A5:** **using stored token** in sync/boards/lists; **board/list persistence**; Trello card **update/delete**; **force re-sync**; advanced sync management beyond manual MVP (**4A** sync UI + **4B** board/list picker end-to-end; manual listId paste no longer required). **A2 done:** encrypted `trello_connections` + crypto/repository. **A3 done (backend only):** connect/authorize HTTP routes — **OAuth not live for users**; manual apiKey/token still live (ADR 004)
...
- Trello OAuth **A4** (frontend callback/Connect UI) + **A5** (sync/boards using stored token); real-time dashboard (polling / WebSockets / cross-tab sync); spaced repetition; advanced flashcard study (known/unknown, Anki)
