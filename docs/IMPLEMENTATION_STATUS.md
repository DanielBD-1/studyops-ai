...
**Not in TASK-MATERIAL-BACKEND-A1:** dashboard material counts; task detail page; migrations/indexes; new sort modes; Trello/flashcards/admin/generate changes. (**`/courses/:id`** URL-persisted filters shipped later in **TASK-COURSE-FILTERS-E1**.)
...
| **Learning assets** | `totalFlashcards`, **`dueFlashcardsCount`** (**Due now** — count shown including **0**; when **> 0**, **Ready for review** + link to **`/flashcards?reviewState=due_now`** — **FLASHCARD-REVIEW-A4c** + **A5a** deep link) |
...
| `/dashboard` | Yes | **Student dashboard cockpit** — real user-owned stats from **`GET /api/dashboard/stats`** (**5B** + **5C** + **5C.1** + **DASHBOARD-DEPTH-A1** last-7-days focus snapshot + **DASHBOARD-DEPTH-A2** deadline-aware aggregates + **TASK-UPCOMING-FILTERS-A1** **`dueNext7DaysPendingTasks`** — optional **`referenceDate`**, browser-local date on fetch, **`overduePendingTasks`** / **`dueTodayPendingTasks`** / **`dueNext7DaysPendingTasks`** / **`deadlineReferenceDate`**, **At a glance** **Overdue** / **Due today** / **Due next 7 days**, recommendation **overdue-tasks → due-today-tasks → due-next-7-days-tasks → pending-tasks → …** + **BX-I3** decision layout + **FLASHCARD-REVIEW-A5c** due-flashcard-aware hero + **BX-I4** course workload row accents + **BX-I6B** command-center presentation + **BX-I7C** desktop grid at **≥1280px**); read-only; rule-based **“What should I study next?”** flagship hero; deadline heroes and positive overdue/due-today/next-7-days stats link to backend-filtered task lists **`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, and **`/tasks?status=pending&deadline=next_7_days`** (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** — no secondary course CTA on deadline kinds); generic **`pending-tasks`** hero still supports **`/tasks?status=pending`** and **`/tasks?courseId=<id>&status=pending`** (**A1**); Study pulse cockpit band (**Last 7 days** focus metrics + Pending/Completed/Total + progress bars); course workload command deck; tertiary **At a glance** stat bands (**Overdue**, **Due today**, **Due next 7 days**, **Due now** + **Ready for review** deep link — **A2** + **A4c** + **A5a** + **TASK-UPCOMING-FILTERS-A1**); fetch on mount + **Try again** + **Refresh stats**; silent refresh when mounted after stat-changing actions elsewhere; presentation **8C-2A** + **B2** cockpit width + **B3** stat tiles + **BX-I3** + **BX-I6B** + **BX-I7C** (**`layout.css`** grid only) |
...
| `/tasks` | Yes | **All study tasks** — course + status + **study material** filters (material filter only when a specific course is selected — **TASK-MATERIAL-FILTERS-A2**; includes **Tasks without material** and UUID material options — **TASK-MATERIAL-UNLINKED-A1**); **URL-persisted filters** via query params **`courseId`**, **`materialId`** (UUID or **`none`**), **`status`**, **`deadline=overdue|due_today|next_7_days`** (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1** + **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** + **TASK-MATERIAL-BACKEND-A1** — **`deadline`** implies **`status=pending`**; backend-filtered; **`materialId`** sent on list fetch; **`referenceDate`** sent on fetch but **not** in URL; **`materialId=none`** requires **`courseId`**; Back/Forward restores state; manual filter changes update URL); entry from dashboard deadline links and material detail **View linked tasks** deep link (**TASK-MATERIAL-NAV-B1** — **`/tasks?courseId=&materialId=`**); create, edit pending, optional **due date** (**DASHBOARD-DEPTH-P0**), complete, delete, **Start Focus**; **`TaskCard`** due-date display + material **`Link`** to **`/study-materials/:materialId`** when `task.materialId` set (**TASK-MATERIAL-LINKS-A1**); **`focusReturnTo`** preserves current query string; backend list order **TASK-DUE-SORT-A1** — **Pending**/deadline: **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`**; **Completed**/All: **`created_at DESC` → `id ASC`**; frontend renders API order — **no** sort selector; presentation upgraded **8C-3A** + **B4-A** task command surface / command band, filter toolbar framing, semantic **`ul > li`** task list, native status filter **`aria-pressed`** (**B4-A-F1**), scoped empty/error/loading wrappers + **BX-I7E1** desktop panels at **≥1280px** (horizontal command-controls band, **2-column** populated task list, create FormCard full-width, inline edit intended full width via **`:has(.task-workspace__edit-card)`** — **`layout.css`** only) |
...
- **Dashboard deadline intelligence (beyond DASHBOARD-DEPTH-A2 / A1 / P0 / TASK-DUE-FILTERS-A1 / TASK-UPCOMING-FILTERS-A1 / TASK-DUE-SORT-A1)** — **shipped:** rolling **Last 7 days** focus snapshot (**A1**); optional per-task **`dueDate`** on create/edit + card display (**P0**); deadline-aware dashboard aggregates (**A2** + **TASK-UPCOMING-FILTERS-A1** — **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`dueNext7DaysPendingTasks`**, **`deadlineReferenceDate`**; optional validated **`referenceDate`**; browser-local frontend date alignment; UTC fallback when omitted; recommendation priority including **overdue-tasks** / **due-today-tasks** / **due-next-7-days-tasks**; **At a glance** **Overdue** / **Due today** / **Due next 7 days**); backend-filtered overdue/due-today/next-7-days task lists + global URL **`deadline`** + course-page URL **`deadline`** (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** + **TASK-COURSE-FILTERS-E1** — **`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, **`/tasks?status=pending&deadline=next_7_days`**; **`referenceDate`** not in browser URL); deadline-aware list ordering (**TASK-DUE-SORT-A1**). **Deferred:** unbounded **Upcoming**; **Due soon**; **This week** filters; no-due-date filter; custom date ranges; stored user timezone persistence; calendar context; reminders/notifications; exam dates; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates + A2 aggregates + A1/A1-upcoming deadline filters are **not** a complete deadline-planning system — further dashboard/task deadline phases require explicit approval.
