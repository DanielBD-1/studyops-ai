...
**Not in FLASHCARD-REVIEW-A1:** Frontend Known/Unknown buttons (shipped in **FLASHCARD-REVIEW-A2**); review-state filter UI (shipped in **FLASHCARD-REVIEW-A3**); SRS-lite scheduling columns (shipped in **FLASHCARD-REVIEW-A4a**); full SRS (SM-2/Anki); frontend Due now filter (**A4b**); dashboard due cards (**A4c**); review history table; **`ease_factor`**.
...
**Not in FLASHCARD-REVIEW-A2:** Review-state filter (shipped in **FLASHCARD-REVIEW-A3**); SRS-lite scheduling (shipped in **FLASHCARD-REVIEW-A4a** — backend only; frontend does not filter by due date yet); full SRS (SM-2/Anki); frontend Due now filter; dashboard due cards (**FLASHCARD-REVIEW-A4b** / **A4c** — not shipped); plan JSON review persistence; backend/database/migration changes; auto-advance after review.
...
**Not in FLASHCARD-REVIEW-A3:** SRS-lite scheduling (shipped in **FLASHCARD-REVIEW-A4a** — backend only); frontend Due now filter; full SRS (SM-2/Anki); dashboard due cards; review history table; backend **`GET /api/flashcards?mastery=`** or **`?due=now`**; URL-persisted filters; sorting by **`lastReviewedAt`**; plan JSON review persistence; auto-advance after review; backend/database/migration changes beyond **A4a**.
...
**Not in FLASHCARD-REVIEW-A4a:** Frontend Due now filter (**FLASHCARD-REVIEW-A4b**); dashboard due cards (**FLASHCARD-REVIEW-A4c**); backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; SM-2 / Anki; **`ease_factor`**; review history table; URL-persisted filters; sorting by **`lastReviewedAt`**; auto-advance after review; plan JSON review persistence; sub-day intervals.
...
- Real-time dashboard (polling / WebSockets / cross-tab sync); **full SRS** / SM-2/Anki scheduling; **`ease_factor`**; review history table; frontend Due now filter (**FLASHCARD-REVIEW-A4b**); backend **`GET /api/flashcards?due=now`**; backend mastery/due query params; dashboard due cards (**FLASHCARD-REVIEW-A4c**); URL-persisted flashcard filters; sorting by **`lastReviewedAt`** — **SRS-lite scheduling foundation shipped** in **FLASHCARD-REVIEW-A4a** (`next_review_at`, `review_interval_days`, server-side computation on **`POST /api/flashcards/:flashcardId/review`**, migration **014** applied; **not** full SRS); **aggregate review state + Known/Unknown UI + client-side review-state filter shipped** in **FLASHCARD-REVIEW-A1** + **FLASHCARD-REVIEW-A2** + **FLASHCARD-REVIEW-A3** (migration **013**, saved DB cards only; plan JSON **unchanged**); **Trello stored-token sync (A5A/A5B) and A5C hardening are complete**; see Trello post-A5C deferred items above
