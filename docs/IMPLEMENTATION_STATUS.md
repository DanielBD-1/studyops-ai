...
**`study_tasks` (Phase 3A-a; source extended in Phase 10B; optional due date in Phase DASHBOARD-DEPTH-P0):** Study task rows (`user_id`, `course_id`, optional `material_id`, optional **`due_date`**); RLS by `user_id = auth.uid()`; **`anon` has no access**; `source` CHECK **`manual` or `plan`** (Phase **10B** via `009_plan_source_import_dedupe.sql`). Table **applied and verified** on Supabase (see `docs/database/005-study-tasks-schema-and-rls.md`). **`due_date`** added in migration **015** — applied and verified on Supabase (see `docs/database/015-study-tasks-due-date.md`). **Plan import** (Phase **10B**) creates rows with **`source = plan`** via material-scoped import API; manual create remains **`source = manual`**; plan import does **not** populate **`due_date`**.

**`study_tasks` backend API (Phase 3A-b; due date in DASHBOARD-DEPTH-P0; deadline list filters in TASK-DUE-FILTERS-A1 + TASK-UPCOMING-FILTERS-A1; list ordering in TASK-DUE-SORT-A1):** Express routes with **`requireAuth`**; service-role queries always filter by authenticated `user_id`. Task responses are **camelCase** and include **`dueDate`** (`YYYY-MM-DD` \| `null`) and do **not** include `userId`, study material `content`, or generated `plan` JSON. **`difficulty`** / **`tags`** are returned (defaults on create) but **not client-editable**. **`status`** is **not** PATCHable — use **`POST /api/tasks/:taskId/complete`** only (completion preserves **`dueDate`**). Create: optional **`dueDate`** (omit or `null` → `NULL`; valid date stored). PATCH: omit preserves; `null` clears; valid date replaces. List APIs (**`GET /api/tasks`**, **`GET /api/courses/:courseId/tasks`**) support optional **`materialId`** (**TASK-MATERIAL-BACKEND-A1** — omitted = no material predicate; UUID = owned-material filter; **`none`** = **`material_id IS NULL`** without material ownership lookup), optional **`deadline=overdue|due_today|next_7_days`** + optional **`referenceDate=YYYY-MM-DD`** (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** — see dedicated sections), and backend-owned list ordering (**TASK-DUE-SORT-A1** — **Pending** / deadline lists: **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`**; **Completed** / **All**: **`created_at DESC` → `id ASC`**; **no** public sort query param). Missing/wrong-owner/mismatched material → neutral **`404`** “Study material not found”; valid owned material with zero matches → **`200`** empty array. Wrong-owner or missing task → neutral **`404`** “Task not found”. **Frontend** task UI: course-level Phases **3A-c**–**3A-c.3**, global Phases **3A-d**–**3A-e**, optional due date forms + **`TaskCard`** display (**DASHBOARD-DEPTH-P0**), study material filter UI (**TASK-MATERIAL-FILTERS-A2** + **TASK-MATERIAL-UNLINKED-A1** — controls; membership enforced by backend **TASK-MATERIAL-BACKEND-A1**), URL-persisted global filters including **`materialId`** and **`deadline`** (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1** + **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**), in-memory course deadline filters (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**), material detail related tasks band (**TASK-MATERIAL-DETAIL-A3**); plan import on material detail via Phase **10B** (below); list UI renders API order only (**TASK-DUE-SORT-A1**).

**`flashcards` (Phases 3B-a through 3B-g + FLASHCARD-REVIEW-A1 + FLASHCARD-REVIEW-A2 + FLASHCARD-REVIEW-A3 + FLASHCARD-REVIEW-A4a + FLASHCARD-REVIEW-A4b + FLASHCARD-REVIEW-A4c + FLASHCARD-REVIEW-A5a + FLASHCARD-REVIEW-A5b + FLASHCARD-REVIEW-A5c):**
...
| **FLASHCARD-REVIEW-A4a** | SRS-lite scheduling columns + server-side scheduling on **`POST /api/flashcards/:flashcardId/review`** (backend/database) |
| **FLASHCARD-REVIEW-A4b** | Frontend **Due now** filter for **saved DB flashcards** (material detail + **`/flashcards`**) — client-side by **`nextReviewAt`** |
| **FLASHCARD-REVIEW-A4c** | Dashboard **`dueFlashcardsCount`** on **`GET /api/dashboard/stats`** + **Due now** stat on **`/dashboard`** (aggregate only; link to **`/flashcards`** when count **> 0**) |
| **FLASHCARD-REVIEW-A5a** | Global **`/flashcards`** URL-persisted filters (course/material/review state) — client-side **`reviewState`** only; dashboard deep link to **Due now** filter |
| **FLASHCARD-REVIEW-A5b** | Frontend auto-advance after successful Known/Unknown review on **saved DB flashcards** (material detail + **`/flashcards`**) — pending advance + post-merge deck index |
| **FLASHCARD-REVIEW-A5c** | Dashboard hero / **`deriveDashboardRecommendation`** due-flashcard-aware when **`pendingTasks === 0`** and **`dueFlashcardsCount > 0`** — primary CTA **`/flashcards?reviewState=due_now`** via **`buildFlashcardsPageDueNowLink()`** |

Normalized flashcard rows (`user_id`, `course_id`, optional `material_id`, `question`, `answer`, `tags`, `source` CHECK **`manual` or `plan`** — Phase **10B** via `009_plan_source_import_dedupe.sql`). **Review state (FLASHCARD-REVIEW-A1):** aggregate columns **`mastery`** (`new` \| `learning` \| `known`), **`last_reviewed_at`**, **`review_count`**, **`known_count`**, **`unknown_count`** — migration **`013_flashcard_review_state.sql`** **applied successfully** on Supabase. **SRS-lite scheduling (FLASHCARD-REVIEW-A4a):** **`next_review_at`** (nullable — NULL means due immediately / never reviewed), **`review_interval_days`** (whole-day interval, default **0**, CHECK **`>= 0`**) — migration **`014_flashcard_srs_scheduling.sql`** **applied successfully** on Supabase; scheduling updated **only** via **`POST /review`** server-side computation; **not** full SRS (no SM-2/Anki, no **`ease_factor`**, no review history table). **Review UI (FLASHCARD-REVIEW-A2** + **A5b):** saved DB flashcards on material detail and **`/flashcards`** show **Known** / **Unknown** after answer reveal; frontend calls **`POST /api/flashcards/:flashcardId/review`** and merges response locally (**no** refetch on review); after successful review (**FLASHCARD-REVIEW-A5b**), study UI auto-advances to the next card and resets reveal — card stays in deck → **`getNextIndex`** (wrap); card leaves active filter → same slot when possible; failure → no advance; plan JSON **`FlashcardStudy`** in **`GeneratedPlanSection`** has **no** review buttons (**unchanged** — flip/reveal only). **Review-state filter (FLASHCARD-REVIEW-A3):** **`filterFlashcardsByReviewState`** in **`flashcard-filters.js`** filters already-loaded saved DB cards by **`mastery`** — **All**, **New + Learning** (`needs_review`), **New**, **Learning**, **Known**; affects study deck and manage list on material detail and **`/flashcards`**; composes with course/material filters on global page; **no** backend **`?mastery=`**; **URL-persisted filters (FLASHCARD-REVIEW-A5a):** global **`/flashcards`** only — **`courseId`**, **`materialId`**, **`reviewState`** query params; bidirectional sync; **`reviewState`** client-side only (**not** sent to **`GET /api/flashcards`**); material detail **`DbFlashcardsSection`** **not** URL-persisted; **Due now filter (FLASHCARD-REVIEW-A4b):** **`isFlashcardDueNow`** + **`due_now`** in **`filterFlashcardsByReviewState`** — client-side over already-loaded cards by **`nextReviewAt`** from **A4a** (due: null/undefined/empty/invalid or past/equal timestamp; not due: valid future timestamp); **Due now** option after **All** on material detail and **`/flashcards`**; affects study deck and manage list; **no** backend **`?due=now`**. RLS by `user_id = auth.uid()`; **`anon` has no access**; ownership triggers mirror `study_tasks`. Base table **applied and verified** on Supabase on **2026-05-26** (see `docs/database/006-flashcards-schema-and-rls.md`). Material detail shows **saved DB flashcards** (study + create/edit/delete + review + review-state filter + Due now filter) and **generated-plan flashcards** (both may appear after import). Global page shows **all saved flashcards** with course/material/review-state/**Due now** filters, **create**, study, edit, delete, and review. **No** course-level flashcard management UI. **Dashboard due awareness (FLASHCARD-REVIEW-A4c):** **`GET /api/dashboard/stats`** includes **`dueFlashcardsCount`** — owned saved DB flashcards where **`next_review_at IS NULL OR next_review_at <= now`**; aggregate count only (**no** question/answer); **`/dashboard`** **At a glance** → **Learning assets** shows **Due now** (including **0**); when count **> 0**, **Ready for review** + link to **`/flashcards?reviewState=due_now`** (**FLASHCARD-REVIEW-A5a** deep link); hero due-awareness shipped in **FLASHCARD-REVIEW-A5c**. **SRS-lite foundation only** — **no** backend **`?due=now`**, **no** dashboard charts, **no** admin due cards, SM-2/Anki, or review history table.
...
**PRD drift (approved refinement):** PRD §9 describes `POST /api/courses/:courseId/generate` with `{ studyText }`. The **implemented** route is material-scoped (above). Course-level paste-generate remains **deferred**. **`public.flashcards` table** (3B-b), **backend API** (3B-c + **FLASHCARD-REVIEW-A1** review endpoint + **FLASHCARD-REVIEW-A4a** SRS-lite scheduling on review), **material-detail frontend** (3B-d: saved list + plan import; 3B-e: manual create/edit/delete; **FLASHCARD-REVIEW-A2**: Known/Unknown on saved DB cards; **FLASHCARD-REVIEW-A3**: client-side review-state filter on saved DB cards; **FLASHCARD-REVIEW-A4b**: client-side Due now filter on saved DB cards), and **global `/flashcards`** (3B-f: list/study/filter/edit/delete + **FLASHCARD-REVIEW-A2** review + **FLASHCARD-REVIEW-A3** review-state filter + **FLASHCARD-REVIEW-A4b** Due now filter; 3B-g: global create) exist; **bulk create**, course-level management, full SRS (SM-2/Anki), backend **`?due=now`** / **`?mastery=`** query params, dashboard charts, and other advanced study features remain **deferred** (minimal dashboard due-card awareness shipped in **FLASHCARD-REVIEW-A4c**; **`/flashcards`** URL-persisted filters shipped in **FLASHCARD-REVIEW-A5a** — client-side **`reviewState`** only; not full SRS or admin due cards).
...
|--------|-------|---------|
| `GET` | `/api/courses/:id/tasks` | List tasks for an **owned** course — optional `?status=pending` \| `completed`; optional `?deadline=overdue` \| `due_today` \| `next_7_days` + `?referenceDate=YYYY-MM-DD` (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**) |
| `POST` | `/api/courses/:id/tasks` | Create task — body: `title`, `estimatedMinutes`, optional `description`, `priority`, `materialId` |
| `GET` | `/api/tasks` | List caller’s tasks — optional `?courseId`, `?status=pending` \| `completed`; optional `?deadline=overdue` \| `due_today` \| `next_7_days` + `?referenceDate=YYYY-MM-DD` (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**) |
...
**UI:** Filter buttons rendered above the task list; active filter highlighted; switching filter cancels open edit, closes create form, and refetches. **Create form and "Add study task" button visible on All filter only** (new task always creates `pending`). Filtered empty states: "No pending tasks." / "No completed tasks." — no misleading create CTA. At **3A-c.2 ship**, filter was **in-memory only** — **URL-persisted course-page filters later shipped in TASK-COURSE-FILTERS-E1** (+ **FIX1** race stabilization).

**Not in 3A-c.2:** URL-persisted filters (shipped later in **TASK-COURSE-FILTERS-E1**), `materialId` linking (added in **3A-c.3**), mark incomplete, global `/tasks` (Phase **3A-d**), generated-plan → `study_tasks` import.
...
**Not in TASK-MATERIAL-LINKS-A1:** filter tasks by study material (shipped in **TASK-MATERIAL-FILTERS-A2**); URL-persisted task filters on **`/tasks`** (shipped in **TASK-MATERIAL-FILTERS-E1**); material-scoped task bands on material detail (shipped in **TASK-MATERIAL-DETAIL-A3**); other **`TaskCard`** behavior changes (course link, Start Focus, CRUD actions unchanged).
...
## Implemented — Task material membership backend filter (Phase TASK-MATERIAL-BACKEND-A1)

**Backend + frontend** — task list **material membership** on authenticated list APIs. **No** migration, index, or schema change (reads existing **`study_tasks.material_id`**).

| Endpoint | **`materialId` query** | Behavior |
|----------|-------------------------|----------|
| **`GET /api/tasks`** | omitted | No material predicate (still scoped by authenticated **`user_id`**) |
| **`GET /api/tasks`** | UUID | Verify owned material (global: material→course→user); filter **`material_id = UUID`** |
| **`GET /api/tasks`** | **`none`** | Filter **`material_id IS NULL`** across owned courses — **no** material ownership lookup |
| **`GET /api/courses/:courseId/tasks`** | omitted / UUID / **`none`** | Same contract; UUID verifies material belongs to **route course** |

| Security / validation | Detail |
|-----------------------|--------|
| **Ownership** | All queries filter by authenticated **`user_id`**; course routes enforce course ownership |
| **Material failures** | Missing, wrong-owner, or course/material mismatch → **`404`** neutral **“Study material not found”** — no existence/ownership leak |
| **Empty material** | Valid owned material with zero linked tasks → **`200`** **`[]`** |
| **Invalid query** | **`materialId=all`**, malformed UUID, repeated **`materialId`** → **`400`** **`VALIDATION_ERROR`** — no stack trace or internal DB detail |
| **Composition** | Composes with **`courseId`**, **`status`**, **`deadline`**, **`referenceDate`**; existing invalid pairs unchanged (**`status=completed`** + **`deadline`**; **`referenceDate`** without **`deadline`**) |
| **Ordering** | Unchanged (**TASK-DUE-SORT-A1**) |

**Frontend:** **`GlobalTasksSection`** and **`CourseTasksSection`** map All / UUID / **Tasks without material** → omitted / UUID / **`none`** on list fetch; render API results directly (**no** client-side membership filter). **`MaterialRelatedTasksSection`** requests **`listCourseTasks(courseId, { materialId })`**. Removed **`filterTasksByMaterial`** and **`getMaterialLinkedTasks`**; **`summarizeLinkedTaskCounts`** / **`selectLinkedTaskPreview`** remain presentation-only. Global **`/tasks`** URL persistence unchanged (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1**). At **A1 ship**, **`/courses/:id`** filters were in-memory only — **URL-persisted course-page filters later shipped in TASK-COURSE-FILTERS-E1** (+ **FIX1** race stabilization). Mutations refetch with active filters.

**Not in TASK-MATERIAL-BACKEND-A1:** task detail page; migrations/indexes; new sort modes; Trello/flashcards/admin/generate changes. (**Dashboard material-linked pending task counts shipped later in TASK-MATERIAL-DASHBOARD-A1**; **`/courses/:id`** URL-persisted filters shipped later in **TASK-COURSE-FILTERS-E1**.)

**Implementation files:** `backend/src/modules/tasks/tasks.service.js`, `backend/src/shared/validation/task.schema.js`, `frontend/src/services/tasks.service.js`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/src/components/materials/MaterialRelatedTasksSection.jsx`, `frontend/src/utils/task-filters.js`, backend + frontend tests.

**Reviews:** Supervisor Review — **APPROVE WITH NOTES**; focused Supervisor Re-review — **PASS**; Security Review — **PASS / SECURITY APPROVED**; manual smoke — **PASS WITH NOTES**.

**Tests:** backend **`npm run lint`** passed; backend **`npm test`** **658/658**; frontend **`npm run lint`** passed (3 pre-existing unrelated warnings); frontend **`npm test`** **520/520**; frontend **`npm run build`** passed; **`git diff --check`** clean.

---

## Implemented — Course page URL-persisted task filters (Phase TASK-COURSE-FILTERS-E1)

**Frontend-only** — URL-persisted material/status/deadline filters on **`/courses/:courseId`**. Course ID remains in the route path only. Bidirectional sync between filter state and browser URL via React Router **`useSearchParams`**. List fetch sends active **`materialId`**, **`status`**, **`deadline`**, and **`referenceDate`** to backend (**TASK-MATERIAL-BACKEND-A1** + **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**) — **no** client-side task membership filtering. **No** backend, API, database, migration, document-service, or package changes.

| Query param | Values | Notes |
|-------------|--------|-------|
| **`materialId`** | UUID \| **`none`** | **`none`** = unlinked tasks; invalid UUID stripped; stale UUID preserved until materials hydrate, then reset to **All** and removed from URL |
| **`status`** | `pending` \| `completed` | Invalid values fall back to **All**; omitted when **All** |
| **`deadline`** | `overdue` \| `due_today` \| `next_7_days` | Backend-filtered; implies **`status=pending`**; invalid values canonicalized away; **`referenceDate`** sent on fetch but **not** stored in browser URL |

| Behavior | Detail |
|----------|--------|
| **Open / refresh** | **`readCoursePageInitialTaskFilters`** initializes filter state from URL on first render so filtered deep links do not issue an initial unfiltered list request |
| **Manual filter change** | Updates URL; user-driven changes use **`replace: false`**; all/default values omitted |
| **Back/Forward** | **`searchParams`** changes restore filter state; **`skipUrlSyncRef`** prevents state ↔ URL loops |
| **Deadline / status rules** | Selecting a deadline sets **Pending** and emits **`status=pending&deadline=<value>`**; selecting **Completed** clears deadline from URL; **`?status=completed&deadline=…`** canonicalizes to **`status=completed` only** without extra history entry |
| **Mutation refetch** | Complete/edit refetch preserves active URL-derived filters |
| **`focusReturnTo`** | **`/courses/:courseId?<active-query>`** when filters active |
| **Material-detail link** | Secondary **View tasks on course** → **`/courses/:courseId?materialId=<material-UUID>`** via **`buildCoursePageMaterialLink`**; primary **View linked tasks** remains **`/tasks?courseId=…&materialId=…`** |

**Helpers:** **`parseCoursePageSearchParams`**, **`buildCoursePageSearchParams`**, **`readCoursePageInitialTaskFilters`**, **`resolveCoursePageTaskFilters`**, **`buildCoursePageMaterialLink`** in **`frontend/src/utils/task-nav-query.js`**.

**UI:** **`CourseTasksSection`** — URL sync + filtered **`listCourseTasks`**; **`MaterialRelatedTasksSection`** — secondary course-page deep link.

**Known issue at E1 ship (resolved in FIX1):** Manual browser smoke found overlapping unfiltered + filtered requests on filtered deep links; a slower stale unfiltered response could overwrite the filtered task list while URL/controls stayed correct.

**Not in TASK-COURSE-FILTERS-E1:** backend/API/database/migration/package/CI changes; global **`/tasks`** URL behavior changes beyond shared helpers; dashboard URL filters.

**Implementation files:** `frontend/src/utils/task-nav-query.js`, `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/src/components/materials/MaterialRelatedTasksSection.jsx`, tests/helpers and integration/unit tests.

**Reviews:** Supervisor Review — **APPROVE WITH NOTES**. Security Review — **not required** (frontend-only URL/filter sync over existing authenticated APIs).

**Tests (frontend):** **`npm run lint`** passed; **`npm test`** **573/573**; **`npm run build`** passed. **Git/CI:** implementation **uncommitted** in working tree at documentation gate — **not** merged; main CI green **not** claimed for this bundle.

---

## Implemented — Course deep-link fetch race stabilization (Phase TASK-COURSE-FILTERS-E1-FIX1)

**Focused frontend fix** for stale-request race on **`/courses/:courseId`** filtered deep links and overlapping filter changes. **No** backend, API, database, migration, document-service, or package changes.

| Mechanism | Detail |
|-----------|--------|
| **URL-aware initial state** | **`readCoursePageInitialTaskFilters`** on first render — filtered deep links issue a filtered first request (**no** bare unfiltered precursor) |
| **Latest-request-wins** | **`loadSeqRef`** + course-change invalidation — obsolete success/error/loading completions cannot update current UI |
| **Course switch** | Synchronous filter reset when **`courseId`** changes — course A tasks cannot appear on course B |

**Manual smoke (post-FIX1):** UUID and **`materialId=none`** deep links issue no initial unfiltered request; obsolete responses do not overwrite settled results; course-to-course navigation does not leak prior-course tasks.

**Accepted non-blocking notes:** React Strict Mode may duplicate development requests with the **same** active filters; during rapid filter changes the prior list may remain briefly while the latest request is loading — settled result is correct; **no** dedicated stale-auth-error test — active auth failures still use existing **`handleAuthError`**; obsolete requests cannot trigger stale auth handling.

**Not in FIX1:** `AbortController`; client-side task filtering; artificial delays.

**Implementation files:** `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/src/utils/task-nav-query.js`, race/url-sync tests and helpers.

**Reviews:** Focused Supervisor Review — **APPROVE WITH NOTES**. Security Review — **not required**.

**Tests (frontend):** **`npm run lint`** passed; **`npm test`** **573/573**; **`npm run build`** passed; **`git diff --check`** clean. Manual browser smoke — **PASS WITH NOTES**.

---

## Implemented — Dashboard material-linked pending task counts (Phase TASK-MATERIAL-DASHBOARD-A1)

**Backend + frontend** — material-linked pending-task summary on **`GET /api/dashboard/stats`** and **`/dashboard`**. **No** migration, schema change, or new task list API.

**Response fields (always present on `GET /api/dashboard/stats`):**

| Field | Shape | Rules |
|-------|-------|-------|
| **`materialsWithPendingTasks`** | `number` | Count of owned study materials with **≥ 1** pending **linked** task |
| **`topMaterialsByPendingTasks`** | array (max **5**) | `{ materialId, courseId, materialTitle, pendingTasks }` per row |

**Aggregation / ownership:**

| Rule | Detail |
|------|--------|
| **Pending linked only** | **`status = pending`** and **`material_id IS NOT NULL`** |
| **User-owned tasks** | Authenticated **`user_id`** on **`study_tasks`** |
| **Material metadata** | From **`study_materials`** joined through owned **`courses`** only |
| **Excluded** | Completed tasks; unlinked tasks (**`material_id IS NULL`**); task/material **course mismatches** |
| **Ordering** | **`pendingTasks` DESC → `materialTitle` ASC → `materialId` ASC** (deterministic) |
| **Top list cap** | **5** materials |
| **Queries** | Exactly **two** fixed aggregate-support queries — **no** N+1 |
| **Minimization** | Counts + material id/title/course id only — **no** task bodies, material **`content`**, or plan JSON |

**Dashboard UI (`DashboardStub.jsx`):**

| Section order | Content |
|---------------|---------|
| 1 | Hero (existing recommendation) |
| 2 | Study pulse |
| 3 | **Materials with pending tasks** (new) |
| 4 | Course workload |
| 5 | At a glance |

Each material row shows **material title**, **pending-task count**, **View linked tasks** → **`/tasks?courseId=<courseId>&materialId=<materialId>`** via **`buildTasksPageMaterialLink`**, and **View tasks on course** → **`/courses/:courseId?materialId=<materialId>`** via **`buildCoursePageMaterialLink`**.

**Not in TASK-MATERIAL-DASHBOARD-A1:** task-detail routes; per-material deadline analytics; charts; recommendations; **`totalTasks`** changes; new backend task APIs; deployment; E2E framework coverage.

**Reviews:** Supervisor Review — **APPROVE WITH NOTES**; Security Review — **SECURITY APPROVED WITH NOTES**.

**Tests:** backend **`npm run lint`** passed; backend **`npm test`** **673/673**; frontend **`npm run lint`** passed (**6** non-blocking **`react-refresh/only-export-components`** warnings); frontend **`npm test`** **616/616**; frontend **`npm run build`** passed; **`git diff --check`** clean. Manual browser smoke — initial **FAIL** (global material deep-link hydration); repeat after **FIX2** — **PASS WITH NOTES**. **Git/CI:** bundle **uncommitted** in working tree at documentation gate — repository baseline **`ad15820`**; CI green **not** claimed.

---

## Implemented — Dashboard auth test seam cleanup (Phase TASK-MATERIAL-DASHBOARD-A1-FIX1)

**Focused frontend/test fix** — removed mutable production auth override introduced during initial dashboard runtime test work.

| Removed | Detail |
|---------|--------|
| **`authContextValueForTests`** | Mutable override object |
| **`__setAuthContextValueForTests`** | Global test mutator |
| **`useAuth()` early override path** | Production hook short-circuit |

Tests inject authentication only through test-local **`AuthContext.Provider`**. The ordinary exported React Context was reviewed and accepted — **no** production auth behavior change for real users.

**Reviews:** Focused Supervisor Re-review — **APPROVE WITH NOTES**. Security Review — covered under **TASK-MATERIAL-DASHBOARD-A1** bundle (**SECURITY APPROVED WITH NOTES**).

**Not in FIX1:** new auth APIs; route guard changes; JWT flow changes.

---

## Implemented — Global `/tasks` material deep-link hydration (Phase TASK-MATERIAL-DASHBOARD-A1-FIX2)

**Focused frontend fix** — manual smoke found dashboard **View linked tasks** **`/tasks?courseId=<courseId>&materialId=<materialId>`** lost **`materialId`** while **`/tasks`** hydrated. Dashboard link helper and backend task API were already correct.

| Mechanism | Detail |
|-----------|--------|
| **URL-aware synchronous initial filters** | Seed filter state from the initial URL on first render |
| **Provisional material UUID retention** | Do **not** treat empty pre-hydration material list as stale UUID evidence |
| **Course-specific material hydration state** | Track readiness for the exact parent course |
| **Successful-empty vs not-loaded/error** | Distinguish conclusive empty material load from still-loading / failed load |
| **Canonicalization timing** | Strip invalid **`materialId`** only after **conclusive successful** hydration |
| **`materialId=none` preservation** | Unlinked sentinel unchanged |
| **Latest-request-wins** | Overlapping list requests cannot overwrite current filter results |
| **Navigation** | Back/Forward and refresh preserve deep-link filters |

**Manual smoke (post-FIX2):** Dashboard material row → **View linked tasks** retains **`courseId`** + **`materialId`** through hydrate, refresh, and Back/Forward — **PASS WITH NOTES**.

**Not in FIX2:** backend/API/database/migration changes; course-page filter behavior (**TASK-COURSE-FILTERS-E1-FIX1** unchanged).

**Implementation files:** `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/src/utils/task-nav-query.js`, related tests/helpers.

---

## Implemented — Global `/tasks` same-mount URL navigation test (Phase TASK-MATERIAL-DASHBOARD-A1-FIX2-T1)

**Test-only** — deterministic runtime coverage for same-mount URL navigation **Course A + Material A → Course B + Material B** while Course B material hydration is delayed. Validates **FIX2** provisional retention and latest-request-wins behavior under delayed hydration.

**Not in FIX2-T1:** production code changes beyond tests already landed with **FIX2**.

---

## Implemented — Dashboard runtime suite registration (Phase TASK-MATERIAL-DASHBOARD-A1-T2)

**Test registration only** — registered the previously omitted dashboard runtime integration suite in the standard frontend **`npm test`** command (via **`frontend/package.json`** `test` script wiring only at ship time — **no** new test assertions in **T2** itself).

**Not in T2:** production code changes.

---

## Implemented — Task material filters (Phase TASK-MATERIAL-FILTERS-A2)

> **Superseded (list membership only):** **TASK-MATERIAL-BACKEND-A1** moved task material membership to backend list APIs. Filter UI and composition described below reflect what this phase originally shipped; client-side **`filterTasksByMaterial`** removed — see **TASK-MATERIAL-BACKEND-A1**.
...
|---------|-------------------------|
| **`/courses/:id`** | **Study material** `<select>` always visible; options: **All materials in course**, **Tasks without material** (**TASK-MATERIAL-UNLINKED-A1**), plus each course material from course-page **`materials`** prop; **no** extra materials API call in **`CourseTasksSection`** |
...
**Not in TASK-MATERIAL-FILTERS-A2:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters on **`/tasks`** (shipped in **TASK-MATERIAL-FILTERS-E1**); material detail related tasks band (shipped in **TASK-MATERIAL-DETAIL-A3**); filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1**); Study Rooms / collaboration; PDF; SRS; dashboard charts.
...
> **Superseded (data load only):** **TASK-MATERIAL-BACKEND-A1** — related tasks now load via **`listCourseTasks(courseId, { materialId })`** with backend membership; band UI/counts/preview from this phase remain. Historical client-side filter description below reflects original ship.
...
| **Navigation** | Primary **`Link`** **View linked tasks** → **`/tasks?courseId=<courseId>&materialId=<materialId>`** (**TASK-MATERIAL-NAV-B1**); secondary **`Link`** **View tasks on course** → **`/courses/:courseId`** (shown when not loading/error) |
...
**Not in TASK-MATERIAL-DETAIL-A3:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters on **`/tasks`** (shipped in **TASK-MATERIAL-FILTERS-E1** — B1 **`buildTasksPageMaterialLink`** unchanged); filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1** on task list surfaces only — not in this band); **`TaskCard`** or task actions inside the band; new backend routes or query params.
...
## Implemented — Material detail → /tasks deep link (Phase TASK-MATERIAL-NAV-B1)

> **Superseded (list fetch only):** **TASK-MATERIAL-BACKEND-A1** — deep links still use **`/tasks?courseId=&materialId=`**; list membership now backend-filtered. Historical “client-side only / no backend **`?materialId=`**” notes below reflect original ship.

**Frontend-only** — material detail **View linked tasks** CTA and shared **`/tasks`** query helpers. **No** backend, API, database, migration, document-service, or package changes.

| Aspect | Detail |
|--------|--------|
| **Material detail CTA** | **`MaterialRelatedTasksSection`** — primary **`Link`** **View linked tasks** to **`/tasks?courseId=<courseId>&materialId=<materialId>`** via **`buildTasksPageMaterialLink`**; secondary **View tasks on course** → **`/courses/:courseId`**; counts, preview rows, loading/error/empty behavior unchanged |
| **Query helpers** | **`parseTasksPageSearchParams`**, **`buildTasksPageSearchParams`**, **`resolveInitialTaskFilters`**, **`buildTasksPageMaterialLink`** in **`frontend/src/utils/task-nav-query.js`** — UUID validation for **`courseId`** / **`materialId`**; **`materialId=none`** sentinel added later in **TASK-MATERIAL-UNLINKED-A1** |
| **`/tasks` entry from material detail** | Deep link opens **`/tasks`** with course/material filters initialized from URL when params are valid and owned |
| **Later URL behavior (E1 + UNLINKED)** | **TASK-MATERIAL-FILTERS-E1** expanded **`/tasks`** to **full URL-persisted** bidirectional sync — manual course/material/status filter changes update the URL; Back/Forward restores state; defaults omitted; **`status=pending|completed`** supported; **TASK-MATERIAL-UNLINKED-A1** added **`materialId=none`** with valid **`courseId`** — current **`/tasks`** behavior is **not** initial-load-only |
| **Material filtering** | Client-side over already-loaded tasks (**TASK-MATERIAL-FILTERS-A2**); **`listAllTasks({ courseId?, status? })`** unchanged — **no** backend **`?materialId=`** |
| **Focus return** | **`focusReturnTo`** on **`TaskCard`** preserves full **`/tasks?…`** query string when present |

**Not in TASK-MATERIAL-NAV-B1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted filters on **`/courses/:id`**; URL-persisted dashboard filters; filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1**); **`status`** query param alone (shipped in **TASK-MATERIAL-FILTERS-E1**). (**`/flashcards`** URL-persisted filters shipped separately in **FLASHCARD-REVIEW-A5a**.)

**Implementation files:** `frontend/src/utils/task-nav-query.js`, `frontend/src/components/materials/MaterialRelatedTasksSection.jsx`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/tests/unit/material-linked-tasks.test.js`.

**Reviews:** Supervisor Review — **approved with notes**. Security Review — **not required** (frontend-only navigation and filter initialization over existing authenticated APIs).

**Tests (frontend):** `npm run lint`, `npm test`, `npm run build` passed; main CI **green**. Backend tests not re-run (backend untouched).

---

## Implemented — Task URL-persisted filters (Phase TASK-MATERIAL-FILTERS-E1)

> **Superseded (list fetch only):** **TASK-MATERIAL-BACKEND-A1** — URL sync and filter controls from this phase remain; **`materialId`** is now sent on list fetch. Historical “client-side filter only / not sent to API” rows below reflect original ship.

**Frontend-only** — URL-persisted course/material/status filters on global **`/tasks`** only. Bidirectional sync between filter state and browser URL via React Router **`useSearchParams`**. **No** backend, API, database, migration, document-service, or package changes. At **E1 ship**, **`/courses/:id`** task filters were in-memory only — **URL-persisted course-page filters later shipped in TASK-COURSE-FILTERS-E1** (+ **FIX1** race stabilization).

| Query param | Values | Notes |
|-------------|--------|-------|
| **`courseId`** | UUID | Initialized only when course belongs to loaded user courses |
| **`materialId`** | UUID \| `none` | UUID: initialized only after course materials load and material belongs to that course. **`none`**: unlinked-tasks sentinel — requires valid **`courseId`**; does not wait for materials load; not UUID-validated. **Client-side filter only** — **not** sent to **`GET /api/tasks`** |
| **`status`** | `pending` \| `completed` | Invalid values fall back to **All**; omitted when **All** |
| **`deadline`** | `overdue` \| `due_today` \| `next_7_days` | **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** — backend-filtered; implies **`status=pending`**; invalid values canonicalized away; **`referenceDate`** sent to API on fetch but **not** stored in browser URL |

| Behavior | Detail |
|----------|--------|
| **Open URL** | Parses query string; **`resolveInitialTaskFilters`** validates against loaded **`courses`** / **`filterMaterials`**; invalid/unknown/orphan params safely fall back |
| **Manual filter change** | Updates URL once; user-driven changes use **`replace: false`**; all/default values omitted; **`materialId`** never emitted without **`courseId`**; course change resets material filter and removes **`materialId`** from URL |
| **Back/Forward** | **`searchParams`** changes restore filter state; **`skipUrlSyncRef`** prevents state ↔ URL loops |
| **Invalid URL sanitization** | Canonical query rebuilt with **`replace: true`** only when safe |
| **Backend fetch** | **`listAllTasks({ courseId?, status?, deadline?, referenceDate? })`** — **`deadline`** / **`referenceDate`** sent when deadline filter active (**TASK-DUE-FILTERS-A1**); still **no** backend **`?materialId=`** |
| **Material filter** | Still **`filterTasksByMaterial(tasks, materialFilter, filterMaterials)`** in memory (**TASK-MATERIAL-FILTERS-A2**) |
| **NAV-B1 deep link** | **`buildTasksPageMaterialLink(courseId, materialId)`** from **TASK-MATERIAL-NAV-B1** — **`MaterialRelatedTasksSection`** **View linked tasks** link contract preserved |
| **Focus return** | **`focusReturnTo`** preserves full **`/tasks?…`** query string |

**Helpers:** **`parseTasksPageSearchParams`**, **`buildTasksPageSearchParams`**, **`resolveInitialTaskFilters`**, **`buildTasksPageMaterialLink`** in **`frontend/src/utils/task-nav-query.js`**.

**UI:** **`GlobalTasksSection`** — URL sync effect + **`pushFiltersToUrl`** on course/material/status handlers and after create.

**Not in TASK-MATERIAL-FILTERS-E1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`** (shipped later in **TASK-MATERIAL-BACKEND-A1**); URL-persisted filters on **`/courses/:id`** (shipped later in **TASK-COURSE-FILTERS-E1**); URL-persisted dashboard filters; filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1**); backend/API/database/migration/package/CI changes. (**`/flashcards`** URL-persisted filters shipped separately in **FLASHCARD-REVIEW-A5a**.)

**Implementation files:** `frontend/src/utils/task-nav-query.js`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/tests/unit/material-linked-tasks.test.js`.

**Reviews:** Supervisor Review — **approved with notes**. Security Review — **not required** (frontend-only URL/filter sync over existing authenticated data). Main CI **green**.

**Tests (frontend):** `npm test` **381/381**; lint passed; build passed. Backend tests not re-run (backend untouched).

---

## Implemented — Tasks without material filter (Phase TASK-MATERIAL-UNLINKED-A1)

> **Superseded (list fetch only):** **TASK-MATERIAL-BACKEND-A1** — **`materialId=none`** sentinel and URL rules from this phase remain; unlinked membership now backend-filtered. Historical client-side filter description below reflects original ship.

**Frontend-only** — **Tasks without material** filter on **`/courses/:id`** and global **`/tasks`**. Filters already-loaded task lists for tasks with no study material link. Unlinked detection uses **`task.materialId` only** — null, undefined/missing, or empty string (**not** `materialTitle`). **No** backend, API, database, migration, document-service, or package changes.

| Surface | Behavior |
|---------|----------|
| **`/courses/:id`** | **Study material** `<select>` adds **Tasks without material** (`value="none"`) after **All materials in course**; at **UNLINKED-A1 ship** filtered in memory via **`filterTasksByMaterial`** — **backend membership + URL persistence later shipped in TASK-MATERIAL-BACKEND-A1** + **TASK-COURSE-FILTERS-E1**; composes with status filters |
| **`/tasks`** | Same option when a specific owned course is selected; **`materialId=none`** URL support extends **TASK-MATERIAL-FILTERS-E1** (`/tasks?courseId=<uuid>&materialId=none`); **`awaitingMaterials`** guard skips wait when URL material is **`none`** |
| **Sentinel** | **`materialFilter === 'none'`** |
| **URL rules** | **`materialId=none`** emitted only with valid **`courseId`**; **`/tasks?materialId=none`** without valid course falls back to **all/all**; **`none`** parsed before UUID validation — not treated as a UUID |
| **Backend fetch** | **`listAllTasks({ courseId?, status? })`** and **`listCourseTasks(courseId, status?)`** unchanged — **no** backend **`?materialId=`** |
| **Cross-course** | **No** unlinked filter when **All courses** is selected on **`/tasks`** (material filter hidden — requires selected course) |
| **NAV-B1 deep link** | **`buildTasksPageMaterialLink(courseId, materialId)`** from **TASK-MATERIAL-NAV-B1** — UUID material deep links only |
| **Empty state** | **No tasks without a study material link in this course.** when **`materialFilter === 'none'`** yields zero visible tasks |

**Filter logic:** **`isTaskUnlinked`** + **`filterTasksByMaterial`** branch for **`'none'`** in **`frontend/src/utils/task-filters.js`**. URL sentinel handling in **`parseTasksPageSearchParams`**, **`buildTasksPageSearchParams`**, **`resolveInitialTaskFilters`** in **`frontend/src/utils/task-nav-query.js`**.

**UI:** **`CourseTasksSection`** — option + specific empty state; filter changes cancel create/edit. **`GlobalTasksSection`** — option after **All materials in course**; URL sync via existing E1 bidirectional flow.

**Not in TASK-MATERIAL-UNLINKED-A1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`** (shipped later in **TASK-MATERIAL-BACKEND-A1**); URL-persisted task filters on **`/courses/:id`** (shipped later in **TASK-COURSE-FILTERS-E1**); cross-course global unlinked filter without a selected course; backend/API/database/migration/package/CI changes; **`TaskCard`**, **`MaterialRelatedTasksSection`**, dashboard, Trello, flashcards, or admin changes.

**Implementation files:** `frontend/src/utils/task-filters.js`, `frontend/src/utils/task-nav-query.js`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/tests/unit/task-filters.test.js`, `frontend/tests/unit/material-linked-tasks.test.js`.

**Reviews:** Supervisor Review — **approved with notes**. Security Review — **not required** (frontend-only filter over already authenticated, already-loaded task data). Main CI **green**.

**Tests (frontend):** `npm test` **393/393**; lint passed; build passed. Backend tests not re-run (backend untouched).
...
| Method | Route | Purpose |
|--------|-------|---------|
| `GET` | `/api/dashboard/stats` | Return caller-owned aggregate dashboard stats (**200**); optional query **`referenceDate=YYYY-MM-DD`** (**DASHBOARD-DEPTH-A2**) |

**Query (DASHBOARD-DEPTH-A2):**

| Param | Behavior |
|-------|----------|
| **`referenceDate` omitted** | Server uses **UTC calendar date** as **`deadlineReferenceDate`** |
| **`referenceDate=YYYY-MM-DD`** (valid) | Supplied date used as **`deadlineReferenceDate`** |
| Malformed, empty, timestamp, impossible date, repeated-array form, or unknown keys | **400** **`VALIDATION_ERROR`** |

Frontend sends the **browser-local calendar date** on every dashboard stats request so overdue/due-today counts align with the user's local day.

**Response fields:**

- **`totalCourses`**, **`totalStudyMaterials`**, **`totalGeneratedPlans`** (active generated plans only — Phase **11A-1**), **`totalTasks`**, **`pendingTasks`**, **`completedTasks`**, **`totalFlashcards`**, **`dueFlashcardsCount`** (**FLASHCARD-REVIEW-A4c** — owned saved DB flashcards where **`next_review_at IS NULL OR next_review_at <= now`**; count-only query; **no** question/answer fields)
- **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`deadlineReferenceDate`** (**DASHBOARD-DEPTH-A2** — aggregate counts only; **`deadlineReferenceDate`** echoes the effective reference calendar date; overdue = caller-owned **pending** tasks with non-null **`due_date` < `deadlineReferenceDate`**; due today = pending with **`due_date` = `deadlineReferenceDate`**; completed, null-date, future, and other-user tasks excluded)
...
- **`focusMinutesLast7Days`** — sum of **`duration_minutes`** for completed sessions with **`ended_at >= now - 168 hours`** (rolling window, not calendar week — **DASHBOARD-DEPTH-A1**)
- **`completedFocusSessionsLast7Days`** — count of those last-7-days completed sessions (**DASHBOARD-DEPTH-A1**)
...
- **`materialsWithPendingTasks`**, **`topMaterialsByPendingTasks[]`** (**TASK-MATERIAL-DASHBOARD-A1** — pending **linked** tasks only; user-owned; material metadata from owned courses; task/material course mismatches excluded; completed/unlinked excluded; top list capped at **5**; order **`pendingTasks` DESC → `materialTitle` ASC → `materialId` ASC**; exactly **two** fixed aggregate-support queries — **no** N+1)
- **`courseStats[]`** — per owned course: **`courseId`**, **`courseName`**, **`totalTasks`**, **`completedTasks`**, **`totalFlashcards`**

**Data minimization:** Counts and per-course aggregates only — **no** study material **`content`**, generated **`plan`** JSON, task **`description`**, flashcard **`question`**/**`answer`**, Trello credentials/card IDs, or raw DB rows.
...
**Visible sections (baseline 5C + later depth phases; section order updated in TASK-MATERIAL-DASHBOARD-A1):**

| Order | Section | Displayed fields / behavior |
|-------|---------|----------------------------|
| 1 | **Hero** | Rule-based recommendation (**BX-I3** + deadline/due-flashcard awareness) |
| 2 | **Study pulse** | Task progress + **Last 7 days** focus band (**DASHBOARD-DEPTH-A1**) |
| 3 | **Materials with pending tasks** (**TASK-MATERIAL-DASHBOARD-A1**) | **`materialsWithPendingTasks`**; up to **5** rows from **`topMaterialsByPendingTasks[]`** — **`materialTitle`**, **`pendingTasks`**, **View linked tasks** → **`/tasks?courseId=&materialId=`**, **View tasks on course** → **`/courses/:courseId?materialId=`** |
| 4 | **Course workload** | `courseStats[]` rows (**BX-I3** / **BX-I4** / **BX-I6B**) |
| 5 | **At a glance** | Overview, Tasks, Focus, Learning assets (**Due now** + **Ready for review** — **A4c** + **A5a**), Trello; **Overdue** / **Due today** / **Due next 7 days** (**A2** + **TASK-UPCOMING-FILTERS-A1**) |

**Legacy 5C band names (now under At a glance or superseded):** Overview (`totalCourses`, `totalStudyMaterials`, `totalGeneratedPlans`); Tasks (`totalTasks`, `pendingTasks`, `completedTasks`); Focus (`totalFocusMinutes`, `completedFocusSessions`); Learning assets (`totalFlashcards`, **`dueFlashcardsCount`**); Trello (`trelloSyncedTasks`).
...
**Not in 3B-f:** Global create flashcard UI (shipped in **3B-g**); course-level flashcard management; known/unknown; spaced repetition; Anki; URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only); pagination/rate limiting.
...
**Not in 3B-g:** Bulk create; AI/Gemini generation; plan import on `/flashcards`; known/unknown; spaced repetition; Anki; URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only); course-level management; shared CRUD extraction.
...
| **Responses** | **`GET /api/flashcards`**, create, PATCH, and review return camelCase flashcard including **`mastery`**, **`lastReviewedAt`**, **`reviewCount`**, **`knownCount`**, **`unknownCount`** (at A1 ship); scheduling fields **`nextReviewAt`** and **`reviewIntervalDays`** added in **FLASHCARD-REVIEW-A4a**; **no** **`userId`** |
...
**Not in FLASHCARD-REVIEW-A1:** Frontend Known/Unknown buttons (shipped in **FLASHCARD-REVIEW-A2**); review-state filter UI (shipped in **FLASHCARD-REVIEW-A3**); SRS-lite scheduling columns (shipped in **FLASHCARD-REVIEW-A4a**); frontend Due now filter (shipped in **FLASHCARD-REVIEW-A4b**); full SRS (SM-2/Anki); dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); review history table; **`ease_factor`**.
...
| **UX gates (at A2 ship)** | Known/Unknown only when **`onReviewOutcome`**, answer revealed, and **`card.id`** present; buttons disabled while saving; at A2 ship user stayed on same card (**no** auto-advance); auto-advance after successful review shipped in **FLASHCARD-REVIEW-A5b** |
...
**Not in FLASHCARD-REVIEW-A2:** Review-state filter (shipped in **FLASHCARD-REVIEW-A3**); SRS-lite scheduling (shipped in **FLASHCARD-REVIEW-A4a**); frontend Due now filter (shipped in **FLASHCARD-REVIEW-A4b**); full SRS (SM-2/Anki); dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); plan JSON review persistence; backend/database/migration changes; auto-advance after review (shipped in **FLASHCARD-REVIEW-A5b**).
...
| Layer | What exists |
|-------|-------------|
| **Filter helper** | **`filterFlashcardsByReviewState(flashcards, reviewFilter)`** in **`flashcard-filters.js`** — **`all`** returns all cards; **`needs_review`** → **`new`** + **`learning`**; **`new`** / **`learning`** / **`known`** exact **`mastery`** match; **`due_now`** shipped in **FLASHCARD-REVIEW-A4b** (see below); unknown filter value returns all (defensive); empty or non-array input → **`[]`** |
| **Global `/flashcards`** | **`GlobalFlashcardsSection`** — **`reviewFilter`** state (default **`all`**); **`displayedFlashcards`** via helper; study deck and manage list use **`displayedFlashcards`**; **Review state** select (**All**, **Due now** — **A4b**, **New + Learning**, **New**, **Learning**, **Known**) composes with course/material filters; filtered-empty vs true-empty messaging |
...
| **A2 preserved (at A3 ship)** | Known/Unknown unchanged — same **`POST /api/flashcards/:flashcardId/review`**; local merge by id; **no** refetch on review; auto-advance shipped separately in **FLASHCARD-REVIEW-A5b** |
...
**Not in FLASHCARD-REVIEW-A3:** SRS-lite scheduling (shipped in **FLASHCARD-REVIEW-A4a**); frontend Due now filter (shipped in **FLASHCARD-REVIEW-A4b**); full SRS (SM-2/Anki); dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only); review history table; backend **`GET /api/flashcards?mastery=`** or **`?due=now`**; sorting by **`lastReviewedAt`**; plan JSON review persistence; auto-advance after review (shipped in **FLASHCARD-REVIEW-A5b**); backend/database/migration changes beyond **A4a**.

---

## Implemented — Flashcard SRS-lite scheduling backend (Phase FLASHCARD-REVIEW-A4a)

**Backend + database only** — SRS-lite scheduling foundation on existing **`public.flashcards`** rows. Computes **`next_review_at`** and **`review_interval_days`** server-side on review. **Not** full SRS. **No** frontend changes in this phase. Plan JSON flashcards in **`GeneratedPlanSection`** **unchanged**.

| Layer | What exists |
|-------|-------------|
| **Migration** | **`supabase/migrations/014_flashcard_srs_scheduling.sql`** — **applied successfully** on Supabase; adds **`next_review_at`** (`timestamptz`, nullable) and **`review_interval_days`** (`integer` NOT NULL default **0**, CHECK **`review_interval_days >= 0`**); **no** **`ease_factor`**; **no** review history table; **no** new indexes; existing RLS row policies unchanged |
| **Scheduling helper** | **`computeNextReviewState`** in **`flashcard-scheduling.js`** — pure server-side; **`MAX_INTERVAL_DAYS = 90`**; whole-day intervals only |
| **Algorithm** | **`known`** from **`new`** or **`learning`** → **`mastery = known`**, **`review_interval_days = 1`**, **`next_review_at = now + 1 day`**; **`known`** from **`known`** with interval **0** → interval **1**, next review in **1** day; **`known`** from **`known`** with interval **≥ 1** → interval **`min(previous × 2, 90)`**, **`next_review_at = now + new interval`**; **`unknown`** from any state → **`mastery = learning`**, **`review_interval_days = 0`**, **`next_review_at = now + 1 day`** |
| **Backend API** | **`POST /api/flashcards/:flashcardId/review`** — unchanged strict body **`{ outcome: "known" \| "unknown" }`**; **`reviewFlashcard`** loads owned card, computes scheduling via helper, writes **`mastery`**, counters, **`last_reviewed_at`**, **`next_review_at`**, **`review_interval_days`**; wrong-owner/missing → neutral **`404`** “Flashcard not found” |
| **Bypass protection** | Create/PATCH schemas and service whitelists **do not** accept scheduling or review fields; client cannot send **`nextReviewAt`**, **`reviewIntervalDays`**, **`mastery`**, counters, or **`userId`** on review/create/PATCH |
| **Responses** | List/create/PATCH/review return camelCase flashcard including **`nextReviewAt`** and **`reviewIntervalDays`** plus A1 review fields; **no** **`userId`** |

**Implementation files:** `supabase/migrations/014_flashcard_srs_scheduling.sql`, `backend/src/modules/flashcards/flashcard-scheduling.js`, `backend/src/modules/flashcards/flashcards.service.js`, `backend/tests/unit/flashcard-scheduling.test.js`, `backend/tests/unit/flashcards.service.test.js`, `backend/tests/integration/flashcards.test.js`, `backend/tests/helpers/mockSupabaseFlashcards.js`.

**Tests:** `cd backend && npm run lint` and `npm test` passed. Main CI **green**.

**Reviews:** Supervisor Review **approved with notes**. Security Review **approved with notes** (read-modify-write concurrency on counters/scheduling accepted for A4a — same as A1).

**Release order:** Migration **014** must be applied before deploying this backend revision against real Supabase (backend selects **`next_review_at`** and **`review_interval_days`** on all flashcard queries). **Applied successfully** on Supabase.

**Not in FLASHCARD-REVIEW-A4a:** Frontend Due now filter (shipped in **FLASHCARD-REVIEW-A4b**); dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; SM-2 / Anki; **`ease_factor`**; review history table; URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only); sorting by **`lastReviewedAt`**; auto-advance after review (shipped in **FLASHCARD-REVIEW-A5b**); plan JSON review persistence; sub-day intervals.

---

## Implemented — Flashcard Due now filter (Phase FLASHCARD-REVIEW-A4b)

**Frontend-only** — client-side **Due now** filter for **saved DB flashcards** already loaded from the API. Uses **`nextReviewAt`** from **FLASHCARD-REVIEW-A4a**. **Not** full SRS. **No** backend query params. Plan JSON flashcards in **`GeneratedPlanSection`** **unchanged**.

| Layer | What exists |
|-------|-------------|
| **Due helper** | **`isFlashcardDueNow(card, now?)`** in **`flashcard-filters.js`** — due when **`nextReviewAt`** is null, undefined, empty string, invalid date, or **`Date.parse(nextReviewAt) <= now`**; not due when **`nextReviewAt`** is a valid future timestamp |
| **Filter helper** | **`filterFlashcardsByReviewState`** — **`due_now`** case filters via **`isFlashcardDueNow`**; existing **`all`**, **`needs_review`**, **`new`**, **`learning`**, **`known`** unchanged; unknown filter value returns all (defensive) |
| **Global `/flashcards`** | **`GlobalFlashcardsSection`** — **Review state** select adds **Due now** after **All**; **`displayedFlashcards`** via helper; study deck and manage list use **`displayedFlashcards`**; due-only empty copy **No flashcards are due for review right now.** when cards exist but none are due |
| **Material detail** | **`DbFlashcardsSection`** — same **Due now** / **`displayedFlashcards`** pattern in saved DB flashcards area only (plan JSON section unchanged) |
| **A2 / A3 preserved (at A4b ship)** | Known/Unknown unchanged — same **`POST /api/flashcards/:flashcardId/review`**; local merge by id; **no** refetch on review; auto-advance shipped separately in **FLASHCARD-REVIEW-A5b**; mastery filters (**All**, **New + Learning**, **New**, **Learning**, **Known**) unchanged |
| **Plan JSON** | **`GeneratedPlanSection`** — **no** Due now filter; **no** Known/Unknown; **no** review API |
| **Service typedef** | **`Flashcard`** JSDoc in **`flashcards.service.js`** documents **`nextReviewAt`** and **`reviewIntervalDays`** — **no** runtime service behavior change |

**Implementation files:** `frontend/src/utils/flashcard-filters.js`, `frontend/src/components/flashcards/GlobalFlashcardsSection.jsx`, `frontend/src/components/materials/DbFlashcardsSection.jsx`, `frontend/src/services/flashcards.service.js` (typedef only), `frontend/tests/unit/flashcard-filters.test.js`, `frontend/tests/unit/flashcard-study-review.test.js`.

**Tests:** `cd frontend && npm run lint`, `npm test`, `npm run build` passed. Main CI **green**.

**Reviews:** Supervisor Review **approved**. Security Review **not required** (frontend-only client-side filtering over already-loaded authenticated data).

**Not in FLASHCARD-REVIEW-A4b:** Dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only); backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; full SRS (SM-2/Anki); **`ease_factor`**; review history table; notifications; calendar scheduling; auto-advance after review (shipped in **FLASHCARD-REVIEW-A5b**); plan JSON review persistence; backend/database/migration changes.

---

## Implemented — Flashcard dashboard due-card awareness (Phase FLASHCARD-REVIEW-A4c)

**Backend + frontend** — minimal dashboard due-card awareness for **saved DB flashcards** on the existing dashboard stats endpoint and **`/dashboard`** UI. **Not** full SRS. **No** new endpoint. **No** hero / **`deriveDashboardRecommendation`** changes at A4c ship (hero due-awareness shipped in **FLASHCARD-REVIEW-A5c**). Plan JSON flashcards **unchanged**.

| Layer | What exists |
|-------|-------------|
| **Backend stats** | **`GET /api/dashboard/stats`** response adds **`dueFlashcardsCount`** — aggregate **`count`** on owned **`flashcards`** where **`next_review_at IS NULL OR next_review_at <= now`**; **`user_id`** scoped; **`select('id', { count: 'exact', head: true })`** only — **no** question/answer |
| **Unchanged backend fields** | **`totalFlashcards`** (all owned cards), **`courseStats`** (per-course flashcard counts) — behavior unchanged |
| **Dashboard UI** | **`DashboardStub.jsx`** **At a glance** → **Learning assets** **`StatItem`** **Due now** with **`dueFlashcardsCount`** ( **`?? 0`** fallback); when count **> 0**, detail shows **Ready for review** + **`Link`** to **`/flashcards?reviewState=due_now`** (**Review flashcards**) — deep link added in **FLASHCARD-REVIEW-A5a** |
| **Service typedef** | **`DashboardStats`** in **`dashboard.service.js`** includes **`dueFlashcardsCount: number`** — **no** other runtime API behavior change |
| **A4b + A5a** | Client-side **Due now** filter on **`/flashcards`** / material detail; dashboard **Ready for review** opens **`/flashcards?reviewState=due_now`** (**A5a**); hero due-awareness shipped in **FLASHCARD-REVIEW-A5c** |

**Implementation files:** `backend/src/modules/dashboard/dashboard.service.js`, `backend/tests/helpers/mockSupabaseDashboard.js`, `backend/tests/integration/dashboard.test.js`, `frontend/src/services/dashboard.service.js`, `frontend/src/pages/DashboardStub.jsx`, `frontend/tests/unit/dashboard.service.test.js`, `frontend/tests/unit/dashboard-format.test.js`.

**Tests:** `cd backend && npm run lint`, `npm test`; `cd frontend && npm run lint`, `npm test`, `npm run build` passed. Main CI **green**.

**Reviews:** Supervisor Review **approved with notes**. Security Review **not required** (read-only aggregate extension on existing authenticated dashboard endpoint; backend security spot-check passed).

**Not in FLASHCARD-REVIEW-A4c:** Dashboard charts; backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a**); deep link to Due now filter (shipped in **FLASHCARD-REVIEW-A5a**); hero / recommendation logic changes (shipped in **FLASHCARD-REVIEW-A5c**); admin dashboard due cards; full SRS (SM-2/Anki); **`ease_factor`**; review history table; notifications; calendar scheduling; flashcard Q/A in dashboard response; new endpoint; migrations; packages.

---

## Implemented — Flashcard URL-persisted filters (Phase FLASHCARD-REVIEW-A5a)

**Frontend-only** — URL-persisted course/material/review-state filters on global **`/flashcards`** only. Bidirectional sync between filter state and browser URL via React Router **`useSearchParams`**. **`reviewState`** is client-side only — **not** sent to **`GET /api/flashcards`**. **No** backend, API, database, migration, document-service, or package changes. Material detail **`DbFlashcardsSection`** filters remain in-memory only (**no** URL query params on **`/study-materials/:materialId`**). Plan JSON **`GeneratedPlanSection`** flashcards **unchanged**. Hero due-awareness shipped separately in **FLASHCARD-REVIEW-A5c**.

| Query param | Values | Notes |
|-------------|--------|-------|
| **`courseId`** | UUID | Initialized only when course belongs to loaded user courses |
| **`materialId`** | UUID | Initialized only after course materials load and material belongs to that course; **never** emitted without **`courseId`** |
| **`reviewState`** | `due_now` \| `needs_review` \| `new` \| `learning` \| `known` | Client-side filter only — **not** sent to backend; **`all`** omitted from URL; invalid values fall back to **All** and are stripped |

| Behavior | Detail |
|----------|--------|
| **Open URL** | Parses query string; validates owned course/material; invalid/stale **`courseId`** / **`materialId`** fall back safely and canonicalize URL |
| **Manual filter change** | Updates URL; user-driven changes use **`replace: false`**; all/default values omitted; **`reviewState=all`** not emitted; course change resets material filter and removes **`materialId`** from URL |
| **Back/Forward** | **`searchParams`** changes restore filter state; **`skipUrlSyncRef`** prevents state ↔ URL loops |
| **Invalid URL sanitization** | Invalid **`reviewState`** falls back safely and is stripped; canonical query rebuilt with **`replace: true`** when safe |
| **Backend fetch** | **`listFlashcards({ courseId?, materialId? })`** unchanged — **no** backend **`?due=now`** or **`?mastery=`** |
| **Review filter** | Still **`filterFlashcardsByReviewState`** in memory (**FLASHCARD-REVIEW-A3** + **A4b**) |
| **Dashboard deep link** | When **`dueFlashcardsCount > 0`**, **Ready for review** links to **`/flashcards?reviewState=due_now`** |

**Helpers:** **`parseFlashcardsPageSearchParams`**, **`buildFlashcardsPageSearchParams`**, **`resolveInitialFlashcardFilters`** in **`frontend/src/utils/flashcard-nav-query.js`**.

**UI:** **`GlobalFlashcardsSection`** — URL sync via **`useSearchParams`** + **`skipUrlSyncRef`**.

**Not in FLASHCARD-REVIEW-A5a:** Backend **`GET /api/flashcards?due=now`** or **`?mastery=`**; URL-persisted filters on **`/study-materials/:materialId`**; plan JSON review persistence; auto-advance after review (shipped in **FLASHCARD-REVIEW-A5b**); full SRS (SM-2/Anki); review history table; dashboard hero / recommendation logic changes (shipped in **FLASHCARD-REVIEW-A5c**); backend/API/database/migration/package/CI changes.

**Implementation files:** `frontend/src/utils/flashcard-nav-query.js`, `frontend/src/components/flashcards/GlobalFlashcardsSection.jsx`, `frontend/src/pages/DashboardStub.jsx` (Due now deep link only).

**Tests:** `frontend/tests/unit/flashcard-nav-query.test.js` added to **`frontend/package.json`** `test` script. `cd frontend && npm run lint`, `npm test`, `npm run build` passed. Main CI **green**.

**Reviews:** Supervisor Review **approved with notes**. Security Review **not required** (frontend-only URL/filter sync over existing authenticated data).

---

## Implemented — Flashcard review auto-advance (Phase FLASHCARD-REVIEW-A5b)

**Frontend-only** — after successful **Known** / **Unknown** on **saved DB flashcards**, study UI auto-advances to the next card and resets reveal. Pending advance + effect uses latest parent **`flashcards`** props after local merge/re-render. **No** backend, API, database, migration, URL, or plan JSON changes.

| Layer | What exists |
|-------|-------------|
| **Surfaces** | Global **`/flashcards`** **`GlobalFlashcardsSection`** and material detail **`DbFlashcardsSection`** — both wire **`onReviewOutcome`**; parent **`handleReviewOutcome`** returns **`true`** on success, **`false`** on all error paths |
| **Advance helper** | **`computeIndexAfterSuccessfulReview(indexBeforeReview, reviewedCardId, nextFlashcards)`** in **`flashcard-study.js`** — reviewed card leaves filtered deck → **`Math.min(indexBeforeReview, nextTotal - 1)`**; card stays → **`getNextIndex`** (wrap); empty deck → **0** (parent empty-state unmounts **`FlashcardStudy`**) |
| **Race avoidance** | **`FlashcardStudy.jsx`** — **`await onReviewOutcome(...)`** then set **`pendingAdvanceRef`** + **`skipDeckResetRef`**; separate **`useEffect([flashcards, studySetKey])`** computes index from post-merge deck — **not** immediately after **`await`**; deck-reset effect skips when pending advance is queued |
| **Failure path** | Strict **`result !== true`** → no advance; existing review error/success messaging unchanged; manual **Next** / **Previous** unchanged (**disabled** while **`reviewing`**) |
| **Plan JSON** | **`GeneratedPlanSection`** — **no** **`onReviewOutcome`**; flip/reveal only (**unchanged**) |
| **A5a preserved** | **`/flashcards`** URL-persisted filters and **`flashcard-nav-query.js`** **unchanged**; hero due-awareness shipped separately in **FLASHCARD-REVIEW-A5c** |

**Implementation files:** `frontend/src/utils/flashcard-study.js`, `frontend/src/components/materials/FlashcardStudy.jsx`, `frontend/src/components/flashcards/GlobalFlashcardsSection.jsx`, `frontend/src/components/materials/DbFlashcardsSection.jsx`, `frontend/tests/unit/flashcard-study.test.js`, `frontend/tests/unit/flashcard-study-review.test.js`.

**Tests:** `cd frontend && npm run lint`, `npm test` (**423** pass), `npm run build` passed. Main CI **green**.

**Reviews:** Supervisor Review **approved with notes**. Security Review **not required** (frontend-only UX over existing authenticated review API).

**Not in FLASHCARD-REVIEW-A5b:** Backend/database/migration changes; URL changes; plan JSON review persistence; full SRS (SM-2/Anki); **`ease_factor`**; review history table; backend **`GET /api/flashcards?due=now`** or **`?mastery=`**; dashboard hero / recommendation logic changes (shipped in **FLASHCARD-REVIEW-A5c**); **`flashcards.service.js`** runtime behavior changes.

---

## Implemented — Dashboard hero due-flashcard recommendation (Phase FLASHCARD-REVIEW-A5c)

**Frontend-only** — rule-based dashboard hero / **`deriveDashboardRecommendation`** now due-flashcard-aware using existing **`dueFlashcardsCount`** from **`GET /api/dashboard/stats`**. **No** backend, API, database, migration, or new dashboard stats fields at A5c ship.

| Layer | What exists |
|-------|-------------|
| **Logic** | **`deriveDashboardRecommendation`** in **`dashboard-recommendation.js`** — reads **`dueFlashcardsCount ?? 0`**; new **kind** **`due-flashcards`** when **`pendingTasks === 0`** and **`dueFlashcardsCount > 0`** |
| **Priority order (at A5c ship)** | **no-courses → pending-tasks → due-flashcards → plan-gap → flashcards → add-tasks-or-plans → empty-workspace → caught-up** |
| **Superseded by DASHBOARD-DEPTH-A2** | Current priority: **no-courses → overdue-tasks → due-today-tasks → pending-tasks → due-flashcards → …** (remaining branches unchanged) |
| **Pending tasks win (at A5c ship)** | When **`pendingTasks > 0`**, **`pending-tasks`** recommendation unchanged even if **`dueFlashcardsCount > 0`** — **A2** adds **`overdue-tasks`** and **`due-today-tasks`** above **`pending-tasks`** |
| **Due hero copy** | Headline: **`{N} flashcard(s) due for review.`** (singular/plural); context: *Based on your review schedule and study stats.* |
| **Primary CTA** | **`Review due flashcards`** → **`buildFlashcardsPageDueNowLink()`** → **`/flashcards?reviewState=due_now`** |
| **Generic flashcards** | When **`dueFlashcardsCount === 0`** and **`totalFlashcards > 0`**, existing **`flashcards`** kind unchanged (**`/flashcards`** without query) |
| **Presentation at A5c ship** | **`DashboardDecisionHero`** renders recommendation object only — **no** Study pulse **Last 7 days** band yet (**DASHBOARD-DEPTH-A1** superseded presentation scope below) |
| **At a glance** | **Learning assets → Due now** stat + **Ready for review** link (**A4c** + **A5a**) unchanged — tertiary band alongside hero |
| **A5a / A5b preserved** | URL-persisted **`/flashcards`** filters and auto-advance after successful review **unchanged** |

**Superseded by DASHBOARD-DEPTH-A1 (historical — do not erase A5c context):** At A5c ship, docs recorded **`DashboardStub.jsx` unchanged** and **`pending-tasks`** primary CTA to bare **`/tasks`**. **DASHBOARD-DEPTH-A1** later added the Study pulse **Last 7 days** band and filtered pending links **`/tasks?status=pending`** / **`/tasks?courseId=<id>&status=pending`** — see below.

**Implementation files:** `frontend/src/utils/dashboard-recommendation.js`, `frontend/tests/unit/dashboard-format.test.js`.

**Tests:** `cd frontend && npm run lint`, `npm test` (**426** pass), `npm run build` passed. Main CI **green**.

**Reviews:** Supervisor Review **approved with notes**. Security Review **not required** (frontend-only over existing aggregate stat).

**Not in FLASHCARD-REVIEW-A5c:** Backend/database/migration changes; new dashboard stats fields; URL contract changes at A5c; **`flashcard-nav-query.js`** changes; plan JSON review persistence; full SRS (SM-2/Anki); **`ease_factor`**; review history table; backend **`GET /api/flashcards?due=now`** or **`?mastery=`**; dashboard charts; admin due-card metrics; notifications; calendar scheduling; **`A5a`** / **`A5b`** behavior changes; **Last 7 days** focus snapshot (**DASHBOARD-DEPTH-A1**); filtered pending-task hero links (**DASHBOARD-DEPTH-A1**).

---

## Implemented — Dashboard last-7-days focus snapshot (Phase DASHBOARD-DEPTH-A1)

**Backend + frontend** — rolling focus activity on existing **`GET /api/dashboard/stats`** and **`/dashboard`** Study pulse. **Not** charts, streaks, goals, deadline-aware dashboard logic, or AI scheduling.

| Layer | What exists |
|-------|-------------|
| **API fields** | **`focusMinutesLast7Days`** (number), **`completedFocusSessionsLast7Days`** (number) |
| **Window** | Rolling **168 hours** from server `now` (`computeLast7DaysThresholdIso`) — **not** a calendar week |
| **Count rule** | **Completed sessions only** — **`ended_at IS NOT NULL`** and **`ended_at >= threshold`**; in-progress/abandoned excluded |
| **Study pulse UI** | **Last 7 days** band — Focus time (`formatFocusMinutes`) + Completed focus sessions |
| **Pending hero links** | **`pending-tasks`** primary CTA → **`/tasks?status=pending`** (`buildTasksPagePendingLink`); optional secondary → **`/tasks?courseId=<uuid>&status=pending`** for most-pending course (`buildTasksPageCoursePendingLink`) |
| **Data minimization** | Aggregate minutes/counts only — no session rows, task titles, or material content |

**Merge:** feature commit **`c746333`**; merge commit **`bb499ce`**. **Reviews:** Supervisor Review approved; Security Review approved. Main CI **green**.

**Not in DASHBOARD-DEPTH-A1:** Dashboard overdue/due-today task counts; deadline-aware recommendations; due-date sorting/filtering/URL params; charts; streaks; goals; AI scheduling.

**Implementation files:** `backend/src/modules/dashboard/dashboard.service.js`, `frontend/src/pages/DashboardStub.jsx`, `frontend/src/services/dashboard.service.js`, `frontend/src/utils/dashboard-recommendation.js`, `frontend/src/utils/task-nav-query.js`, related dashboard tests.

---

## Implemented — Task optional due dates (Phase DASHBOARD-DEPTH-P0)

**Backend + frontend + database** — optional day-level due date on **`study_tasks`**. Migration **015** applied and verified on Supabase; persistence smoke test passed. See **`docs/database/015-study-tasks-due-date.md`**.

| Layer | What exists |
|-------|-------------|
| **Database** | **`public.study_tasks.due_date date null`** — no default, no backfill, no index, no trigger, no RLS change; existing rows **NULL** |
| **API** | Responses include **`dueDate: YYYY-MM-DD \| null`**; create omit/`null` → **NULL**; PATCH omit preserves; PATCH `null` clears; PATCH valid date replaces; past dates allowed; years **0001–9999**; validation via **`calendar-date.js`** |
| **UI** | Optional due date on course + global task create/edit; **`TaskCard`** + material related-task preview display (future / today / overdue pending / completed-neutral; year when needed; no line when unset) |
| **Preserved** | Plan import does **not** set due dates; Gemini schema unchanged; Trello due dates unchanged; dashboard recommendations **unchanged at P0 ship** (deadline-aware aggregates shipped in **DASHBOARD-DEPTH-A2**); list order **`created_at DESC`**; complete preserves due date |

**Verification:** backend tests **533/533**; frontend tests **486/486**; frontend build passed. **Reviews:** Supervisor Re-review approved; Security Review approved.

**Not in DASHBOARD-DEPTH-P0 (deferred at P0 ship — superseded for dashboard aggregates by DASHBOARD-DEPTH-A2; task-list overdue/due-today filters shipped in TASK-DUE-FILTERS-A1; bounded next-7-days filter shipped in TASK-UPCOMING-FILTERS-A1; deadline-aware list ordering shipped in TASK-DUE-SORT-A1):** Dashboard overdue/due-today counts; deadline-aware hero. **Still deferred:** unbounded Upcoming; Due soon; This week filters; no-due-date filter; custom date ranges; priority sorting; user-selectable sort modes; reminders; exam dates; calendar integration; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates are **not** a complete deadline-planning system.

**Implementation files:** `supabase/migrations/015_study_tasks_due_date.sql`, `backend/src/shared/validation/calendar-date.js`, `backend/src/shared/validation/task.schema.js`, `backend/src/modules/tasks/tasks.service.js`, `frontend/src/utils/task-due-date.js`, `frontend/src/utils/validation.js`, `CourseTasksSection.jsx`, `GlobalTasksSection.jsx`, `TaskCard.jsx`, `MaterialRelatedTasksSection.jsx`, related tests.

---

## Implemented — Dashboard deadline-aware aggregates (Phase DASHBOARD-DEPTH-A2) — **Complete**

**Backend + frontend** — deadline-aware task aggregates on existing **`GET /api/dashboard/stats`** and **`/dashboard`**. Reads **`study_tasks.due_date`** (migration **015** unchanged — **no** new schema, indexes, or migration in A2). **Bounded next-7-days aggregate** shipped in **TASK-UPCOMING-FILTERS-A1** — see that section. **Not** unbounded upcoming-deadline windows, stored user timezone, calendar integration, notifications, exam dates, or AI scheduling.

| Layer | What exists |
|-------|-------------|
| **Query** | Optional **`referenceDate=YYYY-MM-DD`** — omitted → server **UTC** calendar date; valid date → used as reference; invalid/malformed/empty/timestamp/impossible/repeated-array/unknown keys → **400** **`VALIDATION_ERROR`** |
| **Frontend date alignment** | **`dashboard.service.js`** sends browser-local calendar date on **every** stats request |
| **API fields** | **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`dueNext7DaysPendingTasks`** (**TASK-UPCOMING-FILTERS-A1**), **`deadlineReferenceDate`** (aggregate counts + echoed reference date only) |
| **Overdue semantics** | Caller-owned **pending** tasks with non-null **`due_date` < `deadlineReferenceDate`** |
| **Due-today semantics** | Caller-owned **pending** tasks with **`due_date` = `deadlineReferenceDate`** |
| **Due next 7 days semantics** | Caller-owned **pending** tasks with **`due_date > deadlineReferenceDate`** and **`due_date <= deadlineReferenceDate + 7` calendar days** — same boundaries as **`deadline=next_7_days`** list filter (**TASK-UPCOMING-FILTERS-A1**) |
| **Excluded** | Completed tasks; null **`due_date`**; overdue/today dates (for next-7-days count); dates beyond day 7; other users' tasks |
| **Recommendation priority** | **no-courses → overdue-tasks → due-today-tasks → due-next-7-days-tasks → pending-tasks → due-flashcards →** remaining existing branches (**plan-gap**, **flashcards**, **add-tasks-or-plans**, **empty-workspace**, **caught-up**) — **`due-next-7-days-tasks`** added in **TASK-UPCOMING-FILTERS-A1** |
| **Deadline heroes** | **`overdue-tasks`** / **`due-today-tasks`** / **`due-next-7-days-tasks`** kinds; primary CTA **`View pending tasks`** → **`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, or **`/tasks?status=pending&deadline=next_7_days`** (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** superseded bare **`/tasks?status=pending`** for deadline kinds); **no** secondary course-scoped CTA |
| **At a glance** | **Tasks** band adds **Overdue**, **Due today**, and **Due next 7 days**; positive counts link to backend-filtered deadline task lists (**`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, **`/tasks?status=pending&deadline=next_7_days`** — **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**) |
| **Data minimization** | Aggregate counts only — **no** task titles, descriptions, or raw rows |

**Verification:** backend lint passed; backend tests **541/541**; frontend lint passed (pre-existing warnings only); frontend tests **494/494**; frontend build passed. **Reviews:** Supervisor Review **PASS**; Security Review **PASS**; manual smoke test **PASS**.

**Not in DASHBOARD-DEPTH-A2 (deferred at A2 ship — overdue/due-today task-list filters/URL params superseded by TASK-DUE-FILTERS-A1; bounded next-7-days filter/aggregate superseded by TASK-UPCOMING-FILTERS-A1; deadline-aware list ordering superseded by TASK-DUE-SORT-A1):** Unbounded Upcoming; Due soon; This week filters; stored user timezone persistence; calendar context; notifications; exam-date model; Gemini/Trello due-date assignment; AI scheduling; charts; streaks; goals.

**Implementation files:** `backend/src/modules/dashboard/dashboard.controller.js`, `backend/src/modules/dashboard/dashboard.service.js`, `backend/src/shared/validation/dashboard.schema.js`, `backend/src/shared/validation/calendar-date.js`, `frontend/src/services/dashboard.service.js`, `frontend/src/pages/DashboardStub.jsx`, `frontend/src/utils/dashboard-recommendation.js`, related dashboard tests.

---

## Implemented — Task deadline filters (Phase TASK-DUE-FILTERS-A1) — **Complete**

**Backend + frontend** — backend-filtered **Overdue** and **Due today** task lists on existing task list APIs and matching UI filters. Reads **`study_tasks.due_date`** (migration **015** unchanged — **no** new schema, indexes, or migration). At A1 ship, list order remained **`created_at DESC`** — due-date **sorting** deferred to **TASK-DUE-SORT-A1** (now shipped). **Not** upcoming/this-week windows, no-due-date filter, custom date ranges, stored user timezone, calendar/reminders/notifications, plan-import due dates, Trello due-date sync, backend material filtering, or collaboration.

**User-visible flow (membership — A1):** Dashboard **Overdue** / **Due today** (recommendation heroes or **At a glance**) → deadline-specific **`/tasks`** URL → backend-filtered matching task set. **Prioritization within that set** shipped in **TASK-DUE-SORT-A1** (see below).

| Layer | What exists |
|-------|-------------|
| **API routes** | **`GET /api/tasks`**, **`GET /api/courses/:courseId/tasks`** |
| **Query params** | Optional **`deadline=overdue|due_today|next_7_days`** (**`next_7_days`** in **TASK-UPCOMING-FILTERS-A1**); optional **`referenceDate=YYYY-MM-DD`** |
| **`deadline` + `status`** | **`deadline`** with omitted **`status`** behaves as **pending**; **`deadline`** with **`status=completed`** → **400** validation error |
| **`referenceDate`** | Without **`deadline`** → **400** validation error; omitted on API → server **UTC** calendar date; frontend sends **browser-local** calendar date on each deadline-filtered fetch |
| **Overdue semantics** | **`status = pending`**, **`due_date IS NOT NULL`**, **`due_date < referenceDate`** |
| **Due today semantics** | **`status = pending`**, **`due_date = referenceDate`** |
| **Global `/tasks`** | URL-persisted **`deadline=overdue|due_today|next_7_days`** (implies **`status=pending`**); invalid frontend values canonicalized away; **`referenceDate` not stored in browser URL**; composes with **`courseId`**, **`materialId`**, **`status`** (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1**); material filter remains **client-side** |
| **Course `/courses/:id`** | **Overdue** / **Due today** / **Next 7 days** deadline filters — at **A1 ship** in-memory only; **URL-persisted on course page later shipped in TASK-COURSE-FILTERS-E1** |
| **Dashboard links** | Recommendation **overdue-tasks** / **due-today-tasks** / **due-next-7-days-tasks** heroes and **At a glance** **Overdue** / **Due today** / **Due next 7 days** statistics link to **`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, and **`/tasks?status=pending&deadline=next_7_days`** (**due-next-7-days-tasks** in **TASK-UPCOMING-FILTERS-A1**) |
| **Empty states** | Deadline-specific copy when a deadline filter yields zero tasks |
| **Preserved at A1 ship** | Existing **All/Pending/Completed** status filters unchanged; material filtering **client-side**; list order **`created_at DESC`** at A1 ship (superseded for **Pending**/deadline lists by **TASK-DUE-SORT-A1**); generic **`/tasks?status=pending`** links unchanged |

**Verification:** backend lint passed; backend tests **570/570**; frontend lint passed; frontend tests **512/512**; frontend build passed. **Reviews:** Supervisor Review **PASS**; focused Supervisor Re-review **PASS**; Security Review **PASS**; manual browser smoke test **PASS**. **`git diff --check`** clean.

**Not in TASK-DUE-FILTERS-A1 (deferred at A1 ship — due-date sorting shipped in TASK-DUE-SORT-A1; bounded next-7-days filter shipped in TASK-UPCOMING-FILTERS-A1):** Unbounded Upcoming; Due soon; This week filters; no-due-date filter; custom date ranges; stored user timezone persistence; calendar integration; reminders/notifications; plan-import due dates; Trello due-date sync; backend **`GET …?materialId=`**; collaboration/chat. Task deadline filtering is **not** all remaining task-roadmap work.

**Implementation files:** `backend/src/modules/tasks/tasks-deadline-query.js`, `backend/src/modules/tasks/tasks.service.js`, `backend/src/shared/validation/task.schema.js`, `frontend/src/services/tasks.service.js`, `frontend/src/utils/task-nav-query.js`, `frontend/src/utils/dashboard-recommendation.js`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/src/pages/DashboardStub.jsx`, related backend/frontend tests.

---

## Implemented — Task list deadline-aware ordering (Phase TASK-DUE-SORT-A1) — **Complete**

**Backend only** — deterministic deadline-aware ordering on existing task list APIs. Reads **`study_tasks.due_date`** (migration **015** unchanged — **no** new schema, indexes, or migration). **Not** upcoming/this-week filters, no-due-date filter, custom date ranges, priority sorting, user-selectable sort modes, drag-and-drop ranking, stored user timezone, calendar/reminders/notifications, plan-import due dates, Trello due-date sync, material preview due-date ordering, backend material filtering, collaboration/chat, or a public sort query parameter.

**Closed loop (with TASK-DUE-FILTERS-A1):** Dashboard deadline count → deadline-specific task list → backend-filtered task set (**TASK-DUE-FILTERS-A1**) → deterministic deadline-aware order (**TASK-DUE-SORT-A1**) → most urgent task first.

| Layer | What exists |
|-------|-------------|
| **API routes (parity)** | **`GET /api/tasks`**, **`GET /api/courses/:courseId/tasks`** — same shared ordering contract |
| **Pending / deadline mode** | When **`status=pending`**, or **`deadline=overdue`**, or **`deadline=due_today`**, or **`deadline=next_7_days`**: **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`** |
| **Completed / All mode** | When **`status=completed`**, or both **`status`** and **`deadline`** omitted (**All**): **`created_at DESC` → `id ASC`** |
| **Semantics** | Oldest overdue date first; then later overdue; then **Due today**; then future dates ascending; then tasks without **`due_date`**; equal **`due_date`**: newest-created first; equal **`due_date`** and **`created_at`**: **`id ASC`** |
| **Architecture** | Ordering is **backend-owned**; frontend renders API order; **no** client-side **`.sort()`**; **no** sort query param; **no** sort selector UI; mutations already refetch task lists |
| **Preserved** | Auth and ownership unchanged; task response shape unchanged; deadline predicates unchanged (**TASK-DUE-FILTERS-A1**); material filtering remains **client-side** and preserves relative API order; **priority** is **not** part of sorting; **no** frontend implementation files changed |

**User-visible behavior:**

- **Pending** lists show dated tasks chronologically (earliest **`due_date`** first).
- **Overdue** lists show the oldest missed deadline first.
- **Due today** lists use deterministic **`created_at DESC`** / **`id ASC`** tie-breaking.
- Tasks without due dates appear after dated **Pending** tasks.
- **Completed** and **All** preserve recency ordering (**`created_at DESC`**).
- Editing a due date can reposition a task after refetch.
- Creating a no-date **Pending** task places it after dated **Pending** tasks.

**Verification:** backend lint passed; backend tests **589/589**; frontend lint passed; frontend tests **512/512**; frontend build passed. **Reviews:** Supervisor Review **PASS**; Security Review **APPROVED**; manual browser smoke test **PASS**. **`git diff --check`** clean.

**Not in TASK-DUE-SORT-A1 (deferred — separate phase gates; bounded next-7-days filter shipped in TASK-UPCOMING-FILTERS-A1):** Unbounded Upcoming; Due soon; This week filters; no-due-date filter; custom date ranges; **priority sorting**; user-selectable sort modes; drag-and-drop / manual ranking; stored user timezone; calendar / reminders / notifications; plan-import due dates; Trello due-date sync; material preview due-date ordering; database performance index until measured evidence exists; collaboration / chat; mobile / native scope. Task list ordering work is **not** all remaining task-roadmap work.

**Implementation files:** `backend/src/modules/tasks/tasks-list-order.js` (shared ordering helper), `backend/src/modules/tasks/tasks.service.js` (global/course list integration), `backend/tests/helpers/mockSupabaseTasks.js` (multi-column mock Supabase ordering), `backend/tests/unit/tasks-list-order.test.js`, `backend/tests/integration/tasks.test.js` (exact-order assertions), `backend/package.json` (register new unit test file only).

---

## Implemented — Task next-7-days deadline filter (Phase TASK-UPCOMING-FILTERS-A1) — **Complete**

**Backend + frontend** — bounded **Next 7 days** future-deadline discovery on existing task list APIs, matching UI filters, and dashboard aggregate/navigation. Reads **`study_tasks.due_date`** (migration **015** unchanged — **no** new schema, indexes, migration, or package dependency). Adds short-term future-deadline discovery **without** ambiguous **Upcoming**, **Due soon**, or **This week** filters.

**Closed product loop:** Immediate overdue/today work → **Due next 7 days** dashboard signal → exact filtered task list → backend membership window → existing deadline-aware sort (**TASK-DUE-SORT-A1**) → nearest future task first.

| Layer | What exists |
|-------|-------------|
| **API routes (parity)** | **`GET /api/tasks`**, **`GET /api/courses/:courseId/tasks`** — same shared filtering semantics |
| **New enum value** | **`deadline=next_7_days`** |
| **API examples** | **`GET /api/tasks?status=pending&deadline=next_7_days&referenceDate=YYYY-MM-DD`**; **`GET /api/courses/:courseId/tasks?status=pending&deadline=next_7_days&referenceDate=YYYY-MM-DD`** |
| **Membership contract** | For resolved **`referenceDate = R`**: **`status = pending`**, **`due_date > R`**, **`due_date <= R + 7` calendar days** — e.g. **`R = 2026-06-19`** includes **2026-06-20** through **2026-06-26**; excludes **R** (today), overdue dates, **2026-06-27** and later, null **`due_date`**, completed tasks |
| **`deadline` + `status`** | Omitted **`status`** implies **pending**; **`deadline`** with **`status=completed`** → **400** validation error |
| **`referenceDate`** | Without **`deadline`** → **400** validation error; frontend sends **browser-local** calendar date on each deadline-filtered fetch; direct API calls without **`referenceDate`** use server **UTC** fallback; **`referenceDate` is not persisted in the browser URL** |
| **Date arithmetic** | Upper boundary calculated in the backend; seven calendar days added without timezone/DST-dependent millisecond arithmetic; month/year/leap-year boundaries supported; **no** stored user timezone introduced |
| **Global `/tasks`** | URL-persisted **`deadline=next_7_days`** (implies **`status=pending`**); composes with **`courseId`**, **`materialId`**, **`status`**; browser URL **`/tasks?status=pending&deadline=next_7_days`** |
| **Course `/courses/:id`** | **Next 7 days** deadline filter — at **A1 ship** in-memory only; **URL-persisted on course page later shipped in TASK-COURSE-FILTERS-E1** |
| **Ordering** | Unchanged **TASK-DUE-SORT-A1** — **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`**; nearest upcoming task first within the window |
| **Dashboard aggregate** | **`dueNext7DaysPendingTasks`** on **`GET /api/dashboard/stats`** — same exact date boundaries as the task-list filter |
| **Dashboard recommendation** | One state: **`due-next-7-days-tasks`**; priority after **Overdue** and **Due today**, before generic **Pending** |
| **At a glance** | One item: **Due next 7 days**; positive count links to **`/tasks?status=pending&deadline=next_7_days`** — **not** a new dashboard section, chart, or layout |
| **Preserved** | **Overdue** and **Due today** semantics unchanged; **Pending** sorting remains deadline-aware; **Completed** and **All** remain recency ordered; material filtering remains **client-side** and preserves backend order; **no** frontend date-window filtering; auth, ownership, and response minimization unchanged |

**User-visible behavior:**

- **Next 7 days** includes tomorrow through day 7; today is excluded; day 8 is excluded.
- Null-due-date and completed tasks are excluded.
- Editing a due date into or out of the window updates membership after refetch.
- Creating a no-date task does **not** add it to this view.

**Verification:** backend lint passed; backend tests **615/615**; frontend lint passed; frontend tests **521/521**; frontend build passed. **Reviews:** Supervisor Review **PASS**; Security Review **APPROVED**; manual browser smoke test **PASS**. **`git diff --check`**: clean.

**Not in TASK-UPCOMING-FILTERS-A1 (deferred — separate phase gates):** Unbounded **Upcoming**; **Due soon**; **This week**; custom date ranges; date picker; no-due-date filter; stored user timezone; priority sorting; user-selectable sort modes; calendar/reminders/notifications; plan-import due dates; Trello due-date sync; backend material filtering; course URL persistence; migrations/indexes until measured evidence; collaboration/chat; mobile/native scope. Task deadline filtering is **not** all remaining task-roadmap work.

**Implementation files:** `backend/src/modules/tasks/tasks-deadline-query.js`, `backend/src/modules/tasks/tasks.service.js`, `backend/src/shared/validation/task.schema.js`, `backend/src/shared/validation/calendar-date.js`, `backend/src/modules/dashboard/dashboard.service.js`, `frontend/src/services/tasks.service.js`, `frontend/src/services/dashboard.service.js`, `frontend/src/utils/task-nav-query.js`, `frontend/src/utils/dashboard-recommendation.js`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/src/pages/DashboardStub.jsx`, related backend/frontend tests.
...
| **Recommendation rules** | Priority order at BX-I3 ship: no courses → pending tasks (optional most-pending course from **`courseStats`** counts) → due flashcards when **`dueFlashcardsCount > 0`** (**FLASHCARD-REVIEW-A5c**) → plan gap → flashcards → add tasks/plans → empty workspace → caught up. At BX-I3 ship, **`pending-tasks`** CTA was bare **`/tasks`** — **DASHBOARD-DEPTH-A1** superseded with **`/tasks?status=pending`** and course-scoped pending link; **DASHBOARD-DEPTH-A2** superseded priority with **overdue-tasks → due-today-tasks → pending-tasks → due-flashcards → …** (see **DASHBOARD-DEPTH-A2** section). |
| **Study pulse** | Task progress bars from existing stats — global and per-course pending/completed counts; subtitle: *“Pending and completed tasks from your stats”*. **DASHBOARD-DEPTH-A1** added **Last 7 days** focus snapshot band (not in original BX-I3 scope). |
...
| `/dashboard` | Yes | **Student dashboard cockpit** — real user-owned stats from **`GET /api/dashboard/stats`** (**5B** + **5C** + **5C.1** + **DASHBOARD-DEPTH-A1** + **DASHBOARD-DEPTH-A2** + **TASK-UPCOMING-FILTERS-A1** + **TASK-MATERIAL-DASHBOARD-A1** **`materialsWithPendingTasks`** / **`topMaterialsByPendingTasks[]`** — optional **`referenceDate`**, browser-local date on fetch, deadline aggregates, **Materials with pending tasks** section with material deep links, section order Hero → Study pulse → Materials with pending tasks → Course workload → At a glance; recommendation **overdue-tasks → due-today-tasks → due-next-7-days-tasks → pending-tasks → …** + **BX-I3** + **FLASHCARD-REVIEW-A5c** + **BX-I4** + **BX-I6B** + **BX-I7C**); read-only; material row **View linked tasks** → **`/tasks?courseId=&materialId=`**; **View tasks on course** → **`/courses/:courseId?materialId=`**; deadline heroes link to backend-filtered task lists; generic **`pending-tasks`** hero **`/tasks?status=pending`** / **`/tasks?courseId=<id>&status=pending`**; fetch on mount + **Try again** + **Refresh stats**; silent refresh after stat-changing actions |
...
| `/courses/:id` | Yes | **Course workspace** — materials list/create, **manual study tasks** (status + **study material** filters including **Tasks without material** — **TASK-MATERIAL-FILTERS-A2** + **TASK-MATERIAL-UNLINKED-A1**; backend **material membership** on list fetch — **TASK-MATERIAL-BACKEND-A1**; **URL-persisted filters** via query params **`materialId`** (UUID or **`none`**), **`status`**, **`deadline=overdue|due_today|next_7_days`** — **TASK-COURSE-FILTERS-E1** + **FIX1**; course ID in path only; defaults omitted; Back/Forward; invalid/stale canonicalization; deadline implies **Pending**; **Completed** clears deadline; mutation refetch preserves active filters; **`focusReturnTo`** preserves course-page query; backend-filtered deadline filters — **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**; backend list order **TASK-DUE-SORT-A1** — **Pending**/deadline: **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`**; **Completed**/All: **`created_at DESC` → `id ASC`**; create, edit pending, material link/unlink, optional **due date**, complete, delete); presentation upgraded **8C-2A** + **BX-I4** header accent + **BX-I6C** subject workspace (materials primary zone, document shelf deck, honest material count subtitle, tasks/danger framing) + **BX-I7D Tier 1** desktop **2-column** document/material shelf at **≥1280px** when populated (**`layout.css`** only); tasks remain below materials |
| `/tasks` | Yes | **All study tasks** — course + status + **study material** filters (material filter only when a specific course is selected — **TASK-MATERIAL-FILTERS-A2**; includes **Tasks without material** and UUID material options — **TASK-MATERIAL-UNLINKED-A1**); **URL-persisted filters** via query params **`courseId`**, **`materialId`** (UUID or **`none`**), **`status`**, **`deadline=overdue|due_today|next_7_days`** (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1** + **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** + **TASK-MATERIAL-BACKEND-A1** + **TASK-MATERIAL-DASHBOARD-A1-FIX2** — URL-aware synchronous initial filters; provisional material UUID retention until conclusive course material hydration; latest-request-wins list loading; **`deadline`** implies **`status=pending`**; backend-filtered; **`materialId`** sent on list fetch; **`referenceDate`** sent on fetch but **not** in URL; **`materialId=none`** requires **`courseId`**; Back/Forward + refresh preserve deep links); entry from dashboard material **View linked tasks**, dashboard deadline links, and material detail **View linked tasks** deep link (**TASK-MATERIAL-NAV-B1** — **`/tasks?courseId=&materialId=`**); create, edit pending, optional **due date** (**DASHBOARD-DEPTH-P0**), complete, delete, **Start Focus**; **`TaskCard`** due-date display + material **`Link`** to **`/study-materials/:materialId`** when `task.materialId` set (**TASK-MATERIAL-LINKS-A1**); **`focusReturnTo`** preserves current query string; backend list order **TASK-DUE-SORT-A1**; presentation **8C-3A** + **B4-A** + **BX-I7E1** |
| `/flashcards` | Yes | **All saved flashcards** — course/material/**review state** filters (client-side **`reviewState`** — **FLASHCARD-REVIEW-A3** + **A4b**); **URL-persisted filters** via query params **`courseId`**, **`materialId`**, **`reviewState`** (**FLASHCARD-REVIEW-A5a** — Back/Forward; manual changes update URL; defaults omitted; **`reviewState`** not sent to backend); create, study, edit/delete, **Known/Unknown review on saved DB cards** (**FLASHCARD-REVIEW-A2**); presentation upgraded **8C-3B** + **B4-B** flashcards command surface / command band, filters/create/study/manage framing, scoped loading/error/empty/filter-empty/action-error wrappers, manage list readability (truncated questions only — answers not exposed in manage list), filter **`role="group"`** + **`aria-label="Filter saved flashcards"`**, **narrow responsive browser viewport ~375px** scoped CSS + **BX-I7E2** desktop panels at **≥1280px** (horizontal command-controls band, **study \| manage** **2-column** populated library, **2-column** manage list when populated, create/inline edit full-width — **`layout.css`** only) |
...
- **bulk create** flashcards; **AI/Gemini** flashcard generation on `/flashcards`; **plan import** on **`/flashcards`** (global page); **course-level** flashcard management; **full SRS** (SM-2/Anki); **`ease_factor`**; backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; review history table; dashboard charts; admin dashboard due cards; pagination/rate limiting; **URL-persisted** flashcard filters on **`/study-materials/:materialId`** (global **`/flashcards`** URL filters shipped in **FLASHCARD-REVIEW-A5a** — **`courseId`**, **`materialId`**, **`reviewState`**; **`reviewState`** client-side only; dashboard deep link to **`/flashcards?reviewState=due_now`**; in-memory course/material/review-state/**Due now** filters on material detail shipped in **3B-f** + **FLASHCARD-REVIEW-A3** + **A4b**); optional shared CRUD form extraction; link from `/courses` to `/flashcards` ( **`public.flashcards` table + RLS** in **3B-b**; **backend API** in **3B-c** + review in **FLASHCARD-REVIEW-A1** + SRS-lite scheduling in **FLASHCARD-REVIEW-A4a**; **Known/Unknown UI** in **FLASHCARD-REVIEW-A2**; **review-state filter** in **FLASHCARD-REVIEW-A3**; **Due now filter** in **FLASHCARD-REVIEW-A4b**; **URL-persisted filters** in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only; **material-detail** in **3B-d**–**3B-e** + **plan import dedupe** in **10B**; **global page** in **3B-f**–**3B-g**; **plan JSON study** in **3B-a**; **plan tasks** import in **3A-f** superseded by **10B**); edit **completed** tasks or mark incomplete (pending-only edit shipped in **3A-c.1**); global **`/tasks`** URL-persisted filters shipped in **TASK-MATERIAL-FILTERS-E1**; **course-page** URL-persisted filters shipped in **TASK-COURSE-FILTERS-E1** (+ **FIX1**)
...
- **Dashboard deadline intelligence (beyond DASHBOARD-DEPTH-A2 / A1 / P0 / TASK-DUE-FILTERS-A1 / TASK-UPCOMING-FILTERS-A1 / TASK-DUE-SORT-A1 / TASK-MATERIAL-DASHBOARD-A1)** — **shipped:** rolling **Last 7 days** focus snapshot (**A1**); optional per-task **`dueDate`** on create/edit + card display (**P0**); deadline-aware dashboard aggregates (**A2** + **TASK-UPCOMING-FILTERS-A1**); backend-filtered overdue/due-today/next-7-days task lists + global/course URL **`deadline`**; deadline-aware list ordering (**TASK-DUE-SORT-A1**); **material-linked pending task summary** on dashboard (**TASK-MATERIAL-DASHBOARD-A1** — **`materialsWithPendingTasks`**, **`topMaterialsByPendingTasks[]`**, top **5**, material deep links to **`/tasks`** and **`/courses/:courseId?materialId=`**). **Deferred:** unbounded **Upcoming**; **Due soon**; **This week** filters; no-due-date filter; custom date ranges; stored user timezone persistence; calendar context; reminders/notifications; exam dates; Gemini-generated due dates; Trello due-date sync; AI scheduling; per-material deadline analytics; dashboard charts. Optional task due dates + A2 aggregates + material pending summary are **not** a complete deadline-planning system — further dashboard/task deadline phases require explicit approval.
...
- Real-time dashboard (polling / WebSockets / cross-tab sync); **full SRS** / SM-2/Anki scheduling; **`ease_factor`**; review history table; backend **`GET /api/flashcards?due=now`**; backend mastery/due query params; dashboard charts; admin dashboard due cards; URL-persisted flashcard filters on **`/study-materials/:materialId`**; sorting by **`lastReviewedAt`** — **SRS-lite scheduling foundation shipped** in **FLASHCARD-REVIEW-A4a** (`next_review_at`, `review_interval_days`, server-side computation on **`POST /api/flashcards/:flashcardId/review`**, migration **014** applied; **not** full SRS); **aggregate review state + Known/Unknown UI + client-side review-state filter + Due now filter shipped** in **FLASHCARD-REVIEW-A1** + **FLASHCARD-REVIEW-A2** + **FLASHCARD-REVIEW-A3** + **FLASHCARD-REVIEW-A4b** (migration **013** for review state; saved DB cards only; plan JSON **unchanged**); **minimal dashboard due-card awareness shipped** in **FLASHCARD-REVIEW-A4c** (`dueFlashcardsCount` on **`GET /api/dashboard/stats`** + **Due now** stat — aggregate only); **`/flashcards`** URL-persisted filters + dashboard deep link to **Due now** shipped in **FLASHCARD-REVIEW-A5a** (`/flashcards?reviewState=due_now`; **`reviewState`** client-side only); **Trello stored-token sync (A5A/A5B) and A5C hardening are complete**; see Trello post-A5C deferred items above
