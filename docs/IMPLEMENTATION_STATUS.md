...
| **`materialId`** | UUID | Initialized only after course materials load and material belongs to that course; **client-side filter only** — **not** sent to **`GET /api/tasks`** |
...
**Not in TASK-MATERIAL-FILTERS-E1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted filters on **`/courses/:id`**; URL-persisted dashboard or flashcard filters; filter for tasks **without** a material link; backend/API/database/migration/package/CI changes.
...
| `/tasks` | Yes | **All study tasks** — course + status + **study material** filters (material filter only when a specific course is selected — **TASK-MATERIAL-FILTERS-A2**); **URL-persisted filters** via query params **`courseId`**, **`materialId`**, **`status`** (**TASK-MATERIAL-FILTERS-E1** — **`materialId`** client-side only; Back/Forward restores state); create, edit pending, complete, delete, **Start Focus**; **`TaskCard`** material **`Link`** to **`/study-materials/:materialId`** when `task.materialId` set (**TASK-MATERIAL-LINKS-A1**); **`focusReturnTo`** preserves current query string; presentation upgraded **8C-3A** + **B4-A** task command surface / command band, filter toolbar framing, semantic **`ul > li`** task list, native status filter **`aria-pressed`** (**B4-A-F1**), scoped empty/error/loading wrappers + **BX-I7E1** desktop panels at **≥1280px** (horizontal command-controls band, **2-column** populated task list, create FormCard full-width, inline edit intended full width via **`:has(.task-workspace__edit-card)`** — **`layout.css`** only) |
