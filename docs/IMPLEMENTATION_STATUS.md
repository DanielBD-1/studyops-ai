...
**Not in DASHBOARD-DEPTH-P0 (deferred at P0 ship — superseded for dashboard aggregates by DASHBOARD-DEPTH-A2; task-list overdue/due-today filters shipped in TASK-DUE-FILTERS-A1):** Dashboard overdue/due-today counts; deadline-aware hero. **Still deferred:** due-date **sorting**; upcoming/this-week filters; no-due-date filter; custom date ranges; reminders; exam dates; calendar integration; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates are **not** a complete deadline-planning system.
...
**Not in DASHBOARD-DEPTH-A2 (deferred at A2 ship — overdue/due-today task-list filters/URL params superseded by TASK-DUE-FILTERS-A1):** Upcoming deadline window; due-date **sorting**; stored user timezone persistence; calendar context; notifications; exam-date model; Gemini/Trello due-date assignment; AI scheduling; charts; streaks; goals.
...
**Backend + frontend** — backend-filtered **Overdue** and **Due today** task lists on existing task list APIs and matching UI filters. Reads **`study_tasks.due_date`** (migration **015** unchanged — **no** new schema, indexes, or migration). **Not** due-date sorting, upcoming/this-week windows, no-due-date filter, custom date ranges, stored user timezone, calendar/reminders/notifications, plan-import due dates, Trello due-date sync, backend material filtering, or collaboration.

**User-visible flow:** Dashboard **Overdue** / **Due today** (recommendation heroes or **At a glance**) → deadline-specific **`/tasks`** URL → backend-filtered matching task list.
...
| **Preserved** | Existing **All/Pending/Completed** status filters unchanged; material filtering **client-side**; list order **`created_at DESC`**; generic **`/tasks?status=pending`** links unchanged |
