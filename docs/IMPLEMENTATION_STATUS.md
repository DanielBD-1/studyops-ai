...
**Still deferred (Trello):** **A4** frontend OAuth callback (state/nonce, fragment handling, Connect UI); **A5** using stored credentials in sync/boards/lists; board/list **persistence**; Trello card update/delete; force re-sync; advanced sync management beyond manual MVP. **A2 foundation complete** — encrypted `trello_connections` in repo. **A3 backend routes complete** — connect/authorize HTTP APIs reviewed; **not live for users** until **A4**.
...
| | `GET /api/trello/authorize-url` — Trello authorize URL (`response_type=token`, `callback_method=fragment`, `scope=read,write`, `expiration=never`, `name=StudyOps`; `return_url` = `FRONTEND_URL/trello/connect/callback`) |
| | `POST /api/trello/connect/complete` — body `{ token }` strict; validates via Trello `GET /members/me`; encrypts and upserts `trello_connections` |
...
- OAuth **state/nonce** validation (belongs to **A4 Security Review**)
