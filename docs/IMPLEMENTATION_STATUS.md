...
**Frontend:** **`GlobalTasksSection`** and **`CourseTasksSection`** map All / UUID / **Tasks without material** → omitted / UUID / **`none`** on list fetch; render API results directly (**no** client-side membership filter). **`MaterialRelatedTasksSection`** requests **`listCourseTasks(courseId, { materialId })`**. Removed **`filterTasksByMaterial`** and **`getMaterialLinkedTasks`**; **`summarizeLinkedTaskCounts`** / **`selectLinkedTaskPreview`** remain presentation-only. Global **`/tasks`** URL persistence unchanged (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1**); **`/courses/:id`** material filter remains in-memory only (**no** course URL params). Mutations refetch with active material filter.

**Not in TASK-MATERIAL-BACKEND-A1:** URL-persisted task filters on **`/courses/:id`**; dashboard material counts; task detail page; migrations/indexes; new sort modes; Trello/flashcards/admin/generate changes.
...
**Frontend-only** — URL-persisted course/material/status filters on global **`/tasks`** only. Bidirectional sync between filter state and browser URL via React Router **`useSearchParams`**. **No** backend, API, database, migration, document-service, or package changes. **`/courses/:id`** task filters remain in-memory only (**no** URL query params on course page).
...
**Not in TASK-MATERIAL-FILTERS-E1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted filters on **`/courses/:id`**; URL-persisted dashboard filters; filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1**); backend/API/database/migration/package/CI changes. (**`/flashcards`** URL-persisted filters shipped separately in **FLASHCARD-REVIEW-A5a**.)
...
| Surface | Behavior |
|---------|----------|
| **`/courses/:id`** | **Study material** `<select>` adds **Tasks without material** (`value="none"`) after **All materials in course**; filters in memory via **`filterTasksByMaterial(tasks, 'none', materials)`**; composes with status filters; **no** URL query params |
...
| **Course `/courses/:id`** | **Overdue** / **Due today** / **Next 7 days** deadline filters **in-memory only** — **not** persisted in course URL |
...
| **Course `/courses/:id`** | **Next 7 days** deadline filter **in-memory only** — **not** course-URL-persisted |
...
- **Dashboard deadline intelligence (beyond DASHBOARD-DEPTH-A2 / A1 / P0 / TASK-DUE-FILTERS-A1 / TASK-UPCOMING-FILTERS-A1 / TASK-DUE-SORT-A1)** — **shipped:** rolling **Last 7 days** focus snapshot (**A1**); optional per-task **`dueDate`** on create/edit + card display (**P0**); deadline-aware dashboard aggregates (**A2** + **TASK-UPCOMING-FILTERS-A1** — **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`dueNext7DaysPendingTasks`**, **`deadlineReferenceDate`**; optional validated **`referenceDate`**; browser-local frontend date alignment; UTC fallback when omitted; recommendation priority including **overdue-tasks** / **due-today-tasks** / **due-next-7-days-tasks**; **At a glance** **Overdue** / **Due today** / **Due next 7 days**); backend-filtered overdue/due-today/next-7-days task lists + global URL **`deadline`** + in-memory course deadline filters (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** — **`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, **`/tasks?status=pending&deadline=next_7_days`**; **`referenceDate`** not in browser URL); deadline-aware list ordering (**TASK-DUE-SORT-A1**). **Deferred:** unbounded **Upcoming**; **Due soon**; **This week** filters; no-due-date filter; custom date ranges; stored user timezone persistence; calendar context; reminders/notifications; exam dates; Gemini-generated due dates; Trello due-date sync; AI scheduling; **URL-persisted** task filters on **`/courses/:id`**. Optional task due dates + A2 aggregates + A1/A1-upcoming deadline filters are **not** a complete deadline-planning system — further dashboard/task deadline phases require explicit approval.
