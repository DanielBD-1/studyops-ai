...
**Not in 3A-c.3:** filtering tasks by `materialId`, generated-plan → `study_tasks` import, global `/tasks` (added in **3A-d**), mark incomplete, URL-persisted filters, flashcards, Trello, dashboard/admin. Material **navigation** links from **`TaskCard`** added in **TASK-MATERIAL-LINKS-A1** (below).
...
**UI:** **`TasksPage`** + **`GlobalTasksSection`** — `listCourses()` for course filter dropdown; `listAllTasks({ courseId?, status? })` with allowlisted query params only. **Course filter:** All courses + one option per owned course (in-memory). **Status filter:** All / Pending / Completed (in-memory). Filter changes cancel edit and refetch. **`TaskCard`** shows **Course:** with **link to `/courses/:id`**; when `task.materialId` is set, **`Material:`** **`Link`** to **`/study-materials/:materialId`** (**TASK-MATERIAL-LINKS-A1** — `materialLabel` or fallback **View study material**). **Pending:** edit, complete, delete. **Completed:** no edit; delete allowed. **Edit:** reuses **`updateTaskFormSchema`**; **`listMaterials(task.courseId)`** lazy-loaded on edit open only (not on page load). List view may show fallback **View study material** until `materialLabel` resolves (materials not preloaded on list). **No create** on `/tasks` at **3A-d** ship — create added in **3A-e** (below). Empty state: manual-task copy + navigate to `/courses`.

**Nav:** Links to `/tasks` from Dashboard and Courses list headers.

**Not in 3A-d:** create task on `/tasks`, `GET /api/tasks/:id`, Start Focus / focus sessions, generated-plan → `study_tasks` import, mark incomplete, material filtering, URL-persisted filters, flashcards, Trello, dashboard/admin.
...
**Known limitations:** `/tasks` renders task section only after `listCourses()` succeeds; material link text on list cards often shows fallback **View study material** until edit loads materials (**TASK-MATERIAL-LINKS-A1**). **Create on `/tasks`** added in Phase **3A-e** (below).
...
**Not in 3A-e:** `GET /api/tasks/:id`, Start Focus / focus sessions, generated-plan → `study_tasks` import, mark incomplete, material filtering, URL-persisted filters, flashcards, Trello, dashboard/admin.
...
**Known limitations / minor notes:** Possible **double refetch** after create (explicit `loadTasks` overrides + filter `useEffect`); brief **loading flash** after create; create **`NOT_FOUND`** may display **“Course not found”** even for material mismatch (UX only, not security); material link text on list cards often shows fallback **View study material** until edit loads materials (**TASK-MATERIAL-LINKS-A1**); `/tasks` task section gated on successful `listCourses()`.
...
**Known limitations:** Global **`/tasks`** list cards often show fallback **View study material** because `materialLabel` is not resolved until **`listMaterials(task.courseId)`** runs on edit open (materials not preloaded for list view). **`/courses/:id`** usually resolves titles from loaded course `materials`.

**Not in TASK-MATERIAL-LINKS-A1:** filter tasks by `materialId`; URL-persisted task filters; material-scoped task bands on material detail; other **`TaskCard`** behavior changes (course link, Start Focus, CRUD actions unchanged).
...
- Real-time dashboard (polling / WebSockets / cross-tab sync); spaced repetition; advanced flashcard study (known/unknown, Anki) — **Trello stored-token sync (A5A/A5B) and A5C hardening are complete**; see Trello post-A5C deferred items above
