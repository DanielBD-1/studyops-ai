...
**Not in TASK-MATERIAL-LINKS-A1:** filter tasks by study material (shipped in **TASK-MATERIAL-FILTERS-A2**); URL-persisted task filters; material-scoped task bands on material detail; other **`TaskCard`** behavior changes (course link, Start Focus, CRUD actions unchanged).
...
**Not in TASK-MATERIAL-FILTERS-A2:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters; material detail task band; filter for tasks **without** a material link; Study Rooms / collaboration; PDF; SRS; dashboard charts.
