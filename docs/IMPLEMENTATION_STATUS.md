...
**`flashcards` (Phases 3B-a through 3B-g + FLASHCARD-REVIEW-A1 + FLASHCARD-REVIEW-A2 + FLASHCARD-REVIEW-A3 + FLASHCARD-REVIEW-A4a + FLASHCARD-REVIEW-A4b + FLASHCARD-REVIEW-A4c + FLASHCARD-REVIEW-A5a + FLASHCARD-REVIEW-A5b):**
...
**Backend + frontend** — minimal dashboard due-card awareness for **saved DB flashcards** on the existing dashboard stats endpoint and **`/dashboard`** UI. **Not** full SRS. **No** new endpoint. **No** hero / **`deriveDashboardRecommendation`** changes. Plan JSON flashcards **unchanged**.
...
| **A4b + A5a** | Client-side **Due now** filter on **`/flashcards`** / material detail; dashboard **Ready for review** opens **`/flashcards?reviewState=due_now`** (**A5a**) — hero / recommendation logic unchanged |
...
| **A5a preserved** | **`/flashcards`** URL-persisted filters and **`flashcard-nav-query.js`** **unchanged**; dashboard hero / **`deriveDashboardRecommendation`** **unchanged** |
...
**Not in FLASHCARD-REVIEW-A5b:** Backend/database/migration changes; URL changes; plan JSON review persistence; full SRS (SM-2/Anki); **`ease_factor`**; review history table; backend **`GET /api/flashcards?due=now`** or **`?mastery=`**; dashboard hero / recommendation logic changes; **`flashcards.service.js`** runtime behavior changes.
