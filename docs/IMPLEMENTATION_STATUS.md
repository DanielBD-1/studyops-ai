...
**`material_generated_plans` (Phases 2L-a + 11A-1 + 11A-2):** Multiple validated generated plan rows per `study_material_id` with **`is_active boolean`** — exactly **one active** row per material (partial unique index); inactive history retained up to **10** total rows per material (retention prunes oldest inactive after generate). `plan` jsonb (object, size-capped); RLS for `authenticated` (unchanged from **2L-a**); **`anon` has no access**; backend writes via **service role** + RPCs **`activate_material_generated_plan`** (generate) and **`reactivate_material_generated_plan`** (history activate — Phase **11A-2**) with ownership filters (see `docs/database/004-material-generated-plans-schema-and-rls.md`, **`docs/database/010-material-generated-plans-active-history.md`**, **`docs/database/011-reactivate-material-generated-plan.md`**; migrations **010** and **011** **applied manually** on Supabase). **No** failed-attempt rows, raw Gemini payloads, duplicated material `content`, or history **UI** (Phase **11A-3** deferred).
...
Delivered in phases **2D–2F** (generate orchestration + UI), **2L-a/b/c** (DB + backend persistence + frontend load/clear), **11A-1** (active history + retention), and **11A-2** (history REST API). Not the monolithic PRD flow with client paste on the course page.
...
- **One active plan per material** (Phase **11A-1**) — regenerate inserts a new active row; prior active becomes inactive; up to **10** rows per material retained (oldest inactive pruned). **History REST APIs** (Phase **11A-2**) — list/get-by-id/activate/delete-version; **no** history UI (Phase **11A-3** deferred).
...
**Not in 11A-2:** history **UI** (Phase **11A-3** deferred); document-service or Gemini prompt changes; background retention jobs.
...
- Saved generated **plan library** or plan **history UI** (DB retains up to **10** rows per material with one **active** since **11A-1**; history REST APIs shipped in **11A-2** — list/get-by-id/activate/delete-version; **11A-3** frontend history UI still deferred)
