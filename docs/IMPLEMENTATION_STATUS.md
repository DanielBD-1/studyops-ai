...
**`flashcards` (Phases 3B-a through 3B-g + FLASHCARD-REVIEW-A1 + FLASHCARD-REVIEW-A2 + FLASHCARD-REVIEW-A3 + FLASHCARD-REVIEW-A4a + FLASHCARD-REVIEW-A4b + FLASHCARD-REVIEW-A4c):**
...
**Not in TASK-MATERIAL-NAV-B1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted filters on **`/courses/:id`**; URL-persisted dashboard or flashcard filters; filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1**); **`status`** query param alone (shipped in **TASK-MATERIAL-FILTERS-E1**).
...
**Not in TASK-MATERIAL-FILTERS-E1:** backend **`GET /api/tasks?materialId=`** or **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted filters on **`/courses/:id`**; URL-persisted dashboard or flashcard filters; filter for tasks **without** a material link (shipped in **TASK-MATERIAL-UNLINKED-A1**); backend/API/database/migration/package/CI changes.
...
| **Learning assets** | `totalFlashcards`, **`dueFlashcardsCount`** (**Due now** — count shown including **0**; when **> 0**, **Ready for review** + link to **`/flashcards`** only — **FLASHCARD-REVIEW-A4c**) |
...
**Not in FLASHCARD-REVIEW-A4a:** Frontend Due now filter (shipped in **FLASHCARD-REVIEW-A4b**); dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; SM-2 / Anki; **`ease_factor`**; review history table; URL-persisted filters; sorting by **`lastReviewedAt`**; auto-advance after review; plan JSON review persistence; sub-day intervals.
...
**Not in FLASHCARD-REVIEW-A4b:** Dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; URL-persisted filters; full SRS (SM-2/Anki); **`ease_factor`**; review history table; notifications; calendar scheduling; auto-advance after review; plan JSON review persistence; backend/database/migration changes.
...
| **Dashboard UI** | **`DashboardStub.jsx`** **At a glance** → **Learning assets** **`StatItem`** **Due now** with **`dueFlashcardsCount`** ( **`?? 0`** fallback); when count **> 0**, detail shows **Ready for review** + **`Link`** to **`/flashcards`** (**Review flashcards**) — **no** **`?due=now`** or other query params |
...
| **A4b filter unchanged** | Client-side **Due now** filter on **`/flashcards`** / material detail remains separate — **no** deep link from dashboard to Due now filter |
...
- **Backend task filtering by `materialId`** — **`GET /api/tasks?materialId=`** and **`GET /api/courses/:id/tasks?materialId=`** (frontend-only material filter shipped in **TASK-MATERIAL-FILTERS-A2** + **TASK-MATERIAL-UNLINKED-A1** — filters already-loaded tasks in memory; **no** backend query params added); **URL-persisted** task filters on **`/courses/:id`** (**`/tasks`** URL filters shipped in **TASK-MATERIAL-FILTERS-E1** + **`materialId=none`** in **TASK-MATERIAL-UNLINKED-A1** — **`courseId`**, **`materialId`**, **`status`**; **`materialId`** client-side only); **bulk create** flashcards; **AI/Gemini** flashcard generation on `/flashcards`; **plan import** on **`/flashcards`** (global page); **course-level** flashcard management; **full SRS** (SM-2/Anki); **`ease_factor`**; backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; review history table; dashboard charts; admin dashboard due cards; pagination/rate limiting; **URL-persisted** flashcard filters (in-memory course/material/review-state/**Due now** filters shipped in **3B-f** + **FLASHCARD-REVIEW-A3** + **FLASHCARD-REVIEW-A4b** — client-side only; minimal **`dueFlashcardsCount`** on dashboard shipped in **FLASHCARD-REVIEW-A4c**); optional shared CRUD form extraction; link from `/courses` to `/flashcards` ( **`public.flashcards` table + RLS** in **3B-b**; **backend API** in **3B-c** + review in **FLASHCARD-REVIEW-A1** + SRS-lite scheduling in **FLASHCARD-REVIEW-A4a**; **Known/Unknown UI** in **FLASHCARD-REVIEW-A2**; **review-state filter** in **FLASHCARD-REVIEW-A3**; **Due now filter** in **FLASHCARD-REVIEW-A4b**; **material-detail** in **3B-d**–**3B-e** + **plan import dedupe** in **10B**; **global page** in **3B-f**–**3B-g**; **plan JSON study** in **3B-a**; **plan tasks** import in **3A-f** superseded by **10B**); edit **completed** tasks or mark incomplete (pending-only edit shipped in **3A-c.1**); in-memory status/course/material task filters on **`/courses/:id`** (**3A-c.2** / **TASK-MATERIAL-FILTERS-A2**) are **not** URL-persisted; global **`/tasks`** URL-persisted filters shipped in **TASK-MATERIAL-FILTERS-E1**
...
- Real-time dashboard (polling / WebSockets / cross-tab sync); **full SRS** / SM-2/Anki scheduling; **`ease_factor`**; review history table; backend **`GET /api/flashcards?due=now`**; backend mastery/due query params; dashboard charts; admin dashboard due cards; URL-persisted flashcard filters; sorting by **`lastReviewAt`** — **SRS-lite scheduling foundation shipped** in **FLASHCARD-REVIEW-A4a** (`next_review_at`, `review_interval_days`, server-side computation on **`POST /api/flashcards/:flashcardId/review`**, migration **014** applied; **not** full SRS); **aggregate review state + Known/Unknown UI + client-side review-state filter + Due now filter shipped** in **FLASHCARD-REVIEW-A1** + **FLASHCARD-REVIEW-A2** + **FLASHCARD-REVIEW-A3** + **FLASHCARD-REVIEW-A4b** (migration **013** for review state; saved DB cards only; plan JSON **unchanged**); **minimal dashboard due-card awareness shipped** in **FLASHCARD-REVIEW-A4c** (`dueFlashcardsCount` on **`GET /api/dashboard/stats`** + **Due now** stat on **`/dashboard`** — aggregate only; **no** deep link / URL filters); **Trello stored-token sync (A5A/A5B) and A5C hardening are complete**; see Trello post-A5C deferred items above
