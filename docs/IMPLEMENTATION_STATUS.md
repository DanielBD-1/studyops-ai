...
## UI maturity and desktop layout (BX-I7A audit)

**Functional core + presentation hardening:** Auth through admin aggregate stats, material-scoped AI generate + plan history, tasks, flashcards, Trello sync, focus, dashboard decision layout, dark graphite / glass tokens (**BX-I2**), and **B4**/**BX-I6** polish phases (**BX-I3**–**BX-I6D**, **B4-A**–**B4-F3C3**) are **shipped**. That work improved visual consistency, state surfaces, accessibility, and page-level polish — but the UI is **not** final **Stitch-level** presentation yet.

**BX-I7A finding (documentation audit — no code change):** On wide **desktop browser** viewports the app can feel **too narrow and list-like** because:

| Item | Current production |
|------|-------------------|
| Global content cap | **`--content-max-cockpit`** and **`--content-max-shell`** = **1120px** (`frontend/src/styles/tokens.css`) |
| Page shell | **`page--cockpit`** routes center content at that max width |
| App chrome | **`app-shell__inner`** aligns to the same shell cap |
| Route density | Many hub routes still use **one-column** list/form/command-band stacks inside the cap |
| **DESIGN.md** / Stitch direction | Wider **desktop SaaS** cockpit / **12-column-style** operational density — **not** fully matched at **1120px** |

**Proposed next implementation phase — BX-I7B (not started, not automatic):** Global desktop content width tokens / page shell widening — **`frontend/src/styles/tokens.css`** and **`frontend/src/styles/layout.css`** only. **Must not** change: JSX, behavior, backend/API/database, fake metrics, sidebar, or mobile/native patterns. Requires explicit **`approved — implement Phase BX-I7B`** after separate planning + Supervisor Review.

**Do not claim:** Stitch is fully implemented in production; UI is final; desktop width issue is solved; **B4**/**BX-I6** alone completed the full visual direction.
...
## Implemented — UI presentation polish (Phases 8A–8C, 12A-1, B1–B3, BX-I2, BX-I3, BX-I4, BX-I5, BX-I6B, BX-I6C, BX-I6D, B4-A, B4-B, B4-C, B4-D, B4-E, B4-F1, B4-F2, B4-F3A, B4-F3B, B4-F3C1, B4-F3C2, B4-F3C3)
...
- **BX-I7B** — global desktop content width tokens / page shell widening (**`tokens.css`** + **`layout.css`** only). **Proposed** after **BX-I7A** audit to address **1120px** **`--content-max-cockpit`** / **`--content-max-shell`** gap on wide desktop browsers. **No** JSX, behavior, backend/API/database, fake metrics, sidebar, or mobile/native patterns. **Not automatic** — requires explicit **`approved — implement Phase BX-I7B`**
...
- Further **UI styling** beyond **B4-F3C3** (desktop global content width **BX-I7B**, charts, sidebar, course accents on material detail) requires explicit human approval — **`DESIGN.md` v2.3** / Stitch direction for wider desktop cockpit density is **not fully built** (**BX-I7A**); **`DESIGN.md` v2.3** direction for charts/sidebar/material accents is **not built** until a gated **BX-I7B**, remaining **B4**, or **BX-I6** phase; **`11-generated-plan-visible.png`** **captured** (Phase 2K-c); **`15-processing-with-ai.png`** still **pending** (see `docs/design/SCREENSHOT_INDEX.md`; committed PNGs may predate **8C** / **12A-1** / **B1–B3** / **BX-I2** / **BX-I3** / **BX-I4** / **BX-I5** / **BX-I6B** / **BX-I6C** / **BX-I6D** / **B4-A** / **B4-B** / **B4-C** / **B4-D** / **B4-E** / **B4-F1** / **B4-F2** / **B4-F3A** / **B4-F3C1** / **B4-F3C2** — **`03-dashboard.png`** is a **pre-5C/pre-8C/pre-BX-I3/pre-BX-I4/pre-BX-I6B** reference)
