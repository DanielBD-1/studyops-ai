...
**`flashcards` (Phases 3B-a through 3B-g + FLASHCARD-REVIEW-A1 + FLASHCARD-REVIEW-A2 + FLASHCARD-REVIEW-A3 + FLASHCARD-REVIEW-A4a + FLASHCARD-REVIEW-A4b + FLASHCARD-REVIEW-A4c + FLASHCARD-REVIEW-A5a):**
...
**Not in FLASHCARD-REVIEW-A4a:** Frontend Due now filter (shipped in **FLASHCARD-REVIEW-A4b**); dashboard due-card awareness (shipped in **FLASHCARD-REVIEW-A4c**); backend **`GET /api/flashcards?due=now`**; backend **`GET /api/flashcards?mastery=`**; SM-2 / Anki; **`ease_factor`**; review history table; URL-persisted filters (shipped in **FLASHCARD-REVIEW-A5a** on **`/flashcards`** only); sorting by **`lastReviewedAt`**; auto-advance after review; plan JSON review persistence; sub-day intervals.
...
| **A2 / A3 preserved** | Known/Unknown unchanged — same **`POST /api/flashcards/:flashcardId/review`**; local merge by id; **no** refetch on review; **no** auto-advance; mastery filters (**All**, **New + Learning**, **New**, **Learning**, **Known**) unchanged |
