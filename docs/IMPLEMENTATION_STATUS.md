...
**Still deferred (Trello):** OAuth connect flow (authorize/callback/UI/routes); **using** stored credentials in sync/boards/lists; board/list **persistence**; Trello card update/delete; force re-sync; advanced sync management beyond manual MVP. **A2 foundation complete** — encrypted `trello_connections` in repo; manual flow unchanged.
...
- OAuth authorize URL, callback route, `GET /api/trello/connection`, connect/complete, disconnect
...
- Trello **OAuth** (authorize, callback, Connect UI, connect/disconnect HTTP routes); **using stored token** in sync/boards/lists; **board/list persistence**; Trello card **update/delete**; **force re-sync**; advanced sync management beyond manual MVP (**4A** sync UI + **4B** board/list picker end-to-end; manual listId paste no longer required). **A2 done:** encrypted `trello_connections` + crypto/repository — **manual apiKey/token still live** (ADR 004)
...
- Trello OAuth connect (UI + routes + sync/boards using stored token); real-time dashboard (polling / WebSockets / cross-tab sync); spaced repetition; advanced flashcard study (known/unknown, Anki)
