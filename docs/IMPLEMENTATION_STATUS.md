...
**Frontend-only** — rule-based dashboard hero / **`deriveDashboardRecommendation`** now due-flashcard-aware using existing **`dueFlashcardsCount`** from **`GET /api/dashboard/stats`**. **No** backend, API, database, migration, URL contract, or **`DashboardStub.jsx`** changes.
...
| **Presentation** | **`DashboardStub.jsx`** unchanged — **`DashboardDecisionHero`** renders recommendation object from **`deriveDashboardRecommendation(stats)`** |
...
| **Recommendation rules** | Priority order: no courses → pending tasks (optional most-pending course from **`courseStats`** counts) → due flashcards when **`dueFlashcardsCount > 0`** (**FLASHCARD-REVIEW-A5c**) → plan gap (**`totalStudyMaterials - totalGeneratedPlans`**) → flashcards (when none due) → add tasks/plans → empty workspace → caught up |
