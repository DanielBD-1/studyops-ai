...
**Repo-only (A2 — apply on Supabase when approved):** `public.trello_connections` — migration `012_trello_connections.sql`; encrypted Trello user token storage; **`service_role` only**; see `docs/database/012-trello-connections-schema-and-rls.md`. **Not used** by live sync/boards/lists routes yet.
...
**Still deferred (Trello):** **A5** using stored credentials in sync/boards/lists; board/list **persistence**; Trello card update/delete; force re-sync; advanced sync management beyond manual MVP. **A2–A4-STATE + A4 frontend complete** — encrypted storage, backend connect routes, signed state, and account Connect/Disconnect UI on **`/trello`** + callback route; **manual apiKey/token** remains the **only active sync path** until **A5**.
...
**Storage/security foundation only** — prepares for OAuth; **does not** change live Trello UX or HTTP behavior. Manual apiKey/token on **`/trello`** and ephemeral POST bodies (ADR 004) remain the **only** production path.
...
**Not in A2 (deferred — separate phases + Security Review):**

- Frontend Connect Trello UI or `/trello/connect/callback`
- Refactor `POST /api/trello/boards`, `/lists`, `/sync` to read stored token
- Remove manual apiKey/token fields from `/trello`
...
**Backend-only** — four authenticated routes on `/api/trello`; builds on **A2** encrypted storage. **OAuth is not live for users:** no frontend callback UI, no Connect Trello button, no frontend route for `/trello/connect/callback`. Manual apiKey/token on **`/trello`** and ephemeral POST bodies for boards/lists/sync remain the **only live user path** (ADR 004).
...
| Unchanged | **`POST /api/trello/boards`**, **`POST /api/trello/boards/:boardId/lists`**, **`POST /api/trello/sync`** — still manual `{ apiKey, token }` in body |
...
**Not in A3 (deferred — A4+):**

- Frontend callback route `/trello/connect/callback` and fragment token handling
- Connect Trello button / connect UX on **`/trello`**
- **A5:** Refactor boards/lists/sync to use stored token (if approved)
...
**Backend-only** — HMAC-signed OAuth state on connect flow; blocks account-linking CSRF where a logged-in user could be tricked into completing connect with an attacker’s Trello token. **OAuth is still not live for users:** no frontend callback UI, no Connect button, no `/trello/connect/callback` route. Manual apiKey/token on **`/trello`** remains the **only live user path** (ADR 004).
...
**Not in A4-STATE (deferred — A4 frontend + A5):**

- Frontend `/trello/connect/callback` route and hash-fragment token handling
- Connect Trello button / connect UX on **`/trello`**
- **A5:** Refactor boards/lists/sync to use stored token (if approved)

**See:** `docs/security/trello-oauth-foundation.md`, ADR [006](adrs/006-trello-oauth-encrypted-connections.md)

**User-facing connect:** shipped in **TRELLO-OAUTH-A4-FRONTEND** (see below). **A5A** adds backend stored-token mode on boards/lists/sync; **frontend still uses manual credentials** until **A5B**.

---

## Implemented — Trello OAuth backend stored-token mode (Phase TRELLO-OAUTH-A5A)

**Backend only** — additive, backward-compatible stored-token support on existing discovery/sync routes. Builds on **A2** encrypted storage, **A3** connect routes, and **A4-STATE** signed state. **Frontend A5B not implemented:** **`/trello`** UI still sends manual `{ apiKey, token }` — manual sync remains the **only active UI sync path** until **A5B**.
...
| **Manual mode** | Body has **both** `apiKey` and `token` keys → unchanged manual flow; works even when user is connected |
...
- Board/list persistence; Trello card update/delete; force re-sync
- Wrong-account manual sync hardening (**A5C**)
...
**Frontend only** — Trello **account connection** UX on **`/trello`** and protected OAuth callback. Builds on **A2** encrypted storage, **A3** backend connect routes, and **A4-STATE** signed state validation. **A5A** added backend stored-token mode on boards/lists/sync; **this phase did not change frontend sync UX** — **`/trello`** still sends manual `{ apiKey, token }` (**A5B** pending).

| Item | Detail |
|------|--------|
| **`/trello`** | `TrelloConnectionPanel` — connection status (`@trelloUsername` when connected); **Connect account** (`fetchTrelloAuthorizeUrl` → redirect); **Disconnect** (`disconnectTrello`); trust note that task sync below still uses manual API key/token |
...
| Unchanged sync UX | `TrelloSyncSection`, board/list picker, manual apiKey/token fields, `syncTasksToTrello` — same manual MVP flow |
...
**Not in A4-FRONTEND (deferred — A5B+):**

- Refactor frontend sync form to use **stored** token from `trello_connections` (**A5B**)
- Remove manual apiKey/token fields from sync section when connected (**A5B**)
- Board/list persistence; Trello card update/delete; force re-sync
...
| `/trello` | Yes | **Trello account connection** (**A4-FRONTEND**): Connect/Disconnect panel; protected **`/trello/connect/callback`**. **Trello sync UI (live path):** manual apiKey/token per session (**not stored**); **Load boards** → board/list picker (**4B-2**); task selection (max 50); sync via backend only; credentials cleared after sync attempt; **stored-token sync not implemented** (**A5** deferred). Presentation: **8C-3C** + **B4-C** + **BX-I7E3 Tier A** at **≥1280px** |
...
- Trello **A5:** **using stored token** in sync/boards/lists (if approved); remove manual apiKey/token from sync section; **board/list persistence**; Trello card **update/delete**; **force re-sync**; advanced sync management beyond manual MVP. **A2–A4-FRONTEND done:** encrypted storage, backend connect routes, signed state, Connect/Disconnect UI + callback — **account connection live**; **manual apiKey/token sync still the only active sync path** (ADR 004) until **A5**
...
- Trello **A5** (sync/boards/lists using **stored** token); real-time dashboard (polling / WebSockets / cross-tab sync); spaced repetition; advanced flashcard study (known/unknown, Anki)
