...
**User-facing connect:** shipped in **TRELLO-OAUTH-A4-FRONTEND** (see below). **A5A** adds backend stored-token mode on boards/lists/sync. **A5B** adds frontend connected-account sync UX (see below).
...
- **Deferred (A5C):** backend still accepts manually crafted credentials while connected
...
**Not in A4-FRONTEND (deferred — A5B+ / A5C):**
...
- Trello **A5C** and beyond: **board/list persistence**; Trello card **update/delete**; **force re-sync**; wrong-account manual sync hardening while connected (**A5C** deferred). **A2–A5B done:** encrypted storage, backend connect, signed state, Connect/Disconnect UI + callback, backend stored-token mode, **frontend connected-account sync UX**
