...
- Trello **A5C+** and beyond: **board/list persistence**; Trello card **update/delete**; **force re-sync**. **A2–A5C done:** encrypted storage, backend connect, signed state, Connect/Disconnect UI + callback, backend stored-token mode, frontend connected-account sync UX, **backend manual-credential hardening while connected**
