...
**`study_tasks` backend API (Phase 3A-b; due date in DASHBOARD-DEPTH-P0; deadline list filters in TASK-DUE-FILTERS-A1; list ordering in TASK-DUE-SORT-A1):** Express routes with **`requireAuth`**; service-role queries always filter by authenticated `user_id`. Task responses are **camelCase** and include **`dueDate`** (`YYYY-MM-DD` \| `null`) and do **not** include `userId`, study material `content`, or generated `plan` JSON. **`difficulty`** / **`tags`** are returned (defaults on create) but **not client-editable**. **`status`** is **not** PATCHable — use **`POST /api/tasks/:taskId/complete`** only (completion preserves **`dueDate`**). Create: optional **`dueDate`** (omit or `null` → `NULL`; valid date stored). PATCH: omit preserves; `null` clears; valid date replaces. List APIs (**`GET /api/tasks`**, **`GET /api/courses/:courseId/tasks`**) support optional **`deadline=overdue|due_today`** + optional **`referenceDate=YYYY-MM-DD`** (**TASK-DUE-FILTERS-A1** — see dedicated section) and backend-owned list ordering (**TASK-DUE-SORT-A1** — **Pending** / deadline lists: **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`**; **Completed** / **All**: **`created_at DESC` → `id ASC`**; **no** public sort query param). Wrong-owner or missing task → neutral **`404`** “Task not found”. **Frontend** task UI: course-level Phases **3A-c**–**3A-c.3**, global Phases **3A-d**–**3A-e**, optional due date forms + **`TaskCard`** display (**DASHBOARD-DEPTH-P0**), frontend material filters (**TASK-MATERIAL-FILTERS-A2**), URL-persisted global filters including **`deadline`** (**TASK-MATERIAL-FILTERS-E1** + **TASK-DUE-FILTERS-A1**), in-memory course deadline filters (**TASK-DUE-FILTERS-A1**), material detail related tasks band (**TASK-MATERIAL-DETAIL-A3**); plan import on material detail via Phase **10B** (below); list UI renders API order only (**TASK-DUE-SORT-A1**).
...
|--------|-------|---------|
| `GET` | `/api/courses/:id/tasks` | List tasks for an **owned** course — optional `?status=pending` \| `completed`; optional `?deadline=overdue` \| `due_today` + `?referenceDate=YYYY-MM-DD` (**TASK-DUE-FILTERS-A1**) |
| `POST` | `/api/courses/:id/tasks` | Create task — body: `title`, `estimatedMinutes`, optional `description`, `priority`, `materialId` |
| `GET` | `/api/tasks` | List caller’s tasks — optional `?courseId`, `?status=pending` \| `completed`; optional `?deadline=overdue` \| `due_today` + `?referenceDate=YYYY-MM-DD` (**TASK-DUE-FILTERS-A1**) |
...
| **`deadline`** | `overdue` \| `due_today` | **TASK-DUE-FILTERS-A1** — backend-filtered; implies **`status=pending`**; invalid values canonicalized away; **`referenceDate`** sent to API on fetch but **not** stored in browser URL |
...
**Not in DASHBOARD-DEPTH-P0 (deferred at P0 ship — superseded for dashboard aggregates by DASHBOARD-DEPTH-A2; task-list overdue/due-today filters shipped in TASK-DUE-FILTERS-A1; deadline-aware list ordering shipped in TASK-DUE-SORT-A1):** Dashboard overdue/due-today counts; deadline-aware hero. **Still deferred:** upcoming/this-week filters; no-due-date filter; custom date ranges; priority sorting; user-selectable sort modes; reminders; exam dates; calendar integration; Gemini-generated due dates; Trello due-date sync; AI scheduling. Optional task due dates are **not** a complete deadline-planning system.
...
**Backend + frontend** — deadline-aware task aggregates on existing **`GET /api/dashboard/stats`** and **`/dashboard`**. Reads **`study_tasks.due_date`** (migration **015** unchanged — **no** new schema, indexes, or migration in A2). **Not** upcoming-deadline windows, task-list overdue/due-today filters (**shipped in TASK-DUE-FILTERS-A1**), stored user timezone, calendar integration, notifications, exam dates, or AI scheduling.
...
| **API fields** | **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`deadlineReferenceDate`** (aggregate counts + echoed reference date only) |
...
| **Excluded** | Completed tasks; null **`due_date`**; future dates; other users' tasks |
| **Recommendation priority** | **no-courses → overdue-tasks → due-today-tasks → pending-tasks → due-flashcards →** remaining existing branches (**plan-gap**, **flashcards**, **add-tasks-or-plans**, **empty-workspace**, **caught-up**) |
| **Deadline heroes** | **`overdue-tasks`** / **`due-today-tasks`** kinds; primary CTA **`View pending tasks`** → **`/tasks?status=pending&deadline=overdue`** or **`/tasks?status=pending&deadline=due_today`** (**TASK-DUE-FILTERS-A1** superseded bare **`/tasks?status=pending`** for deadline kinds); **no** secondary course-scoped CTA |
| **At a glance** | **Tasks** band adds **Overdue** and **Due today**; positive counts link to backend-filtered deadline task lists (**`/tasks?status=pending&deadline=overdue`** and **`/tasks?status=pending&deadline=due_today`** — **TASK-DUE-FILTERS-A1**) |
...
**Not in DASHBOARD-DEPTH-A2 (deferred at A2 ship — overdue/due-today task-list filters/URL params superseded by TASK-DUE-FILTERS-A1; deadline-aware list ordering superseded by TASK-DUE-SORT-A1):** Upcoming deadline window; stored user timezone persistence; calendar context; notifications; exam-date model; Gemini/Trello due-date assignment; AI scheduling; charts; streaks; goals.
...
| **Query params** | Optional **`deadline=overdue|due_today`**; optional **`referenceDate=YYYY-MM-DD`** |
...
| **Global `/tasks`** | URL-persisted **`deadline=overdue|due_today`** (implies **`status=pending`**); invalid frontend values canonicalized away; **`referenceDate` not stored in browser URL**; composes with **`courseId`**, **`materialId`**, **`status`** (**TASK-MATERIAL-FILTERS-E1** + **TASK-MATERIAL-UNLINKED-A1**); material filter remains **client-side** |
| **Course `/courses/:id`** | **Overdue** / **Due today** deadline filters **in-memory only** — **not** persisted in course URL |
| **Dashboard links** | Recommendation **overdue-tasks** / **due-today-tasks** heroes and **At a glance** **Overdue** / **Due today** statistics link to **`/tasks?status=pending&deadline=overdue`** and **`/tasks?status=pending&deadline=due_today`** |
...
| **Pending / deadline mode** | When **`status=pending`**, or **`deadline=overdue`**, or **`deadline=due_today`**: **`due_date ASC NULLS LAST` → `created_at DESC` → `id ASC`** |
...
| `/dashboard` | Yes | **Student dashboard cockpit** — real user-owned stats from **`GET /api/dashboard/stats`** (**5B** + **5C** + **5C.1** + **DASHBOARD-DEPTH-A1** last-7-days focus snapshot + **DASHBOARD-DEPTH-A2** deadline-aware aggregates — optional **`referenceDate`**, browser-local date on fetch, **`overduePendingTasks`** / **`dueTodayPendingTasks`** / **`deadlineReferenceDate`**, **At a glance** **Overdue** / **Due today**, recommendation **overdue-tasks → due-today-tasks → pending-tasks → …** + **BX-I3** decision layout + **FLASHCARD-REVIEW-A5c** due-flashcard-aware hero + **BX-I4** course workload row accents + **BX-I6B** command-center presentation + **BX-I7C** desktop grid at **≥1280px**); read-only; rule-based **“What should I study next?”** flagship hero; deadline heroes and positive overdue/due-today stats link to backend-filtered task lists **`/tasks?status=pending&deadline=overdue`** and **`/tasks?status=pending&deadline=due_today`** (**TASK-DUE-FILTERS-A1** — no secondary course CTA on deadline kinds); generic **`pending-tasks`** hero still supports **`/tasks?status=pending`** and **`/tasks?courseId=<id>&status=pending`** (**A1**); Study pulse cockpit band (**Last 7 days** focus metrics + Pending/Completed/Total + progress bars); course workload command deck; tertiary **At a glance** stat bands (**Overdue**, **Due today**, **Due now** + **Ready for review** deep link — **A2** + **A4c** + **A5a**); fetch on mount + **Try again** + **Refresh stats**; silent refresh when mounted after stat-changing actions elsewhere; presentation **8C-2A** + **B2** cockpit width + **B3** stat tiles + **BX-I3** + **BX-I6B** + **BX-I7C** (**`layout.css`** grid only) |
...
- **Dashboard deadline intelligence (beyond DASHBOARD-DEPTH-A2 / A1 / P0 / TASK-DUE-FILTERS-A1)** — **shipped:** rolling **Last 7 days** focus snapshot (**A1**); optional per-task **`dueDate`** on create/edit + card display (**P0**); deadline-aware dashboard aggregates (**A2** — **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`deadlineReferenceDate`**; optional validated **`referenceDate`**; browser-local frontend date alignment; UTC fallback when omitted; recommendation priority including **overdue-tasks** / **due-today-tasks**; **At a glance** **Overdue** / **Due today**); backend-filtered overdue/due-today task lists + global URL **`deadline`** + in-memory course deadline filters (**TASK-DUE-FILTERS-A1** — **`/tasks?status=pending&deadline=overdue`** and **`/tasks?status=pending&deadline=due_today`**; **`referenceDate`** not in browser URL). **Deferred:** upcoming deadline window; due-date **sorting**; no-due-date filter; upcoming/this-week filters; custom date ranges; stored user timezone persistence; calendar context; reminders/notifications; exam dates; Gemini-generated due dates; Trello due-date sync; AI scheduling; backend **`GET …?materialId=`**. Optional task due dates + A2 aggregates + A1 deadline filters are **not** a complete deadline-planning system — further dashboard/task deadline phases require explicit approval.
