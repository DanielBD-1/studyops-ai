...
**DOCS-TASK-MATERIAL-DASHBOARD-A1 (2026-06-29):** Phase **DOCS-TASK-MATERIAL-DASHBOARD-A1** — documentation-only alignment for **TASK-MATERIAL-DASHBOARD-A1** + **FIX1** + **FIX2** + **FIX2-T1** + **T2** in **`IMPLEMENTATION_STATUS.md`**, **`CURRENT_STATE.md`**, and **`AGENT_MEMORY.md`**. Records **`GET /api/dashboard/stats`** fields **`materialsWithPendingTasks`** and **`topMaterialsByPendingTasks[]`** (top **5**; pending linked tasks only; user-owned; material metadata from owned courses; task/material course mismatches excluded; completed/unlinked excluded; order **`pendingTasks` DESC → `materialTitle` ASC → `materialId` ASC**; **two** fixed aggregate queries; **no** N+1; **no** migration); **`/dashboard`** section order Hero → Study pulse → **Materials with pending tasks** → Course workload → At a glance; material row deep links **View linked tasks** → **`/tasks?courseId=…&materialId=…`**, **View tasks on course** → **`/courses/:courseId?materialId=…`**; **FIX1** removed mutable production auth test override — tests use Provider-scoped **`AuthContext.Provider`** only; **FIX2** stabilized global **`/tasks`** material deep-link hydration (URL-aware initial filters; provisional UUID retention; hydration readiness; canonicalize after conclusive successful hydration; **`materialId=none`**; latest-request-wins; Back/Forward + refresh); **FIX2-T1** same-mount URL navigation test; **T2** dashboard runtime suite registration. **No** application code, tests, migrations, packages, or CI changes in this gate. Backend **673/673**; frontend **616/616**; lint passed (frontend **6** non-blocking **`react-refresh/only-export-components`** warnings); build passed; **`git diff --check`** clean. Supervisor Review **APPROVE WITH NOTES** (A1 + FIX1 re-review + FIX2); Security Review **SECURITY APPROVED WITH NOTES**; initial manual smoke **FAIL** → repeat after **FIX2** **PASS WITH NOTES**. **Git closure not started** — A1 bundle **uncommitted** in working tree; repository baseline **`ad15820`** (**TASK-COURSE-FILTERS-E1** + **FIX1**); CI green **not** claimed.

**TASK-MATERIAL-DASHBOARD-A1-T2 (2026-06-29):** Phase **TASK-MATERIAL-DASHBOARD-A1-T2** — registered previously omitted dashboard runtime integration suite in standard frontend **`npm test`**. **Test registration only** — **no** new test assertions in **T2** itself. **Uncommitted** in working tree at documentation gate.

**TASK-MATERIAL-DASHBOARD-A1-FIX2-T1 (2026-06-29):** Phase **TASK-MATERIAL-DASHBOARD-A1-FIX2-T1** — test-only runtime coverage for same-mount URL navigation **Course A + Material A → Course B + Material B** while Course B material hydration is delayed. Validates **FIX2** provisional retention under delayed hydration. **Uncommitted** in working tree at documentation gate.

**TASK-MATERIAL-DASHBOARD-A1-FIX2 (2026-06-29):** Phase **TASK-MATERIAL-DASHBOARD-A1-FIX2** — focused frontend fix for global **`/tasks`** material deep-link hydration defect found in manual smoke. **Issue:** dashboard **View linked tasks** **`/tasks?courseId=&materialId=`** lost **`materialId`** while page hydrated. **Fix:** URL-aware synchronous initial filters; provisional material UUID retention; course-specific material hydration state; successful-empty vs not-loaded/error distinction; canonicalize only after conclusive successful hydration; **`materialId=none`** preservation; latest-request-wins task loading; Back/Forward + refresh. Dashboard link helper and backend task API were already correct. Initial manual smoke **FAIL** → repeat **PASS WITH NOTES**. Supervisor Review **APPROVE WITH NOTES**; Security Review **SECURITY APPROVED WITH NOTES**. Frontend **616/616**; lint passed; build passed. **Uncommitted** in working tree at documentation gate.

**TASK-MATERIAL-DASHBOARD-A1-FIX1 (2026-06-29):** Phase **TASK-MATERIAL-DASHBOARD-A1-FIX1** — removed mutable production auth override introduced during initial dashboard runtime test work. Removed **`authContextValueForTests`**, **`__setAuthContextValueForTests`**, and **`useAuth()` early override path**. Tests inject authentication only through test-local **`AuthContext.Provider`**. Ordinary exported React Context reviewed and accepted. Focused Supervisor Re-review **APPROVE WITH NOTES**. **Uncommitted** in working tree at documentation gate.

**TASK-MATERIAL-DASHBOARD-A1 (2026-06-29):** Phase **TASK-MATERIAL-DASHBOARD-A1** — dashboard material-linked pending-task counts + deep links. **`GET /api/dashboard/stats`** always returns **`materialsWithPendingTasks`** and **`topMaterialsByPendingTasks[]`** (**materialId**, **courseId**, **materialTitle**, **pendingTasks**). Pending **linked** tasks only; user-owned; material metadata from owned courses; task/material course mismatches excluded; completed/unlinked excluded; top **5**; deterministic order; exactly **two** aggregate-support queries; **no** N+1; **no** migration. **`/dashboard`:** Hero → Study pulse → **Materials with pending tasks** → Course workload → At a glance; each row **View linked tasks** → **`/tasks?courseId=…&materialId=…`**, **View tasks on course** → **`/courses/:courseId?materialId=…`**. Backend **673/673**; frontend **616/616**; lint passed; build passed. Supervisor Review **APPROVE WITH NOTES**; Security Review **SECURITY APPROVED WITH NOTES**; initial manual smoke **FAIL** (hydration defect) — resolved in **FIX2**. **Deferred:** task-detail routes; per-material deadline analytics; charts; recommendations changes; new backend task APIs; E2E framework coverage. **Uncommitted** in working tree — baseline **`ad15820`**.

**DOCS-TASK-COURSE-FILTERS-E1 (2026-06-23):** Phase **DOCS-TASK-COURSE-FILTERS-E1** — documentation-only alignment for **TASK-COURSE-FILTERS-E1** + **TASK-COURSE-FILTERS-E1-FIX1** in **`IMPLEMENTATION_STATUS.md`**, **`CURRENT_STATE.md`**, and **`AGENT_MEMORY.md`**. Records URL-persisted task filters on **`/courses/:courseId`** (**`materialId`** UUID or **`none`**, **`status`**, **`deadline`**); course ID in path only; defaults omitted; deep links, refresh, bookmark/share, Back/Forward; invalid/stale canonicalization; deadline implies **Pending**; **Completed** clears deadline; mutation refetch preserves active filters; **`focusReturnTo`** preserves course-page query; material-detail secondary **View tasks on course** → **`/courses/:courseId?materialId=<uuid>`**; primary **View linked tasks** remains **`/tasks?courseId=…&materialId=…`**; **FIX1** closed stale-request race (URL-aware initial state + latest-request-wins sequencing — **no** `AbortController`, **no** client-side task filtering, **no** artificial delays). **No** application code, tests, migrations, packages, or CI changes. Implementation Supervisor Review **APPROVE WITH NOTES** (E1 + FIX1); Security Review **not required**; manual browser smoke **PASS WITH NOTES**; frontend **573/573**, lint passed, build passed; **`git diff --check`** clean. Implementation **committed** at **`ad15820`** — prior docs gate incorrectly stated uncommitted.

**TASK-COURSE-FILTERS-E1-FIX1 (2026-06-23):** Phase **TASK-COURSE-FILTERS-E1-FIX1** — focused frontend stabilization for stale-request race on **`/courses/:courseId`** filtered deep links. **Issue (E1 manual smoke):** an older unfiltered request could overlap a filtered deep-link request; stale response could overwrite the correct filtered result. **Fix:** URL-aware initial filter state (**`readCoursePageInitialTaskFilters`**); latest-request-wins sequencing (**`loadSeqRef`**); stale success/error/loading suppression; invalidation of requests from a previous course. **Not in FIX1:** `AbortController`; client-side task filtering; artificial delays. Post-FIX1 smoke: UUID and **`materialId=none`** deep links issue no initial unfiltered request; obsolete responses do not overwrite settled results; course-to-course navigation does not leak prior-course tasks. **Accepted non-blocking notes:** React Strict Mode may duplicate dev requests with same active filters; during rapid filter changes prior list may remain briefly while latest request loads — settled result correct; no dedicated stale-auth-error test; active auth failures use existing **`handleAuthError`**; obsolete requests cannot trigger stale auth handling. Frontend **573/573**; lint passed; build passed. Supervisor Review **APPROVE WITH NOTES**; Security Review **not required**; manual smoke **PASS WITH NOTES**. **Committed** at **`ad15820`**.

**TASK-COURSE-FILTERS-E1 (2026-06-23):** Phase **TASK-COURSE-FILTERS-E1** — frontend-only URL-persisted material/status/deadline filters on **`/courses/:courseId`**. Query params: **`materialId`** (UUID or **`none`**), **`status=pending|completed`**, **`deadline=overdue|due_today|next_7_days`**; course ID in route path only; default **All** values omitted; bidirectional URL sync via **`useSearchParams`**; list fetch sends active filters to backend (**TASK-MATERIAL-BACKEND-A1** + deadline filters) — **no** client-side membership filtering. Deep links, refresh persistence, bookmarkable/shareable URLs, Back/Forward restoration, invalid/stale canonicalization, deadline implies **Pending**, **Completed** clears deadline, mutation refetch preserves active filters, **`focusReturnTo`** preserves course-page query. Material-detail secondary **View tasks on course** → **`/courses/:courseId?materialId=<uuid>`** via **`buildCoursePageMaterialLink`**. **Known issue at E1 ship:** stale-request race on filtered deep links — **resolved in FIX1**. **No** backend/API/database/migration/package changes. Frontend **573/573**; lint passed; build passed. Supervisor Review **APPROVE WITH NOTES**; Security Review **not required**. **Committed** at **`ad15820`**.

**DOCS-TASK-MATERIAL-BACKEND-A1 (2026-06-21):** Phase **DOCS-TASK-MATERIAL-BACKEND-A1** — documentation-only alignment for **TASK-MATERIAL-BACKEND-A1** in **`IMPLEMENTATION_STATUS.md`**, **`CURRENT_STATE.md`**, **`AGENT_MEMORY.md`**, **`docs/database/015-study-tasks-due-date.md`**, and **`README.md`**. **No** application code, tests, migrations, packages, or CI changes. **Security Review: not required** (docs-only alignment of **SECURITY APPROVED** implementation). **Documentation Supervisor Review: APPROVE WITH NOTES** — no Blocking or Important findings; N-1 and N-2 accepted as non-blocking editorial notes; no documentation fix phase required. Next gate: human-controlled **Git closure** → CI green — **not** started.

**TASK-MATERIAL-BACKEND-A1 (2026-06-21):** Phase **TASK-MATERIAL-BACKEND-A1** — backend task list **material membership** on **`GET /api/tasks`** and **`GET /api/courses/:courseId/tasks`**. Optional **`materialId`**: omitted = no material predicate; UUID = owned-material filter; **`none`** = **`material_id IS NULL`**. Ownership via authenticated **`user_id`** + material→course chain; missing/wrong-owner/mismatch → neutral **`404 Study material not found`**; valid owned material with zero tasks → **`200 []`**; invalid values → **`400 VALIDATION_ERROR`**. Composes with **`courseId`**, **`status`**, **`deadline`**, **`referenceDate`**; ordering unchanged (**TASK-DUE-SORT-A1**). Frontend sends **`materialId`** on list fetch; renders API results directly — removed **`filterTasksByMaterial`** / **`getMaterialLinkedTasks`**; global **`/tasks`** URL persistence unchanged (**TASK-MATERIAL-FILTERS-E1** + **UNLINKED-A1**); course filters in-memory only; **no** migration/index/package. Backend **658/658**; frontend **520/520**; lint passed; build passed; **`git diff --check`**: clean. **Supervisor Review: APPROVE WITH NOTES**; focused Supervisor Re-review **PASS**; **Security Review: PASS / SECURITY APPROVED**; manual smoke **PASS WITH NOTES**. **Deferred at A1 ship:** URL-persisted task filters on **`/courses/:id`** (later **TASK-COURSE-FILTERS-E1**); dashboard material-linked pending counts (later **TASK-MATERIAL-DASHBOARD-A1**); task detail page; migrations/indexes until measured; new sort modes; Trello/flashcards/admin/generate changes. Docs: **DOCS-TASK-MATERIAL-BACKEND-A1** (Supervisor Review **pending**).
...
**DASHBOARD-DEPTH-A1 (2026-06-17):** Phase **DASHBOARD-DEPTH-A1** — dashboard rolling focus snapshot + filtered pending-task links. **`GET /api/dashboard/stats`** adds **`focusMinutesLast7Days`**, **`completedFocusSessionsLast7Days`** — rolling **168-hour** window; **completed sessions only** (`ended_at IS NOT NULL`). **`/dashboard`** Study pulse **Last 7 days** band. **`pending-tasks`** hero primary CTA **`/tasks?status=pending`**; secondary **`/tasks?courseId=<id>&status=pending`** for most-pending course. Merge **`bb499ce`** / **`c746333`**. **Supervisor Review: approved.** **Security Review: approved.** Main CI **green**. **Explicit exclusions:** charts, streaks, goals, deadline-aware dashboard logic, AI scheduling. Supersedes pre-A1 docs that said **`DashboardStub.jsx` unchanged** and pending CTA to bare **`/tasks`** (**FLASHCARD-REVIEW-A5c** / **BX-I3** historical context). Docs: **CORE-DEPTH-RECENT-DOCS-A1**.

**CORE-DEPTH-BEFORE-COLLAB-DOCS-A1 (2026-06-07):** Phase **CORE-DEPTH-BEFORE-COLLAB-DOCS-A1** — documentation-only codification of human-approved product sequencing. **Personal study-engine depth precedes collaboration** — strengthen and connect the personal study workflow before opening a collaboration layer. The sequencing decision originated in **CORE-DEPTH-BEFORE-COLLAB-PLAN** (planning-only conversation) and previously existed only outside the repository. **`docs/PRODUCT_STRATEGY.md`** is now the standing reference for product sequencing and strategic direction. **`docs/CURRENT_STATE.md`** updated minimally — agent starting path, source-of-truth hierarchy entry, Quick links row, Forbidden assumptions row. **No** application code, tests, migrations, or forbidden doc changes. **`docs/PRODUCT_STRATEGY.md`** provides **guidance only** — it does **not** approve implementation of Study Rooms, PDF, dashboard depth, deployment, full SRS, or any other epic. Existing approval gates in **`AGENTS.md`** remain mandatory (strategy direction ≠ planning approval ≠ implementation approval). **Security Review: not required** (docs-only).

**FLASHCARD-REVIEW-A5c (2026-06-07):** Phase **FLASHCARD-REVIEW-A5c** — frontend-only dashboard hero / recommendation due-flashcard awareness. **`deriveDashboardRecommendation`** in **`dashboard-recommendation.js`** reads existing **`dueFlashcardsCount`** from **`GET /api/dashboard/stats`**; when **`pendingTasks === 0`** and **`dueFlashcardsCount > 0`**, hero recommends reviewing due flashcards (**kind** **`due-flashcards`**) with primary CTA **`Review due flashcards`** → **`buildFlashcardsPageDueNowLink()`** → **`/flashcards?reviewState=due_now`**; when **`pendingTasks > 0`**, **`pending-tasks`** still wins; when **`dueFlashcardsCount === 0`** and **`totalFlashcards > 0`**, generic **`flashcards`** recommendation unchanged; priority order: **no-courses → pending-tasks → due-flashcards → plan-gap → flashcards → add-tasks-or-plans → empty-workspace → caught-up**; **`DashboardStub.jsx`** **unchanged** (renders recommendation object only); **no** backend/database/migration/URL contract changes; **FLASHCARD-REVIEW-A5a** URL filters **unchanged**; **FLASHCARD-REVIEW-A5b** auto-advance **unchanged**; plan JSON **`GeneratedPlanSection`** **unchanged**; **no** backend **`?due=now`** / **`?mastery=`**; **not** full SM-2/Anki; **no** **`ease_factor`**; **no** review history table; **no** dashboard charts/admin due cards/notifications/calendar. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only over existing aggregate stat). **`cd frontend && npm run lint`**, **`npm test`** (**426** pass), **`npm run build`** passed; main CI **green**. Docs: **DOCS-FLASHCARD-REVIEW-A5c**.

**FLASHCARD-REVIEW-A5b (2026-06-07):** Phase **FLASHCARD-REVIEW-A5b** — frontend-only auto-advance after successful Known/Unknown review on **saved DB flashcards**. Surfaces: global **`/flashcards`** **`GlobalFlashcardsSection`** and material detail **`DbFlashcardsSection`**. After successful review: advances to next card, **`revealed=false`**; card stays in current filtered deck → **`getNextIndex`** (wrap); card leaves filtered deck → same slot when possible, safe last-card/empty-deck handling; review failure / handler **`false`** → no advance; pending advance + **`useEffect`** on latest **`flashcards`** props after parent local merge avoids stale-deck race; parent **`handleReviewOutcome`** returns **`true`** on success, **`false`** on failure. **`computeIndexAfterSuccessfulReview`** in **`flashcard-study.js`**; **`FlashcardStudy.jsx`**, **`GlobalFlashcardsSection.jsx`**, **`DbFlashcardsSection.jsx`**. **`GeneratedPlanSection`** plan JSON **unchanged** — **no** **`onReviewOutcome`**, flip/reveal only. **No** backend/database/migration/URL changes; **FLASHCARD-REVIEW-A5a** URL filters **unchanged**; **no** backend **`?due=now`** / **`?mastery=`**; **not** full SM-2/Anki; **no** **`ease_factor`**; **no** review history table; hero / **`deriveDashboardRecommendation`** **unchanged**. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only UX over existing authenticated review API). **`cd frontend && npm run lint`**, **`npm test`** (**423** pass), **`npm run build`** passed; main CI **green**. Docs: **DOCS-FLASHCARD-REVIEW-A5b**.

**FLASHCARD-REVIEW-A5a (2026-06-07):** Phase **FLASHCARD-REVIEW-A5a** — frontend-only URL-persisted filters on global **`/flashcards`**. Supported query params: **`courseId=<uuid>`**, **`materialId=<uuid>`**, **`reviewState=due_now|needs_review|new|learning|known`**. Opening a URL initializes filters; manual course/material/review-state filter changes update the URL; browser Back/Forward restores filter state; all/default values omitted from URL; **`reviewState=all`** not emitted; **`materialId`** never emitted without **`courseId`**; invalid **`reviewState`** falls back safely and is stripped; invalid/stale **`courseId`** / **`materialId`** fall back safely and canonicalize URL. Helpers in **`flashcard-nav-query.js`**; bidirectional sync in **`GlobalFlashcardsSection`** via **`useSearchParams`** + **`skipUrlSyncRef`**. **`reviewState`** remains client-side only — backend **`listFlashcards`** still **`courseId`** / **`materialId`** only; **no** backend **`?due=now`**; **no** backend **`?mastery=`**. **`DbFlashcardsSection`** on material detail **unchanged** (in-memory filters only). **`GeneratedPlanSection`** plan JSON **unchanged**. Dashboard **Ready for review** link → **`/flashcards?reviewState=due_now`** when **`dueFlashcardsCount > 0`**; hero / **`deriveDashboardRecommendation`** **unchanged**. **No** backend/database/migration/package/CI changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only URL/filter sync over existing authenticated data). **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. Docs: **DOCS-FLASHCARD-REVIEW-A5a**.

**TASK-MATERIAL-NAV-B1 (2026-06-07):** Phase **TASK-MATERIAL-NAV-B1** — frontend-only material detail → **`/tasks`** deep link. **`MaterialRelatedTasksSection`** primary **`Link`** **View linked tasks** → **`/tasks?courseId=<courseId>&materialId=<materialId>`** via **`buildTasksPageMaterialLink`** in **`task-nav-query.js`**; secondary **View tasks on course** → **`/courses/:courseId`**; shared helpers **`parseTasksPageSearchParams`**, **`buildTasksPageSearchParams`**, **`resolveInitialTaskFilters`**. **TASK-MATERIAL-FILTERS-E1** later expanded **`/tasks`** to **full URL-persisted** bidirectional sync (manual filter changes update URL; Back/Forward; **`status`**); **TASK-MATERIAL-UNLINKED-A1** later added **`materialId=none`**. Current **`/tasks`** behavior is **not** initial-load-only. **`materialId`** remains client-side only — backend **`listAllTasks`** still **`courseId`** / **`status`** only; **no** backend **`?materialId=`**; **no** **`/courses/:id`** URL filters. **`focusReturnTo`** preserves full **`/tasks?…`** query string. **No** backend/database/migration/package/CI changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only navigation over existing authenticated APIs). **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. Docs: **DOCS-TASK-MATERIAL-NAV-B1-RECONCILE**.

**TASK-MATERIAL-UNLINKED-A1 (2026-06-07):** Phase **TASK-MATERIAL-UNLINKED-A1** — frontend-only **Tasks without material** filter on **`/courses/:id`** and **`/tasks`**. Internal sentinel **`materialFilter === 'none'`**; unlinked detection uses **`task.materialId` only** (null, undefined/missing, or empty string — **not** `materialTitle`). **`/tasks`** URL **`/tasks?courseId=<uuid>&materialId=none`** — **`materialId=none`** requires valid owned **`courseId`**; **`/tasks?materialId=none`** without valid course falls back safely; **`none`** parsed before UUID validation; does not wait for **`listMaterials`**. **`/courses/:id`** adds **Tasks without material** option in memory only — **no** URL params. Client-side **`filterTasksByMaterial`** / **`isTaskUnlinked`** in **`task-filters.js`**; URL helpers in **`task-nav-query.js`**; **`GlobalTasksSection`** + **`CourseTasksSection`**. Backend **`listAllTasks`** / **`listCourseTasks`** still **`courseId`** / **`status`** only — **no** backend **`?materialId=`**; **no** cross-course unlinked filter when **All courses** selected. B1 **`buildTasksPageMaterialLink`** unchanged. **No** backend/database/migration/package/CI changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only filter over existing authenticated data). **`cd frontend && npm run lint`**, **`npm test`** (**393** pass), **`npm run build`** passed; main CI **green**. Docs: **DOCS-TASK-MATERIAL-UNLINKED-A1**.

**TASK-MATERIAL-FILTERS-E1 (2026-06-07):** Phase **TASK-MATERIAL-FILTERS-E1** — frontend-only URL-persisted task filters on **`/tasks`**. Supported query params: **`courseId=<uuid>`**, **`materialId=<uuid>`**, **`status=pending|completed`**. Opening a URL initializes filters (owned course/material validation via **`resolveInitialTaskFilters`**); manual course/material/status filter changes update the URL; browser Back/Forward restores filter state; all/default values omitted from URL; **`materialId`** never emitted without **`courseId`**; course change resets **`materialFilter`** and removes **`materialId`** from URL. Helpers in **`task-nav-query.js`**; bidirectional sync in **`GlobalTasksSection`** via **`useSearchParams`** + **`skipUrlSyncRef`**. **`materialId`** remains client-side only — backend **`listAllTasks`** still receives only **`courseId`** / **`status`**; **no** backend **`?materialId=`**; **no** **`/courses/:id`** URL filter support; **no** dashboard or flashcards URL filter changes. B1 **`buildTasksPageMaterialLink`** deep link unchanged; **`focusReturnTo`** preserves full query string. **No** backend/database/migration/package/CI changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only URL/filter sync over existing authenticated data). **`cd frontend && npm run lint`**, **`npm test`** (**381** pass), **`npm run build`** passed; main CI **green**. Docs: **DOCS-TASK-MATERIAL-FILTERS-E1**.

**FLASHCARD-REVIEW-A4c (2026-06-07):** Phase **FLASHCARD-REVIEW-A4c** — dashboard due-card awareness for **saved DB flashcards**. Existing **`GET /api/dashboard/stats`** adds **`dueFlashcardsCount`** — aggregate count of caller-owned flashcards where **`next_review_at IS NULL OR next_review_at <= now`**; **`user_id`** scoped; count-only query (**no** question/answer). **`/dashboard`** **At a glance** → **Learning assets** shows **Due now** (count including **0**); when **`dueFlashcardsCount > 0`**, **Ready for review** + link to **`/flashcards`** only (**no** URL query params / deep link to Due now filter). **`totalFlashcards`**, **`courseStats`**, and hero / **`deriveDashboardRecommendation`** unchanged. **No** new endpoint; **no** backend **`?due=now`**; **no** dashboard charts; **no** admin due cards; **not** full SRS/SM-2/Anki; **no** **`ease_factor`**; **no** review history table. **`cd backend && npm run lint`**, **`npm test`**, **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. **Supervisor Review: approved with notes.** **Security Review: not required** (read-only aggregate on existing authenticated dashboard endpoint; backend security spot-check passed). Docs: **DOCS-FLASHCARD-REVIEW-A4c**.

**FLASHCARD-REVIEW-A4b (2026-06-07):** Phase **FLASHCARD-REVIEW-A4b** — frontend-only **Due now** filter for **saved DB flashcards**. **`isFlashcardDueNow`** + **`due_now`** case in **`filterFlashcardsByReviewState`** (**`flashcard-filters.js`**) filters already-loaded cards by **`nextReviewAt`** from **FLASHCARD-REVIEW-A4a** — due when null/undefined/empty/invalid or **`Date.parse(nextReviewAt) <= Date.now()`**; future **`nextReviewAt`** hidden. Surfaces: material detail **`DbFlashcardsSection`** and global **`/flashcards`** **`GlobalFlashcardsSection`**; **Review state** select adds **Due now** after **All** (existing **All**, **New + Learning**, **New**, **Learning**, **Known** unchanged); filter affects study deck and manage list; due-only empty copy **No flashcards are due for review right now.** **No** backend **`?due=now`**; **no** URL-persisted filters. **FLASHCARD-REVIEW-A2** Known/Unknown unchanged — same **`POST /api/flashcards/:flashcardId/review`**; local merge; **no** refetch on review; **no** auto-advance. **FLASHCARD-REVIEW-A3** mastery filters unchanged. **`GeneratedPlanSection`** plan JSON **unchanged** — **no** Due now filter; **no** review API. **Not** full SRS/SM-2/Anki; **no** dashboard due cards (**A4c** deferred at A4b ship time); **no** review history table. **No** backend/database/migration/package changes. **Supervisor Review: approved.** **Security Review: not required** (frontend-only filter over already-loaded user-owned data). **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. Docs: **DOCS-FLASHCARD-REVIEW-A4b**.

**FLASHCARD-REVIEW-A4a (2026-06-07):** Phase **FLASHCARD-REVIEW-A4a** — backend/database SRS-lite scheduling foundation for **saved DB flashcards**. Migration **`014_flashcard_srs_scheduling.sql`** **applied successfully** on Supabase — adds **`next_review_at`** (nullable) and **`review_interval_days`** (NOT NULL default **0**, CHECK **`>= 0`**) on **`public.flashcards`**; **no** **`ease_factor`**; **no** review history table; **no** new indexes. **`computeNextReviewState`** in **`flashcard-scheduling.js`** — **`MAX_INTERVAL_DAYS = 90`**; **`POST /api/flashcards/:flashcardId/review`** body **`{ outcome: "known" | "unknown" }`** strict; scheduling computed server-side on review: **known** from **new**/**learning** → interval **1** day; **known** from **known** with interval **0** → interval **1** day; **known** from **known** with interval **≥ 1** → interval doubles capped at **90** days; **unknown** → **`mastery = learning`**, interval **0**, next review in **1** day; scheduling fields **not** writable via PATCH/create; list/create/PATCH/review responses include **`nextReviewAt`** and **`reviewIntervalDays`**; **no** **`userId`**. **No** frontend in A4a — **FLASHCARD-REVIEW-A2**/**A3** unchanged; Due now filter shipped in **FLASHCARD-REVIEW-A4b**; plan JSON **`GeneratedPlanSection`** **unchanged**. **SRS-lite foundation only** — **not** full SRS/SM-2/Anki; **no** dashboard due cards (**A4c** deferred); **no** backend **`?due=now`** or **`?mastery=`**; **no** URL-persisted filters; **no** auto-advance. **Supervisor Review: approved with notes.** **Security Review: approved with notes** (read-modify-write concurrency acceptable for A4a). **`cd backend && npm run lint`** and **`npm test`** passed; main CI **green**. Docs: **DOCS-FLASHCARD-REVIEW-A4a**.

**FLASHCARD-REVIEW-A3 (2026-06-07):** Phase **FLASHCARD-REVIEW-A3** — frontend-only client-side review-state filter / simple review queue for **saved DB flashcards**. **`filterFlashcardsByReviewState`** in **`flashcard-filters.js`** filters already-loaded cards by **`mastery`**: **All**, **New + Learning** (`needs_review`), **New**, **Learning**, **Known**. Surfaces: material detail **`DbFlashcardsSection`** and global **`/flashcards`** **`GlobalFlashcardsSection`**; filter affects study deck and manage list; composes with course/material filters on **`/flashcards`**. **No** backend **`?mastery=`**; **no** URL-persisted filters; **no** sorting by **`lastReviewedAt`**. **FLASHCARD-REVIEW-A2** Known/Unknown unchanged — same **`POST /api/flashcards/:flashcardId/review`**; local merge; **no** refetch on review; **no** auto-advance. **`GeneratedPlanSection`** plan JSON **unchanged** — **no** review filter; **no** review API. **Not** full SRS; **no** **`next_review_at`**; **no** SM-2/Anki; **no** dashboard due cards; **no** review history table. **No** backend/database/migration/package changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only filter over already-loaded user-owned data). **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. **FLASHCARD-REVIEW-A4** **not shipped** — separate phase gate. Docs: **DOCS-FLASHCARD-REVIEW-A3**.

**FLASHCARD-REVIEW-A2 (2026-06-06):** Phase **FLASHCARD-REVIEW-A2** — frontend-only Known/Unknown review buttons for **saved DB flashcards**. Surfaces: material detail **`DbFlashcardsSection`** and global **`/flashcards`** **`GlobalFlashcardsSection`**. After answer reveal, **Known** / **Unknown** call existing **`POST /api/flashcards/:flashcardId/review`** body `{ outcome: "known" | "unknown" }` via **`reviewFlashcard`** in **`flashcards.service.js`**; successful review merges returned flashcard into local state by id (**no** full reload/refetch on review); user stays on same card (**no** auto-advance). **`GeneratedPlanSection`** plan JSON **`FlashcardStudy`** **unchanged** — **no** **`onReviewOutcome`**, **no** review API, **no** persisted review claim. **`buildStudySetKey`** in **`flashcard-study.js`** prevents study position reset on review metadata updates. **Not** full SRS; **no** **`next_review_at`**; **no** SM-2/Anki; **no** dashboard due cards. Review-state filter shipped separately in **FLASHCARD-REVIEW-A3**. **No** backend/database/migration/package changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only over authenticated backend endpoint). **`cd frontend && npm run lint`**, **`npm test`** (**337** pass), **`npm run build`** passed; main CI **green**. Docs: **DOCS-FLASHCARD-REVIEW-A2**.

**FLASHCARD-REVIEW-A1 (2026-06-06):** Phase **FLASHCARD-REVIEW-A1** — backend/database foundation for persisted flashcard review state. Migration **`013_flashcard_review_state.sql`** **applied successfully** on Supabase — adds aggregate columns on **`public.flashcards`**: **`mastery`** (`new` \| `learning` \| `known`), **`last_reviewed_at`**, **`review_count`**, **`known_count`**, **`unknown_count`**; **no** **`next_review_at`**; **no** review history table; **no** new indexes. Backend **`POST /api/flashcards/:flashcardId/review`** — **`requireAuth`**; body **`{ outcome: "known" | "unknown" }`** strict; **`known`** → `mastery = "known"`, increments **`review_count`** + **`known_count`**; **`unknown`** → `mastery = "learning"`, increments **`review_count`** + **`unknown_count`**; **`last_reviewed_at`** set on review; wrong-owner/missing → neutral **`404`** “Flashcard not found”; review state **not** writable via PATCH/create. List/create/PATCH/review responses include review fields; **no** **`userId`**. **No** frontend in A1 — Known/Unknown UI shipped in **FLASHCARD-REVIEW-A2**; review-state filter shipped in **FLASHCARD-REVIEW-A3**. **Not** full SRS; **no** SM-2/Anki; **no** dashboard due cards. **Supervisor Review: approved with notes.** **Security Review: PASS WITH NOTES** (non-atomic counter increment under concurrency accepted for A1). **`cd backend && npm run lint`** and **`npm test`** passed; main CI **green**. **FLASHCARD-REVIEW-A4** **not shipped** — separate phase gate. Docs: **DOCS-FLASHCARD-REVIEW-A1**.

**TASK-MATERIAL-DETAIL-A3 (2026-06-06):** Phase **TASK-MATERIAL-DETAIL-A3** — frontend-only read-only **Related study tasks** band on **`/study-materials/:materialId`**. **`MaterialRelatedTasksSection`** between material cockpit and **Saved flashcards**; loads course tasks via existing **`listCourseTasks(courseId)`**; client-side filter via **`getMaterialLinkedTasks`** / **`summarizeLinkedTaskCounts`** / **`selectLinkedTaskPreview`** in **`task-filters.js`**; pending/completed/total counts; up to **5** compact read-only preview rows (pending before completed); loading, empty, error + **Try again**, populated states; primary **`Link`** **View linked tasks** → **`/tasks?courseId=&materialId=`** (**TASK-MATERIAL-NAV-B1**); secondary **`Link`** **View tasks on course** → **`/courses/:courseId`**; refreshes after successful or partially successful plan task import (**`refreshRelatedTasks`** / **`refreshKey`**); subtitle distinguishes saved DB tasks from AI plan JSON preview; **no** **`TaskCard`**; **no** edit, complete, delete, or Focus actions. **No** backend/API/database/migration changes. **`frontend/package.json`** test script updated to include **`material-linked-tasks.test.js`** only. **Supervisor Review: passed.** Manual smoke: **passed.** Follow-up manual smoke: **passed.** **Security Review: not required** (frontend-only display over already authenticated **`listCourseTasks`** data; no new API surface). **`npm test`**, **`npm run lint`**, **`npm run build`** passed. Material detail deep link shipped in **TASK-MATERIAL-NAV-B1**; URL-persisted **`/tasks`** filters shipped in **TASK-MATERIAL-FILTERS-E1**; **Tasks without material** filter shipped in **TASK-MATERIAL-UNLINKED-A1**. Backend **`materialId`** query params and **`/courses/:id`** URL filters remain **deferred**. Docs: **DOCS-TASK-MATERIAL-DETAIL-A3**; NAV-B1 docs reconciled in **DOCS-TASK-MATERIAL-NAV-B1-RECONCILE**.

**TASK-MATERIAL-FILTERS-A2 (2026-06-06):** Phase **TASK-MATERIAL-FILTERS-A2** — frontend-only study material filters on **`/courses/:id`** and **`/tasks`**. **`filterTasksByMaterial`** / **`resetMaterialFilterForCourseChange`** in **`task-filters.js`**; course task list always shows **Study material** filter using course-page **`materials`**; global **`/tasks`** shows material filter only after selecting a specific course (**`listMaterials`** for selected course); filters already-loaded tasks by **`task.materialId`** in memory; composes with status/course filters; course change on global **`/tasks`** resets material filter to **all**; unknown/stale material filter id falls back to showing all tasks; global **`/tasks`** improves **`materialLabel`** resolution from loaded course materials. **No** backend/API/database/migration/URL query param changes. **`frontend/package.json`** test script updated to include **`task-filters.test.js`** only. **Supervisor Review: passed.** Manual smoke: **Pass with notes.** **Security Review: not required** (frontend-only filtering over already authenticated data). **`npm test`**, **`npm run lint`**, **`npm run build`** passed. Material detail related tasks band shipped in **TASK-MATERIAL-DETAIL-A3** (see above). Backend **`materialId`** query params, URL-persisted task filters, and “tasks without material” filter remain **deferred**. Docs: **DOCS-TASK-MATERIAL-FILTERS-A2**.

**TASK-MATERIAL-LINKS-A1 (2026-06-06):** Phase **TASK-MATERIAL-LINKS-A1** — frontend-only **`TaskCard`** material navigation on **`/courses/:id`** and **`/tasks`**. When `task.materialId` is set, **`TaskCard`** renders **`Material:`** with React Router **`Link`** to **`/study-materials/:materialId`** — link text from `materialLabel` when provided, otherwise fallback **View study material**; when `task.materialId` is absent, no material line. **No** backend/API/database/migration changes. **`frontend/package.json`** test script updated to include **`task-card-material-link.test.js`** only. **Supervisor Review: passed.** **Security Review: not required** (frontend navigation only). **`npm test`**, **`npm run lint`**, **`npm run build`** passed; GitHub CI on **main** green. Frontend material filtering shipped in **TASK-MATERIAL-FILTERS-A2**; material detail related tasks band shipped in **TASK-MATERIAL-DETAIL-A3**. URL-persisted task filters remain **deferred**. Docs: **DOCS-TASK-MATERIAL-LINKS-A1**.

**Current-state correction (POST-MVP-LANGUAGE-A1 + DOCS-CONSISTENCY-FIX) — do not reopen shipped Trello OAuth:**

- **Trello account connect is live** — **`/trello`** Connect/Disconnect + **`/trello/connect/callback`** (**A4-FRONTEND**).
- **Connected-account sync is the primary `/trello` UX** when linked (**A5B** + **A5A**): frontend `{}` / `{ listId, taskIds }` only.
- **Board/list defaults persistence is live** (**A6**) — **`PATCH /api/trello/connection/defaults`**; connected **`/trello`** preselects saved board/list.
- **Manual fallback** — collapsed advanced credentials **when disconnected only**; user must **Disconnect** before manual credentials if they were connected (**A5C** → **`TRELLO_MANUAL_CREDENTIALS_NOT_ALLOWED`** / **400**).
- **A2–A6 are implemented** — not deferred. Older journal entries that say “OAuth not live”, “manual-only sync path”, “no Trello persistence”, or “A5B not started” are **historical** (pre-ship or docs-only gates).

**POST-MVP-LANGUAGE-A1 (2026-06-04):** Documentation-only — post-MVP language alignment across governance docs. Initial MVP baseline marked complete; StudyOps framed as actively developed web product; **`docs/POST_MVP.md`** added. Agents should **stop** describing the project as demo/MVP-limited. **Future work still requires explicit approval** and existing Supervisor/Security/migration/cost/CI gates. **No** code changes; **no** future epic approved.

**DOCS-CONSISTENCY-FIX (2026-06-04):** Documentation-only — aligned **PRD**, **README**, **IMPLEMENTATION_STATUS**, **AGENT_MEMORY**, **CURRENT_STATE** after **A5C** merge; removed contradictory “future work / not live / A5 deferred” language. **No** code changes.

**GEMINI-GENERATE-UX-A1 (2026-06-04):** Phase **GEMINI-GENERATE-UX-A1** — frontend-only generate readiness and AI error polish on **`/study-materials/:materialId`**. **`study-material-readiness.js`** — draft character count status; **`getStudyMaterialReadiness`** from saved `material.content` + unsaved-change state (ready / save_first / too_short / no_material); Generate disabled when not ready. **`generate-errors.js`** — **`formatGenerateError`** maps `GEMINI_*`, `SERVER_ERROR`, `VALIDATION_ERROR` to stable user messages (fallback for unknown errors). Source editor shows draft `{n} / 50,000` with below-min / over-max visual states. Scoped CSS in **`layout.css`** only. **No** backend/document-service/PDF/packages/migrations changes; **`generateMaterial`** still POST `{}`; plan/history/import unchanged. **PDF upload/parsing remains deferred.**

**Trello OAuth A5C (2026-06-04):** Phase **TRELLO-OAUTH-A5C** — backend manual-credential hardening on **`POST /api/trello/boards`**, **`/boards/:boardId/lists`**, **`/sync`**. Connected users sending manual `{ apiKey, token }` receive **`TRELLO_MANUAL_CREDENTIALS_NOT_ALLOWED`** / **400** — *Use your connected Trello account, or disconnect before using manual credentials.* Reject before decrypt, Trello API call, sync log write; error response includes no credential/body details. Connected + stored `{}` / `{ listId, taskIds }` unchanged. Disconnected + manual fallback unchanged. Disconnected + stored → **`TRELLO_NOT_CONNECTED`**. Partial credentials → **`VALIDATION_ERROR`**. **`assertManualCredentialsAllowed`** in **`trello-credentials.resolver.js`**. **Supervisor Review: Pass with notes.** **Security Review: Security approved with notes.** **`npm test` 448 pass; `npm run lint` pass.** **No frontend changes.** Closes A5A/A5B deferred wrong-account hardening gap. Docs: **DOCS-TRELLO-OAUTH-A5C**.

**Trello OAuth A5B (2026-06-04):** Phase **TRELLO-OAUTH-A5B** — frontend connected-account sync on **`/trello`**. When connected: **`POST /api/trello/boards`** `{}`, **`/boards/:boardId/lists`** `{}`, **`/sync`** `{ listId, taskIds }` — **no** `apiKey`/`token` from frontend; manual credential inputs hidden; UI states boards/lists/sync use connected account. When disconnected: Connect prompt + collapsed **Advanced manual credentials** fallback; manual sends apiKey/token only when explicitly used; credentials ephemeral in React state only. **No backend contract changes.** OAuth Connect/Disconnect (**A4**) unchanged. **Supervisor Review: Pass with notes.** **Security Review: PASS.** **`npm test` 289 pass; `npm run lint` pass; `npm run build` pass.** Backend connected-manual hardening shipped in **A5C** (see above). Docs: **DOCS-TRELLO-OAUTH-A5B**.

**Trello OAuth A5A (2026-06-03):** Phase **TRELLO-OAUTH-A5A** — backend-only additive stored-token mode on **`POST /api/trello/boards`**, **`/boards/:boardId/lists`**, **`/sync`**. **`classifyTrelloCredentialMode`**: no `apiKey`/`token` keys → stored mode (server **`TRELLO_API_KEY`** + **`decryptTokenForUser(req.user.id)`**); both keys → manual (when disconnected; **A5C** rejects when connected); one key → **`VALIDATION_ERROR`** / 400; stored mode, not connected → **`TRELLO_NOT_CONNECTED`** / 400. New **`trello-credentials.resolver.js`**; **`trello.controller.js`** delegates credential resolution. **Supervisor Review: Approved** (after test-wiring fix). **Security Review: PASS.** **`npm test` 442 pass; `npm run lint` pass.** **Frontend connected-account UX shipped in A5B** (see above). Connected-manual hardening shipped in **A5C** (see above). Docs: **DOCS-TRELLO-OAUTH-A5A**.

**Trello OAuth A4-FRONTEND (2026-06-03):** Phase **TRELLO-OAUTH-A4-FRONTEND** — frontend OAuth account connection UI. **`/trello`** `TrelloConnectionPanel` (Connect account / Disconnect); protected **`/trello/connect/callback`** reads **`state` from query**, **`token` from hash** only; clears URL before `POST /api/trello/connect/complete` with `{ token, state }`; `beginOAuthExchange` dedupes StrictMode duplicate POST; token/state not in localStorage/sessionStorage/React state/logs/UI; frontend does not validate state (**A4-STATE** backend). **Supervisor Review: Pass.** **Security Review: Pass.** **`npm test` 271 pass; `npm run lint` pass; `npm run build` pass.** **At ship:** sync section still manual until **A5B** (now complete). Docs: **DOCS-TRELLO-OAUTH-A4-FRONTEND**.

**Trello OAuth A4-STATE (2026-06-03):** Phase **TRELLO-OAUTH-A4-STATE** — backend-only signed OAuth state on connect flow. **`GET /api/trello/authorize-url`** generates HMAC-signed state bound to `req.user.id` and embeds it in `return_url` (`/trello/connect/callback?state=…`). **`POST /api/trello/connect/complete`** requires strict `{ token, state }`; verifies state **before** Trello `/members/me` and **before** DB write; invalid/missing/tampered/expired/foreign state → generic `TRELLO_OAUTH_STATE_INVALID` / 400. Signing: prefer **`TRELLO_OAUTH_STATE_SECRET`**; fallback domain-separated derive from **`TRELLO_TOKEN_ENCRYPTION_KEY`**. **Supervisor Review: Pass.** **Security Review: Security approved with notes** — signed state blocks account-linking CSRF; state is stateless and not single-use; replay within 10-minute TTL accepted MVP residual risk; future hardening: single-use nonce persistence. **`npm test` 417 pass; `npm run lint` pass.** **At ship:** connect UI in **A4-FRONTEND**; stored-token sync in **A5A**–**A5C** (now complete). Docs: **DOCS-TRELLO-OAUTH-A4-STATE**.

**Trello OAuth A3 (2026-06-03):** Phase **TRELLO-OAUTH-A3** — backend-only connect/authorize routes: `GET /api/trello/connection`, `GET /api/trello/authorize-url`, `POST /api/trello/connect/complete`, `POST /api/trello/disconnect` (all **`requireAuth`**; user scope via `req.user.id`). Token validated via Trello before encrypted upsert; disconnect best-effort revoke + local hard-delete. Connected response contract: flat `{ connected: true, trelloMemberId, trelloUsername, scopes, expirationPolicy, expiresAt, defaultBoardId, defaultListId, connectedAt, updatedAt }` — **no** token/ciphertext in responses. **Supervisor Review: Pass with notes.** **Security Review: Security approved with notes** — no blocking issues. **A4-STATE** extended connect/complete with signed `{ token, state }` (see above). **At ship:** backend-only — user-facing connect **A4-FRONTEND**; sync evolution **A5A**–**A5C** (all complete). Docs: **DOCS-TRELLO-OAUTH-A3**.
...
**Branch / phase (2026-06-21):** post-**TASK-MATERIAL-BACKEND-A1** — backend task list material membership shipped; documentation alignment gate **DOCS-TASK-MATERIAL-BACKEND-A1** **APPROVE WITH NOTES** — no documentation fix phase required; workflow next step: human-controlled **Git closure** (manual commit/push → CI green; merge only if applicable to current branch workflow and only after CI green). Authoritative shipped detail: **`docs/IMPLEMENTATION_STATUS.md`**; phase gates: **`docs/CURRENT_STATE.md`**.
...
- Trello **manual** apiKey/token not persisted (004 — disconnected fallback; **superseded for encrypted user tokens** by ADR 006 / A2–A6).
...
### 2026-06-03 — Phase DOCS-TRELLO-OAUTH-A5A — Trello OAuth A5A backend stored-token mode record (documentation only)

**Workflow:** DOCS-TRELLO-OAUTH-A5A — record **TRELLO-OAUTH-A5A** backend stored-token boards/lists/sync + reviews (documentation only)

**ADR refs:** [006 — encrypted connections](adrs/006-trello-oauth-encrypted-connections.md)
**Summary:** Documentation-only alignment in **`CURRENT_STATE.md`**, **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, **`docs/security/trello-oauth-foundation.md`**, **`docs/workflows/trello-sync-workflow.md`**, **`README.md`**, and **`SECURITY.md`**. Records **TRELLO-OAUTH-A5A** **complete** (backend only): additive stored-token mode on **`POST /api/trello/boards`**, **`/boards/:boardId/lists`**, **`/sync`** when body omits `apiKey`/`token` keys; manual mode unchanged when both keys present; partial credentials → **`VALIDATION_ERROR`**; not connected + stored mode → **`TRELLO_NOT_CONNECTED`**. **`trello-credentials.resolver.js`** + **`classifyTrelloCredentialMode`**. **Supervisor Review: Approved** (after test-wiring fix). **Security Review: PASS.** **`npm test` 442 pass; `npm run lint` pass.** **At record time:** frontend A5B not yet documented as live — **superseded** by **A5B** + **A5C** + **DOCS-CONSISTENCY-FIX**. **No** application code changes in this documentation phase.
**APIs affected:** none (docs-only record of A5A contract)
**Tests:** none (docs-only)
**Pitfalls (historical — superseded 2026-06-04 by A5B + A5C + DOCS-CONSISTENCY-FIX):** At DOCS-A5A time, frontend stored-token sync was not yet live. **Today:** **A5B** connected-account sync is live; **A5C** blocks connected + manual credentials; see **Current-state correction** at top of this file.
**Follow-up (historical):** **A5B** and **A5C** — **complete**. Do not reopen without a new approved phase gate.

### 2026-06-03 — Phase DOCS-TRELLO-OAUTH-A4-FRONTEND — Trello OAuth A4 frontend connect UI record (documentation only)

**Workflow:** DOCS-TRELLO-OAUTH-A4-FRONTEND — record **TRELLO-OAUTH-A4-FRONTEND** OAuth account connection UI + reviews (documentation only)

**ADR refs:** [006 — encrypted connections](adrs/006-trello-oauth-encrypted-connections.md)
**Summary:** Documentation-only alignment in **`CURRENT_STATE.md`**, **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, **`docs/security/trello-oauth-foundation.md`**, **`docs/workflows/trello-sync-workflow.md`**, and **`README.md`**. Records **TRELLO-OAUTH-A4-FRONTEND** **complete**: Connect/Disconnect panel on **`/trello`**; protected **`/trello/connect/callback`**; state from query, token from hash; URL sanitized before complete POST; StrictMode duplicate POST guard; frontend security boundaries (no client storage/logging of OAuth secrets). **Supervisor Review: Pass.** **Security Review: Pass.** **`npm test` 271 pass; `npm run lint` pass; `npm run build` pass.** **Stored-token sync still NOT implemented** — **`POST /api/trello/boards`**, **`/lists`**, **`/sync`** unchanged (manual apiKey/token); **A5** still required. **No** application code changes in this documentation phase.
**APIs affected:** none (docs-only)
**Tests:** none (docs-only)
**Pitfalls (historical — superseded):** At DOCS-A4-FRONTEND record time, stored-token sync was not live. **Today:** **A5A**–**A5C** complete — see **Current-state correction** at top of this file.
**Follow-up (historical):** **A5A**–**A5C** stored-token sync + connected UX + hardening — **complete** (see **Current-state correction** at top of this file).

### 2026-06-03 — Phase DOCS-TRELLO-OAUTH-A4-STATE — Trello OAuth A4-STATE signed state record (documentation only)

**Workflow:** DOCS-TRELLO-OAUTH-A4-STATE — record **TRELLO-OAUTH-A4-STATE** signed OAuth state + reviews (documentation only)

**ADR refs:** [006 — encrypted connections](adrs/006-trello-oauth-encrypted-connections.md)
**Summary:** Documentation-only alignment in **`CURRENT_STATE.md`**, **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, **`docs/security/trello-oauth-foundation.md`**, **`docs/workflows/trello-sync-workflow.md`**, **`README.md`**, and **`SECURITY.md`**. Records **TRELLO-OAUTH-A4-STATE** **complete** (backend only): signed HMAC state on authorize-url / connect/complete; CSRF protection; generic failure messages; **`TRELLO_OAUTH_STATE_SECRET`** preferred in production. **Supervisor Review: Pass.** **Security Review: Security approved with notes** — replay within 10-minute TTL accepted MVP residual risk; future hardening: single-use nonce persistence. **`npm test` 417 pass; `npm run lint` pass.** **At record time:** OAuth not yet user-facing — **superseded** by **A4-FRONTEND** + **A5A–A5C**. **No** application code changes in this documentation phase.
**APIs affected:** none (docs-only record of A4-STATE contract)
**Tests:** none (docs-only)
**Pitfalls (historical — superseded):** See **Current-state correction** at top of this file.
**Follow-up (historical):** **A4-FRONTEND**, **A5A**–**A5C** — **complete**.

### 2026-06-06 — Phase TASK-MATERIAL-LINKS-A1 — Task card material navigation links complete

**Workflow:** Phase TASK-MATERIAL-LINKS-A1 — frontend-only **`TaskCard`** material navigation

**Human gates:** implementation merged and verified; Supervisor Review passed; Security Review not required

**Summary:** Frontend-only — **`TaskCard`** on **`/courses/:id`** and **`/tasks`** renders linked study materials as React Router **`Link`** elements to **`/study-materials/:materialId`**. When `task.materialId` is set: **`Material:`** line with **`Link`** — link text is `materialLabel` when provided, otherwise fallback **View study material**. When `task.materialId` is absent: no material line. **No** backend, API, database, migration, or package dependency changes (`frontend/package.json` **`test`** script only).

**Implementation files:** `frontend/src/components/tasks/TaskCard.jsx`, `frontend/tests/unit/task-card-material-link.test.js`, `frontend/package.json`

**Reviews:** Supervisor Review — passed. Security Review — **not required** (frontend navigation only).

**Tests:** **`npm test`** passed; **`npm run lint`** passed; **`npm run build`** passed; GitHub CI on **main** green.

**Known limitations:** Global **`/tasks`** list cards often show fallback **View study material** because `materialLabel` is not resolved until edit opens **`listMaterials`** (materials not preloaded on list).

**Not implemented (intentional):** Filter tasks by `materialId`; URL-persisted task filters; material-scoped task bands on material detail. **Superseded (TASK-MATERIAL-FILTERS-A2):** frontend-only material filter shipped; backend **`materialId`** query params, URL-persisted filters, and material task bands remain deferred.

**Follow-up:** ~~Material filter~~ — frontend filter shipped in **TASK-MATERIAL-FILTERS-A2**; URL params / material task bands / backend **`materialId`** query — separate planning/approval.

### 2026-06-06 — Phase DOCS-TASK-MATERIAL-LINKS-A1 — Task card material links documentation (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-LINKS-A1 — record **TASK-MATERIAL-LINKS-A1** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, and **`CURRENT_STATE.md`**. Records **TASK-MATERIAL-LINKS-A1** **complete**: **`TaskCard`** material **`Link`** to **`/study-materials/:materialId`**; removed stale “no material links” / “Material: unavailable” list wording where superseded; backend **`materialId`** query params, URL-persisted task filters, and material task bands remain deferred (frontend material filter shipped later in **TASK-MATERIAL-FILTERS-A2**). **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark backend **`materialId`** query params, URL-persisted task filters, or material task bands as shipped. Frontend material filter shipped in **TASK-MATERIAL-FILTERS-A2** — do not regress to “filter deferred” in status docs.

**Follow-up:** URL params / material task bands / backend **`materialId`** query — separate planning/approval.

### 2026-06-06 — Phase DOCS-TASK-MATERIAL-DETAIL-A3 — Material detail related tasks documentation (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-DETAIL-A3 — record **TASK-MATERIAL-DETAIL-A3** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, and **`CURRENT_STATE.md`**. Records material detail **Related study tasks** read-only band shipped; removes stale “material detail task band deferred” wording; backend **`materialId`** query params, URL-persisted task filters, and “tasks without material” filter remain deferred. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark backend **`materialId`** query params or URL-persisted task filters as shipped. Material detail related tasks band **is** shipped (**TASK-MATERIAL-DETAIL-A3**) — do not regress to “task band deferred” in status docs.

**Follow-up:** Backend materialId query params / URL filters / “tasks without material” filter — separate planning/approval.

### 2026-06-06 — Phase TASK-MATERIAL-DETAIL-A3 — Material detail related study tasks band complete

**Workflow:** Phase TASK-MATERIAL-DETAIL-A3 — frontend-only read-only related study tasks band on material detail

**Human gates:** implementation merged and verified; Supervisor Review passed; manual smoke passed; follow-up manual smoke passed; Security Review not required

**Summary:** **`MaterialRelatedTasksSection`** on **`/study-materials/:materialId`** — between material cockpit and **Saved flashcards**; **`listCourseTasks(courseId)`** + client-side **`getMaterialLinkedTasks`**; pending/completed/total counts; up to **5** compact read-only preview rows (pending first); loading/empty/error + **Try again**/populated; **`Link`** to **`/courses/:courseId`**; refreshes after plan task import success or partial success; distinguishes saved DB tasks from AI plan JSON preview; **no** **`TaskCard`**, edit, complete, delete, or Focus. **No** backend/API/database/migration/URL query param changes.

**Implementation files:** `frontend/src/components/materials/MaterialRelatedTasksSection.jsx`, `frontend/src/pages/StudyMaterialDetail.jsx`, `frontend/src/utils/task-filters.js`, `frontend/src/styles/layout.css`, `frontend/tests/unit/material-linked-tasks.test.js`, `frontend/package.json`

**Reviews:** Supervisor Review — passed. Manual smoke — passed. Follow-up manual smoke — passed. Security Review — **not required** (frontend-only over existing authenticated task list API).

**Tests:** **`npm test`** passed; **`npm run lint`** passed; **`npm run build`** passed.

**Not implemented (intentional):** Backend **`GET /api/tasks?materialId=`** / **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters; “tasks without material” filter; task CRUD/Focus inside the band.

**Follow-up:** Backend materialId query params / URL filters / “tasks without material” filter — separate planning/approval.

### 2026-06-06 — Phase TASK-MATERIAL-FILTERS-A2 — Task material filters complete

**Workflow:** Phase TASK-MATERIAL-FILTERS-A2 — frontend-only study material task filters

**Human gates:** implementation merged and verified; Supervisor Review passed; manual smoke Pass with notes; Security Review not required

**Summary:** Frontend-only material filters on **`/courses/:id`** (always, using course-page **`materials`**) and **`/tasks`** (only when a specific course is selected, via **`listMaterials`**). **`filterTasksByMaterial`** / **`resetMaterialFilterForCourseChange`** in **`task-filters.js`**; filters already-loaded tasks by **`task.materialId`**; composes with status/course filters; unknown/stale filter id shows all tasks; global course change resets material filter. **No** backend/API/database/migration/URL query param changes.

**Implementation files:** `frontend/src/utils/task-filters.js`, `frontend/src/components/tasks/CourseTasksSection.jsx`, `frontend/src/components/tasks/GlobalTasksSection.jsx`, `frontend/tests/unit/task-filters.test.js`, `frontend/package.json`

**Reviews:** Supervisor Review — passed. Manual smoke — Pass with notes. Security Review — **not required**.

**Tests:** **`npm test`** passed; **`npm run lint`** passed; **`npm run build`** passed.

**Not implemented (intentional):** Backend **`GET /api/tasks?materialId=`** / **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters; “tasks without material” filter. Material detail related tasks band shipped in **TASK-MATERIAL-DETAIL-A3**.

**Follow-up:** Backend materialId query params / URL filters / “tasks without material” filter — separate planning/approval.

### 2026-06-06 — Phase DOCS-TASK-MATERIAL-FILTERS-A2 — Task material filters documentation (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-FILTERS-A2 — record **TASK-MATERIAL-FILTERS-A2** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, and **`CURRENT_STATE.md`**. Records frontend material filter shipped; removes stale “filter tasks by materialId deferred” wording; backend **`materialId`** query params, URL-persisted filters, and “tasks without material” filter remain deferred (material detail task band shipped later in **TASK-MATERIAL-DETAIL-A3**). **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls (historical):** At gate time, material detail task bands were still deferred — shipped in **TASK-MATERIAL-DETAIL-A3**. Do not mark backend **`materialId`** query params or URL filters as shipped.

**Follow-up:** Supervisor Review of docs gate.

### 2026-06-03 — Phase DOCS-TRELLO-OAUTH-A3 — Trello OAuth A3 backend connect routes record (documentation only)

**Workflow:** DOCS-TRELLO-OAUTH-A3 — record **TRELLO-OAUTH-A3** backend connect/authorize routes + reviews (documentation only)

**ADR refs:** [006 — encrypted connections](adrs/006-trello-oauth-encrypted-connections.md)
**Summary:** Documentation-only alignment in **`CURRENT_STATE.md`**, **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, **`docs/security/trello-oauth-foundation.md`**, **`docs/workflows/trello-sync-workflow.md`**, and **`README.md`**. Records **TRELLO-OAUTH-A3** **complete** (backend only): `GET /api/trello/connection`, `GET /api/trello/authorize-url`, `POST /api/trello/connect/complete`, `POST /api/trello/disconnect` — all **`requireAuth`**; token validated before encrypted storage; flat connected response `{ connected: true, trelloMemberId, trelloUsername, scopes, expirationPolicy, expiresAt, defaultBoardId, defaultListId, connectedAt, updatedAt }`; disconnect returns `{ connected: false }`. **Supervisor Review: Pass with notes.** **Security Review: Security approved with notes** — no blocking security issues. **`npm test` 396 pass; `npm run lint` pass.** **At record time:** backend-only — **superseded** by **A4**–**A5C** user-facing connect + sync. **No** application code changes in this documentation phase.
**APIs affected:** none (docs-only record of A3 backend routes)
**Tests:** none (docs-only)
**Pitfalls (historical — superseded):** See **Current-state correction** at top of this file.
**Follow-up (historical):** **A4-STATE**, **A4-FRONTEND**, **A5A**–**A5C** — **complete**.

### 2026-06-07 — Phase FLASHCARD-REVIEW-A3 — Flashcard review-state filter complete

**Workflow:** Phase FLASHCARD-REVIEW-A3 — frontend-only client-side review-state filter for saved DB flashcards

**Human gates:** implementation merged and verified; Supervisor Review approved with notes; Security Review not required; main CI green

**Summary:** **`filterFlashcardsByReviewState`** in **`flashcard-filters.js`** filters already-loaded saved DB flashcards by **`mastery`**: **All**, **New + Learning** (`needs_review`), **New**, **Learning**, **Known**. **`GlobalFlashcardsSection`** on **`/flashcards`** and **`DbFlashcardsSection`** on material detail derive **`displayedFlashcards`**; filter affects study deck and manage list; global page composes with course/material filters. **FLASHCARD-REVIEW-A2** Known/Unknown unchanged — same **`POST /api/flashcards/:flashcardId/review`**; local merge; **no** refetch on review; **no** auto-advance. **`GeneratedPlanSection`** plan JSON unchanged — **no** review filter; **no** review API. **Not** full SRS; **no** **`next_review_at`**; **no** SM-2/Anki; **no** dashboard due cards; **no** review history table; **no** backend **`?mastery=`**; **no** URL-persisted filters; **no** sorting by **`lastReviewedAt`**. **No** backend/API/database/migration/package changes.
...
**Tests:** **`cd frontend && npm run lint`** passed; **`npm test`** passed; **`npm run build`** passed; main CI **green**.

**Not implemented (intentional):** Full SRS; **`next_review_at`**; SM-2 / Anki; due dates; dashboard due cards; review history table; backend mastery query params; URL-persisted filters; sorting by **`lastReviewedAt`**; plan JSON review persistence (**FLASHCARD-REVIEW-A4** and related — separate phase gates).

**Follow-up:** **FLASHCARD-REVIEW-A4** — separate planning/approval only.

### 2026-06-07 — Phase DOCS-FLASHCARD-REVIEW-A3 — Flashcard review-state filter documentation (documentation only)

**Workflow:** DOCS-FLASHCARD-REVIEW-A3 — record **FLASHCARD-REVIEW-A3** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, and **`CURRENT_STATE.md`**. Records client-side review-state filter shipped on saved DB flashcards; removes stale “**FLASHCARD-REVIEW-A3** not shipped / review queue deferred” wording; **not** full SRS; **no** **`next_review_at`**; **no** backend **`?mastery=`**; **FLASHCARD-REVIEW-A4** remains deferred. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark **FLASHCARD-REVIEW-A4** as shipped. Do not claim full SRS, **`next_review_at`**, SM-2/Anki, dashboard due cards, backend mastery filtering, or URL-persisted flashcard filters exist. Client-side review-state filter **is** shipped (**FLASHCARD-REVIEW-A3**) — do not regress to “A3 deferred” in status docs.

**Follow-up:** **FLASHCARD-REVIEW-A4b** / **A4c** — separate planning/approval only.

### 2026-06-07 — Phase DOCS-FLASHCARD-REVIEW-A4c — Flashcard dashboard due-card awareness documentation (documentation only)

**Workflow:** DOCS-FLASHCARD-REVIEW-A4c — record **FLASHCARD-REVIEW-A4c** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, and **`CURRENT_STATE.md`**. Records **`dueFlashcardsCount`** on **`GET /api/dashboard/stats`** and **Due now** stat on **`/dashboard`**; due logic **`next_review_at IS NULL OR next_review_at <= now`**; user-owned aggregate only; **no** Q/A in response; link to **`/flashcards`** when count **> 0** (**no** deep link / URL filters); hero unchanged; **not** full SRS; **no** dashboard charts; **no** backend **`?due=now`**/**`?mastery=`**; **no** admin due cards. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not claim full SRS, SM-2/Anki, **`ease_factor`**, dashboard charts, backend **`?due=now`**, backend **`?mastery=`**, URL-persisted filters, admin due cards, or review history table exist. Dashboard due-card awareness **is** shipped (**FLASHCARD-REVIEW-A4c**) — do not regress to “A4c deferred” in status docs.

**Follow-up:** none required for A4c scope — future flashcard/dashboard work remains separate-gated.

### 2026-06-07 — Phase FLASHCARD-REVIEW-A4c — Flashcard dashboard due-card awareness

**Workflow:** FLASHCARD-REVIEW-A4c — dashboard due-card awareness for saved DB flashcards
**ADR refs:** 001, 003 (existing dashboard module; Zod N/A — response shape extension only)
**Summary:** **`GET /api/dashboard/stats`** adds **`dueFlashcardsCount`** — owned flashcards due when **`next_review_at IS NULL OR next_review_at <= now`**; count-only; **no** Q/A. **`DashboardStub.jsx`** **At a glance** → **Learning assets** **Due now** stat; **Ready for review** + **`/flashcards`** link when count **> 0**. **`totalFlashcards`** / **`courseStats`** / hero unchanged. **No** new endpoint; **no** URL query params.
**APIs affected:** **`GET /api/dashboard/stats`** response field **`dueFlashcardsCount`**
**Tests:** `dashboard.test.js`, `mockSupabaseDashboard.js`, `dashboard.service.test.js`, `dashboard-format.test.js`; backend + frontend lint/test/build passed; main CI green
**Pitfalls:** Do not claim dashboard charts, backend **`?due=now`**, deep link to Due now filter, admin due cards, SM-2/Anki, or hero recommendation changes exist.
**Follow-up:** docs gate **DOCS-FLASHCARD-REVIEW-A4c**

### 2026-06-07 — Phase DOCS-FLASHCARD-REVIEW-A4b — Flashcard Due now filter documentation (documentation only)

**Workflow:** DOCS-FLASHCARD-REVIEW-A4b — record **FLASHCARD-REVIEW-A4b** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, and **`CURRENT_STATE.md`**. Records frontend **Due now** filter shipped on saved DB flashcards; **`isFlashcardDueNow`** + **`due_now`** in **`filterFlashcardsByReviewState`**; client-side over already-loaded cards by **`nextReviewAt`** from **A4a**; surfaces global **`/flashcards`** and material detail; **A2**/**A3** unchanged; plan JSON unchanged; **not** full SRS; **no** dashboard due cards; **FLASHCARD-REVIEW-A4c** remains deferred. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark **FLASHCARD-REVIEW-A4c** as shipped. Do not claim full SRS, SM-2/Anki, **`ease_factor`**, dashboard due cards, backend **`?due=now`**, backend **`?mastery=`**, URL-persisted filters, or review history table exist. Due now filter **is** shipped (**FLASHCARD-REVIEW-A4b**) — do not regress to “A4b deferred” in status docs.

**Follow-up:** **FLASHCARD-REVIEW-A4c** — separate planning/approval only.

### 2026-06-07 — Phase FLASHCARD-REVIEW-A4b — Flashcard Due now filter (frontend-only)

**Workflow:** FLASHCARD-REVIEW-A4b — frontend Due now filter on saved DB flashcards
**ADR refs:** none (frontend-only; consumes A4a **`nextReviewAt`** in already-loaded data)
**Summary:** **`isFlashcardDueNow`** + **`due_now`** case in **`filterFlashcardsByReviewState`**. **Due now** option after **All** on **`GlobalFlashcardsSection`** and **`DbFlashcardsSection`**. Filters study deck and manage list. Due-only empty copy when cards exist but none are due. **A2** Known/Unknown and **A3** mastery filters unchanged. Plan JSON unchanged. **No** backend **`?due=now`**; **no** URL-persisted filters.
**APIs affected:** none (client-side filter only; existing **`GET /api/flashcards`** responses already include **`nextReviewAt`** from **A4a**)
**Tests:** `flashcard-filters.test.js`, `flashcard-study-review.test.js`; frontend lint + test + build passed; main CI green; Supervisor Review **approved**; Security Review **not required**
**Pitfalls:** Do not claim dashboard due cards, backend due/mastery query params, SM-2/Anki, or plan JSON review exist. Marking **Known** while **Due now** active may remove card from filtered view after local merge — expected filter behavior, not auto-advance.
**Follow-up:** **FLASHCARD-REVIEW-A4c** (dashboard due cards) — separate phase gate.

### 2026-06-07 — Phase FLASHCARD-REVIEW-A4a — Flashcard SRS-lite scheduling backend (backend/database)

**Workflow:** FLASHCARD-REVIEW-A4a — SRS-lite scheduling foundation on saved DB flashcards
**ADR refs:** 001, 003
**Summary:** Migration **014** adds **`next_review_at`** and **`review_interval_days`** on **`public.flashcards`**. **`POST /api/flashcards/:flashcardId/review`** computes scheduling server-side via **`computeNextReviewState`** — whole-day intervals, **90**-day cap. Review body remains strict **`{ outcome: "known" | "unknown" }`**; scheduling not writable via PATCH/create. **SRS-lite foundation only** — not SM-2/Anki; no frontend; plan JSON unchanged.
**APIs affected:** `POST /api/flashcards/:flashcardId/review` (scheduling fields in response); list/create/PATCH flashcard responses include **`nextReviewAt`**, **`reviewIntervalDays`**
**Tests:** `flashcard-scheduling.test.js`, `flashcards.service.test.js`, `flashcards.test.js` (integration); backend lint + test passed; main CI green
**Pitfalls:** Migration **014** required before backend deploy against real Supabase. Do not claim frontend Due now filter, dashboard due cards, **`?due=now`**, **`?mastery=`**, SM-2/Anki, **`ease_factor`**, or review history table exist. A4a does not change plan JSON flashcards.
**Follow-up:** **FLASHCARD-REVIEW-A4c** (dashboard due cards) — separate phase gates.

### 2026-06-07 — Phase DOCS-FLASHCARD-REVIEW-A4a — Flashcard SRS-lite scheduling documentation (documentation only)

**Workflow:** DOCS-FLASHCARD-REVIEW-A4a — record **FLASHCARD-REVIEW-A4a** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, and **`CURRENT_STATE.md`**. Records SRS-lite scheduling backend shipped; migration **014** applied; **`next_review_at`** and **`review_interval_days`**; server-side scheduling on **`POST /review`**; **90**-day cap; **not** full SRS; **no** frontend Due now; **FLASHCARD-REVIEW-A4b**/**A4c** remain deferred. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark **FLASHCARD-REVIEW-A4b** or **A4c** as shipped. Do not claim full SRS, SM-2/Anki, **`ease_factor`**, frontend Due now filter, dashboard due cards, backend **`?due=now`**, or review history table exist. SRS-lite scheduling **is** shipped (**FLASHCARD-REVIEW-A4a**) — do not regress to “no **`next_review_at`**” in current-state docs.

**Follow-up:** **FLASHCARD-REVIEW-A4b** / **A4c** — separate planning/approval only.

### 2026-06-07 — Phase DOCS-TASK-MATERIAL-NAV-B1-RECONCILE — Material detail task deep link documentation (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-NAV-B1-RECONCILE — register **TASK-MATERIAL-NAV-B1** shipped behavior on current **main** docs (documentation only)

**Summary:** Documentation-only reconciliation in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`CURRENT_STATE.md`**, and **`PRD.md`**. Registers **TASK-MATERIAL-NAV-B1** as a completed phase — material detail primary **View linked tasks** → **`/tasks?courseId=<uuid>&materialId=<uuid>`** via **`buildTasksPageMaterialLink`**; secondary **View tasks on course** → **`/courses/:courseId`**; updates **TASK-MATERIAL-DETAIL-A3** navigation wording; updates **`/study-materials/:materialId`** and **`/tasks`** route descriptions. **Did not** merge or cherry-pick stale **`docs/task-material-nav-b1`** branch (**0807c47**). **TASK-MATERIAL-FILTERS-E1** full URL-persisted **`/tasks`** behavior and **TASK-MATERIAL-UNLINKED-A1** **`materialId=none`** remain documented as current state — **not** reverted to initial-load-only wording. Backend **`?materialId=`** and **`/courses/:id`** URL filters remain **not shipped**. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark backend **`GET /api/tasks?materialId=`** or **`/courses/:id`** URL filters as shipped. **TASK-MATERIAL-NAV-B1** deep link **is** shipped — do not regress to “material detail only links to **`/courses/:courseId`**” in current-state docs. Current **`/tasks`** URL behavior is **full bidirectional sync (E1)** — do not restore stale “initial-load only; manual changes do not update URL” as current behavior.

**Follow-up:** None required for NAV-B1 docs reconcile; backend **`materialId`** query params and **`/courses/:id`** URL filters remain separate phase gates.

### 2026-06-07 — Phase DOCS-TASK-MATERIAL-FILTERS-E1 — Task URL-persisted filters documentation (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-FILTERS-E1 — record **TASK-MATERIAL-FILTERS-E1** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`CURRENT_STATE.md`**, and **`PRD.md`**. Records **`/tasks`** URL-persisted filters shipped (**`courseId`**, **`materialId`**, **`status`**); manual changes update URL; Back/Forward restores state; defaults omitted; **`materialId`** client-side only; **no** backend **`?materialId=`**; **no** **`/courses/:id`** URL filters; **no** dashboard/flashcards URL filter changes; B1 deep link unchanged; Supervisor Review **approved with notes**; Security Review **not required**; main CI **green**. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Backend **`GET /api/tasks?materialId=`** and **`GET /api/courses/:id/tasks?materialId=`** **are** shipped (**TASK-MATERIAL-BACKEND-A1**). URL-persisted task filters on **`/tasks`** **are** shipped (**TASK-MATERIAL-FILTERS-E1** + **UNLINKED-A1** + deadline params) — do not regress to “URL-persisted task filters deferred” in current-state docs. URL-persisted task filters on **`/courses/:id`** **are** shipped (**TASK-COURSE-FILTERS-E1** + **FIX1**) — do not regress to “course filters in-memory only” in current-state docs. Historical journal entries that said course filters were in-memory were accurate at their ship dates.

**Follow-up:** “tasks without material” on global **All courses** (cross-course unlinked filter); full SM-2; backend due/mastery query params — separate phase gates.

### 2026-06-07 — Phase FLASHCARD-REVIEW-A5b — Flashcard review auto-advance (frontend-only)

**Workflow:** FLASHCARD-REVIEW-A5b — auto-advance after successful Known/Unknown on saved DB flashcards
**ADR refs:** none (frontend-only; consumes existing review API and local deck state)
**Summary:** After successful **Known** / **Unknown**, **`FlashcardStudy`** auto-advances and resets reveal on **saved DB flashcards** only (**`/flashcards`** + material detail). **`computeIndexAfterSuccessfulReview`** handles card-stays vs card-leaves-filtered-deck. Pending advance + post-merge **`useEffect`** avoids stale-deck race. Parent handlers return **`true`**/**`false`**. Plan JSON **`GeneratedPlanSection`** unchanged. **No** backend/URL/DB changes. **A5a** URL filters unchanged.
**APIs affected:** none (frontend UX only; existing **`POST /api/flashcards/:flashcardId/review`** unchanged)
**Tests:** `flashcard-study.test.js`, `flashcard-study-review.test.js`; frontend lint + test (**423** pass) + build passed; main CI green; Supervisor Review **approved with notes**; Security Review **not required**
**Pitfalls:** Do not claim plan JSON review, full SM-2/Anki, review history table, backend **`?due=now`**/**`?mastery=`**, URL behavior changes, or dashboard hero changes exist. Marking **Known** while **Due now** active may remove card from filtered deck — **A5b** index logic uses post-filter deck; expected behavior.
**Follow-up:** Full SM-2; backend due/mastery query params — separate phase gates.

### 2026-06-07 — Phase DOCS-FLASHCARD-REVIEW-A5b — Flashcard review auto-advance documentation (documentation only)

**Workflow:** DOCS-FLASHCARD-REVIEW-A5b — record **FLASHCARD-REVIEW-A5b** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`CURRENT_STATE.md`**, and **`PRD.md`**. Records auto-advance after successful Known/Unknown on saved DB flashcards; both **`/flashcards`** and material detail surfaces; failure/no-advance path; race-safe pending advance; plan JSON unchanged; **no** backend/URL/DB; **A5a** unchanged; tests and CI status recorded. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark auto-advance as deferred in current-state docs. Do not claim plan JSON review persistence, full SM-2, review history table, backend query params, URL changes, or dashboard hero changes exist.

**Follow-up:** Full SM-2; backend due/mastery query params — separate phase gates.

### 2026-06-07 — Phase DOCS-FLASHCARD-REVIEW-A5a — Flashcard URL-persisted filters documentation (documentation only)

**Workflow:** DOCS-FLASHCARD-REVIEW-A5a — record **FLASHCARD-REVIEW-A5a** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`CURRENT_STATE.md`**, and **`PRD.md`**. Records **`/flashcards`** URL-persisted filters shipped (**`courseId`**, **`materialId`**, **`reviewState`**); manual changes update URL; Back/Forward restores state; defaults omitted; **`reviewState=all`** not emitted; **`materialId`** never without **`courseId`**; invalid/stale params canonicalized; **`reviewState`** client-side only; **no** backend **`?due=now`** / **`?mastery=`**; dashboard **Ready for review** deep link → **`/flashcards?reviewState=due_now`**; **`DbFlashcardsSection`** on material detail **not** URL-persisted; plan JSON **unchanged**; hero recommendation **unchanged**; tests **`flashcard-nav-query.test.js`**; Supervisor Review **approved with notes**; Security Review **not required**; main CI **green**. **No** application code changes in this documentation phase.
...
### 2026-06-18 — Phase DOCS-TASK-DUE-FILTERS-A1 — Task deadline filters documentation (documentation only)

**Workflow:** DOCS-TASK-DUE-FILTERS-A1 — record **TASK-DUE-FILTERS-A1** shipped behavior (documentation only)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`CURRENT_STATE.md`**, **`AGENT_MEMORY.md`**, **`docs/database/015-study-tasks-due-date.md`**, and **`README.md`**. Records backend-filtered **`deadline=overdue|due_today`** on **`GET /api/tasks`** and **`GET /api/courses/:courseId/tasks`**, validation rules, browser-local **`referenceDate`** vs UTC fallback, global URL **`deadline`**, in-memory course deadline filters, dashboard links to **`/tasks?status=pending&deadline=…`**, preserved **`created_at DESC`** ordering, and deferred scope (no due-date sorting, no upcoming/this-week, etc.). **No** application code, tests, migrations, packages, or CI changes in this documentation phase.

**APIs affected:** none (docs-only — describes existing task list query params)
...
### 2026-06-21 — Phase DOCS-TASK-MATERIAL-BACKEND-A1 — Task material membership backend filter documentation (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-BACKEND-A1 — record **TASK-MATERIAL-BACKEND-A1** shipped behavior (documentation only)

**ADR compliance:** **001** (modular monolith — tasks module only); **003** (Zod validation on **`materialId`** query param)

**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`CURRENT_STATE.md`**, **`AGENT_MEMORY.md`**, **`docs/database/015-study-tasks-due-date.md`**, and **`README.md`**. Records backend list **material membership** on **`GET /api/tasks`** and **`GET /api/courses/:courseId/tasks`** (**`materialId`** omitted / UUID / **`none`**); ownership and neutral **`404`** contract; composition with **`courseId`**, **`status`**, **`deadline`**, **`referenceDate`**; unchanged **TASK-DUE-SORT-A1** ordering; frontend sends **`materialId`** and renders API results directly; supersession notes on historical frontend-only **TASK-MATERIAL-*** sections. **No** application code, tests, migrations, packages, or CI changes in this documentation phase. **Supervisor Review: pending** — **not** closed.

**APIs affected:** none (docs-only — describes existing task list routes)

**Tests:** none (docs-only)

**Pitfalls:** Do not claim **DOCS-TASK-MATERIAL-BACKEND-A1** is reviewed or closed before its documentation Supervisor Review. Do not rewrite historical frontend-only phase bodies — use supersession notes. Do not claim a migration or index was added. Do not cite an exact integration scenario count for material-filter tests. Do not claim the implementation phase is committed/merged/CI-green unless human Git closure completed.

**Follow-up:** Documentation Supervisor Review; then Git closure for implementation + docs (manual commit/push → CI green).

### 2026-06-21 — Phase TASK-MATERIAL-BACKEND-A1 — Task material membership backend filter

**Workflow:** TASK-MATERIAL-BACKEND-A1 — backend + frontend task list material membership

**ADR compliance:** **001** (tasks module in modular monolith); **003** (Zod **`materialId`** query validation; ownership checks before filter)

**Summary:** **`GET /api/tasks`** and **`GET /api/courses/:courseId/tasks`** accept optional **`materialId`**: omitted = no material predicate; UUID = owned-material filter (**material_id = UUID**); **`none`** = **`material_id IS NULL`** (no material ownership lookup). Global UUID proves material→course→user; course-scoped UUID must belong to route course. Missing/wrong-owner/mismatch → **`404 Study material not found`**; valid owned material with zero matches → **`200 []`**; invalid (**`all`**, bad UUID, duplicate param) → **`400 VALIDATION_ERROR`**. Composes with existing **`courseId`**, **`status`**, **`deadline`**, **`referenceDate`**; **TASK-DUE-SORT-A1** ordering unchanged. Frontend **`GlobalTasksSection`**, **`CourseTasksSection`**, **`MaterialRelatedTasksSection`** send **`materialId`** on list fetch; removed **`filterTasksByMaterial`** / **`getMaterialLinkedTasks`**. Global **`/tasks`** URL persistence unchanged; course filters in-memory only. **No** migration/index.

**APIs affected:** **`GET /api/tasks`**, **`GET /api/courses/:courseId/tasks`** — optional **`materialId`** query param

**Tests:** backend **`npm run lint`** passed; backend **`npm test`** **658/658**; frontend **`npm run lint`** passed; frontend **`npm test`** **520/520**; frontend **`npm run build`** passed; **`git diff --check`**: clean
...
**Pitfalls:** Do not claim cross-course unlinked filter when **All courses** selected on global **`/tasks`**. Do not claim **`/courses/:id`** URL-persisted material filters are missing (**TASK-COURSE-FILTERS-E1** shipped). Do not claim dashboard material-linked pending counts are missing (**TASK-MATERIAL-DASHBOARD-A1** shipped). Do not canonicalize URL-persisted **`materialId`** away because a pre-hydration material list is empty — wait for conclusive successful hydration (**TASK-MATERIAL-DASHBOARD-A1-FIX2**). Do not inject test auth via mutable overrides inside production **`AuthContext`** — use Provider-scoped test infrastructure (**TASK-MATERIAL-DASHBOARD-A1-FIX1**). Do not claim migration **015** changed.

**Follow-up:** **DOCS-TASK-MATERIAL-BACKEND-A1** (Supervisor Review pending); Git closure after docs review.

### 2026-06-21 — Phase DOCS-TASK-MATERIAL-BACKEND-A1 — Documentation Supervisor Review closure (documentation only)

**Workflow:** DOCS-TASK-MATERIAL-BACKEND-A1 — documentation Supervisor Review closure (status-only; no content rewrite)

**Documentation Supervisor Review:** **APPROVE WITH NOTES** — no Blocking or Important findings.

**Non-blocking notes:** N-1 and N-2 accepted as non-blocking editorial notes — **not** implemented in this closure update.

**Security Review:** **not required** (docs-only alignment of already **SECURITY APPROVED** **TASK-MATERIAL-BACKEND-A1** implementation).

**Next gate:** human-controlled **Git closure** (manual commit/push → CI green) — **not** started; no commit, push, merge, PR, or CI green claimed.

**Follow-up:** Git closure; further task phases require explicit approval.
