...
**Summary:** Documentation-only alignment in **`CURRENT_STATE.md`**, **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, **`docs/security/trello-oauth-foundation.md`**, **`docs/workflows/trello-sync-workflow.md`**, **`README.md`**, and **`SECURITY.md`**. Records **TRELLO-OAUTH-A5A** **complete** (backend only): additive stored-token mode on **`POST /api/trello/boards`**, **`/boards/:boardId/lists`**, **`/sync`** when body omits `apiKey`/`token` keys; manual mode unchanged when both keys present; partial credentials → **`VALIDATION_ERROR`**; not connected + stored mode → **`TRELLO_NOT_CONNECTED`**. **`trello-credentials.resolver.js`** + **`classifyTrelloCredentialMode`**. **Supervisor Review: Approved** (after test-wiring fix). **Security Review: PASS.** **`npm test` 442 pass; `npm run lint` pass.** **Frontend A5B NOT implemented** — **`/trello`** UI still manual apiKey/token (**only active UI sync path**). **No** application code changes in this documentation phase.
**APIs affected:** none (docs-only record of A5A contract)
**Tests:** none (docs-only)
**Pitfalls:** Do not document frontend stored-token sync as live; do not claim **`/trello`** omits manual credentials until **A5B**; backend A5A ≠ user-facing connected sync UX; connected manual `{ apiKey, token }` is **blocked by A5C** — manual fallback requires disconnect first
**Follow-up:** **A5B** frontend stored-token sync UX (separate phase gate + Security Review); optional **A5C** manual-sync hardening; stop before commit per phase gate
...
**APIs affected:** none (docs-only)
**Tests:** none (docs-only)
**Pitfalls:** Do not document stored-token sync as live; do not remove manual sync credentials from docs until **A5**; do not claim full OAuth replaces manual sync today; account connect ≠ sync automation
**Follow-up:** **A5** stored-token boards/lists/sync (separate phase gate + Security Review); stop before commit per phase gate
...
**ADR refs:** [006 — encrypted connections](adrs/006-trello-oauth-encrypted-connections.md)
**Summary:** Documentation-only alignment in **`CURRENT_STATE.md`**, **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, **`docs/security/trello-oauth-foundation.md`**, **`docs/workflows/trello-sync-workflow.md`**, **`README.md`**, and **`SECURITY.md`**. Records **TRELLO-OAUTH-A4-STATE** **complete** (backend only): signed HMAC state on authorize-url / connect/complete; CSRF protection; generic failure messages; **`TRELLO_OAUTH_STATE_SECRET`** preferred in production. **Supervisor Review: Pass.** **Security Review: Security approved with notes** — replay within 10-minute TTL accepted MVP residual risk; future hardening: single-use nonce persistence. **`npm test` 417 pass; `npm run lint` pass.** **OAuth still not live for users.** **No** application code changes in this documentation phase.
**APIs affected:** none (docs-only record of A4-STATE contract)
**Tests:** none (docs-only)
**Pitfalls:** Do not document OAuth as live for users; do not treat A4-STATE as full OAuth UX completion; frontend callback still belongs to **A4 frontend**; do not assume boards/lists/sync use stored token until **A5**
**Follow-up:** **A4 frontend** callback/connect UX (separate phase gate); optional **A5** stored-token refactor; stop before commit per phase gate
