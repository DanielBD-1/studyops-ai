...
**Branch / phase (2026-06-07):** post-**DOCS-FLASHCARD-REVIEW-A5b** — **FLASHCARD-REVIEW-A5b** auto-advance after successful Known/Unknown review live on saved DB flashcards (global **`/flashcards`** + material detail **`DbFlashcardsSection`**); prior **DOCS-FLASHCARD-REVIEW-A5a** — **FLASHCARD-REVIEW-A5a** URL-persisted filters live on **`/flashcards`** (**`courseId`**, **`materialId`**, **`reviewState`**; **`reviewState`** client-side only; Back/Forward; dashboard deep link **`/flashcards?reviewState=due_now`**); prior **DOCS-TASK-MATERIAL-NAV-B1-RECONCILE** — **TASK-MATERIAL-NAV-B1** material detail **View linked tasks** deep link documented on current **main** docs; prior **TASK-MATERIAL-UNLINKED-A1** — **Tasks without material** filter live on **`/courses/:id`** (in-memory) and **`/tasks`** (in-memory + **`materialId=none`** URL with valid **`courseId`**); prior **DOCS-TASK-MATERIAL-UNLINKED-A1** documentation gate; prior **TASK-MATERIAL-FILTERS-E1** — **`/tasks`** URL-persisted filters live (**`courseId`**, **`materialId`**, **`status`**; **`materialId`** client-side only; Back/Forward restores state); prior **DOCS-TASK-MATERIAL-FILTERS-E1** documentation gate; prior **FLASHCARD-REVIEW-A4c** — dashboard **`dueFlashcardsCount`** on **`GET /api/dashboard/stats`** + **Due now** stat on **`/dashboard`** (aggregate only; **Ready for review** deep link to **`/flashcards?reviewState=due_now`** via **A5a**); prior **DOCS-FLASHCARD-REVIEW-A4c** documentation gate; prior **FLASHCARD-REVIEW-A4b** — frontend **Due now** filter on saved DB flashcards live (global **`/flashcards`** + material detail; client-side by **`nextReviewAt`** from **A4a**); prior **DOCS-FLASHCARD-REVIEW-A4b** documentation gate; prior **FLASHCARD-REVIEW-A4a** — SRS-lite scheduling backend (migration **014** applied; **`next_review_at`**, **`review_interval_days`**); prior **DOCS-FLASHCARD-REVIEW-A4a** documentation gate; prior **FLASHCARD-REVIEW-A3** — frontend review-state filter on saved DB flashcards (global **`/flashcards`** + material detail); prior **DOCS-FLASHCARD-REVIEW-A3** documentation gate; prior **FLASHCARD-REVIEW-A2** — Known/Unknown review on saved DB flashcards; prior **DOCS-FLASHCARD-REVIEW-A2**; prior **FLASHCARD-REVIEW-A1** — aggregate flashcard review state backend live (**`POST /api/flashcards/:flashcardId/review`**, migration **013** applied); prior **DOCS-FLASHCARD-REVIEW-A1** documentation gate; prior **TASK-MATERIAL-DETAIL-A3** — material detail **Related study tasks** read-only band live on **`/study-materials/:materialId`**; prior **DOCS-TASK-MATERIAL-DETAIL-A3** documentation gate; prior **TASK-MATERIAL-FILTERS-A2** — frontend study material task filters on **`/courses/:id`** (always) and **`/tasks`** (when a specific course is selected); prior **DOCS-TASK-MATERIAL-FILTERS-A2**; prior **TASK-MATERIAL-LINKS-A1** — **`TaskCard`** material navigation links; prior **POST-MVP-LANGUAGE-A1** — post-MVP framing; **Trello A2–A6 live** (see **Current-state correction**). Prior **DOCS-CONSISTENCY-FIX** — removed stale Trello OAuth/A5 deferred language from status docs; prior **DOCS-TRELLO-OAUTH-A5C** — **TRELLO-OAUTH-A5C** backend manual-credential hardening documented (**Supervisor Pass with notes**; **Security Security approved with notes**; **`npm test` 448**; connected manual `{ apiKey, token }` → **`TRELLO_MANUAL_CREDENTIALS_NOT_ALLOWED`** / **400**; manual fallback when disconnected only; **A5B** connected-account sync remains primary **`/trello` UX**); prior **DOCS-TRELLO-OAUTH-A5B** — **A5B** frontend connected-account sync; prior **DOCS-TRELLO-OAUTH-A5A** — **A5A** backend stored-token boards/lists/sync; prior **DOCS-TRELLO-OAUTH-A4-FRONTEND** — **A4-FRONTEND** OAuth connect UI; prior **DOCS-TRELLO-OAUTH-A4-STATE** — **A4-STATE** signed state; prior **DOCS-TRELLO-OAUTH-A3** — **A3** backend connect routes; prior **DOCS-BX-I10A6-QA** — **BX-I10A6-QA** Trello failed-row live verification **complete** (**Pass with notes**, review only, **no** repo file changes); prior **DOCS-BX-I10A5** / **BX-I10A5** targeted glass/elevation pass **complete** (commit **`38aa561`**); **BX-I10A5-SMOKE** **Pass with notes**; prior **BX-I10A4** (commit **`7e5e61f`**); prior **BX-I10A3**–**BX-I10A1**; prior **BX-I9C** + **BX-I9C-Auth** final visual QA **complete** (**Pass with notes**); prior **BX-I9B2d** (commit **`ae5cfc8`**); **BX-I9B2** color token sub-sequence **complete**; **BX-I7** desktop layout sequence **complete**; Stitch visual style guide **v2.2** at **`docs/design/studyops_ai_visual_style_guide_v2_2.md`** (**reference artifact only**). Deferred optional follow-ups (**not automatic**): optional material editor/library elevation alignment; optional global shadow/token cleanup only if explicitly approved; **ROADMAP-A1** background automations + AWS architecture planning; optional material detail course accent JSX if explicitly approved. Next optional polish is **separate-gated** — **not automatic** (**BX-I10A6-QA complete** — **no** fix phase required). **BX-I7E4** Admin desktop stats panels **complete** (commit **`467ccd9`**); **BX-I7E3 Tier A** Trello desktop setup panels **complete** (commit **`cba6dde`**); **BX-I7E2** Flashcards desktop panels **complete** (commit **`b18304c`**); **BX-I7E1** Tasks desktop panels **complete** (commit **`d0db43e`**); **BX-I7D Tier 1** courses/course detail desktop shelf expansion **complete** (commit **`52c68ab`**); **BX-I7C** dashboard desktop grid **complete** (commit **`583922d`**); **BX-I7B** global desktop cockpit shell widening **complete** (commit **`00d3255`**); **B4-F3A** / **B4-F3B** / **B4-F3C** all **complete**; **BX-I7A** audit **complete** (documentation only); optional **BX-I7D Tier 2** and **BX-I7E3 Tier B** **proposed**, **not started**; next implementation phase **not automatic**. Phase **DOCS-WEB-ONLY** complete — web-only product scope clarified in governance and status docs. Docs phases **DOCS-A3** and **BX-I1** complete; **BX-I2** implementation complete (commit **`03ee9df`**); **BX-I3** implementation complete (commit **`bdd6f2a`**); **BX-I4** implementation complete (commit **`ff65e21`**); **BX-I5** implementation complete (commit **`c2288d4`**); **BX-I6B** implementation complete (commit **`cceb4e0`**); **BX-I6C** implementation complete (commit **`6a1e6ad`**); **BX-I6D** implementation complete (commit **`9252ba9`**); **B4-A** implementation complete (commit **`4ae80ee`**); **B4-B** implementation complete (commit **`f91415d`**); **B4-C** implementation complete (commit **`cf50729`**); **B4-D** implementation complete (commit **`905ee4d`**); **B4-E** implementation complete (commit **`7f4bf6b`**); **B4-F1** implementation complete (commit **`ea8a899`**); **B4-F2** implementation complete (commit **`ee50b8e`**); **B4-F3A** implementation complete (commit **`596e869`**); **B4-F3B** implementation complete (commit **`ee5357f`**); **B4-F3C1** implementation complete (commit **`d0393d7`**); **B4-F3C2** implementation complete (commit **`d1a3c69`**); **B4-F3C3** implementation complete (commit **`ab28307`**). **`DESIGN.md` v2.3** (commit **`6041932`**) documents Stitch-selected visual direction. Functional MVP through **6A-3** + **10B** + **11A-3** + **12A-1** + **B1**–**B3** + **BX-I2** + **BX-I3** + **BX-I4** + **BX-I5** + **BX-I6B** + **BX-I6C** + **BX-I6D** + **B4-A** + **B4-B** + **B4-C** + **B4-D** + **B4-E** + **B4-F1** + **B4-F2** + **B4-F3A** + **B4-F3B** + **B4-F3C1** + **B4-F3C2** + **B4-F3C3** + **BX-I8B** — authoritative detail in **`docs/IMPLEMENTATION_STATUS.md`**; phase gates in **`docs/CURRENT_STATE.md`**.
