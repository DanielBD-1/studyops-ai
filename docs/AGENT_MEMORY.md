...
**Pitfalls:** Do not add create UI on `/tasks`. Do not pass arbitrary `courseId`/`status` strings to `listAllTasks`. Do not fetch materials for every course on page load.
**Follow-up:** Trello, focus, flashcards table/UI, dashboard analytics, plan → `study_tasks` import — separate approval.
