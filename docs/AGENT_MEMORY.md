...
**Not implemented (intentional):** Filter tasks by `materialId`; URL-persisted task filters; material-scoped task bands on material detail.

**Follow-up:** Material filter / URL params / material task bands — separate planning/approval.
...
**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, and **`CURRENT_STATE.md`**. Records **TASK-MATERIAL-LINKS-A1** **complete**: **`TaskCard`** material **`Link`** to **`/study-materials/:materialId`**; removed stale “no material links” / “Material: unavailable” list wording where superseded; filter/URL-param/material-band work remains deferred. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark material **filtering**, URL-persisted task filters, or material task bands as shipped. Global **`/tasks`** may still show **View study material** fallback when title is unresolved — expected, not a regression.
