...
**TASK-MATERIAL-NAV-B1 (2026-06-07):** Phase **TASK-MATERIAL-NAV-B1** — frontend-only material detail → **`/tasks`** deep link and initial-load filter initialization. **`MaterialRelatedTasksSection`** adds primary **View linked tasks** **`Link`** to **`/tasks?courseId=<courseId>&materialId=<materialId>`** via **`buildTasksPageMaterialLink`**; secondary **View tasks on course** → **`/courses/:courseId`** unchanged. **`GlobalTasksSection`** reads **`courseId`** / **`materialId`** from URL on **initial load only** (one-time init refs); valid owned **`courseId`** sets course filter; after **`listMaterials`** for that course, valid material sets material filter; invalid/missing/unknown params silent fallback; **`statusFilter`** stays **all**; manual filter changes **do not** update the URL. Helpers: **`parseTasksPageSearchParams`**, **`resolveInitialTaskFilters`**, **`buildTasksPageMaterialLink`** in **`task-nav-query.js`**. Material filtering remains **client-side** (**TASK-MATERIAL-FILTERS-A2**); **no** backend **`?materialId=`** on **`listAllTasks`**; **no** **`status`** query param; **not** full URL-persisted filters. **`focusReturnTo`** preserves current **`/tasks`** search string when straightforward. **No** backend/database/migration/package changes. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only over existing authenticated APIs). **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. Backend **`materialId`** query params and **URL-persisted** task filters remain **deferred**. Docs: **DOCS-TASK-MATERIAL-NAV-B1**.
