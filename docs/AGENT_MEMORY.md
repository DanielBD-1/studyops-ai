...
**Not implemented (intentional):** Backend **`GET /api/tasks?materialId=`** / **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters; material detail task band; “tasks without material” filter.

**Follow-up:** Backend materialId query params / URL filters / material task bands — separate planning/approval.
...
**Summary:** Documentation-only alignment in **`IMPLEMENTATION_STATUS.md`**, **`AGENT_MEMORY.md`**, **`PRD.md`**, and **`CURRENT_STATE.md`**. Records frontend material filter shipped; removes stale “filter tasks by materialId deferred” wording; backend **`materialId`** query params, URL-persisted filters, material task bands, and “tasks without material” filter remain deferred. **No** application code changes in this documentation phase.

**APIs affected:** none (docs-only)

**Tests:** none (docs-only)

**Pitfalls:** Do not mark backend **`materialId`** query params, URL filters, or material detail task bands as shipped.
