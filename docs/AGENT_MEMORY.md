...
**Trello OAuth A2 (2026-06-03):** Phase **TRELLO-OAUTH-A2-DB** — encrypted `trello_connections` schema (migration **012**), `trello-token-crypto.js`, `trello-connection.repository.js`, unit tests, optional `TRELLO_API_KEY` / `TRELLO_TOKEN_ENCRYPTION_KEY` placeholders in `backend/.env.example`. **Supervisor + Security Review passed** for A2 implementation. **OAuth is not live:** no connect/callback routes, no Connect UI, manual apiKey/token on `/trello` unchanged. Docs: ADR **006**, `docs/security/trello-oauth-foundation.md`, **TRELLO-OAUTH-A2-DOCS-ALIGNMENT**. **Pitfall:** Do not assume `trello_connections` rows exist or are used — repository is for future connect phases only until routes ship.
