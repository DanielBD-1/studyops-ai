...
**Pitfalls:** Do not mark backend **`GET /api/tasks?materialId=`** or **`/courses/:id`** URL filters as shipped. URL-persisted task filters on **`/tasks`** **are** shipped (**TASK-MATERIAL-FILTERS-E1**) — do not regress to “URL-persisted task filters deferred” in current-state docs. **`/courses/:id`** task filters remain in-memory only.

**Follow-up:** Backend **`materialId`** query params; **`/courses/:id`** URL filters; “tasks without material” filter — separate phase gates.
