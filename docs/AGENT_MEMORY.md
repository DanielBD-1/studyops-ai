...
**Trello OAuth A5A (2026-06-03):** Phase **TRELLO-OAUTH-A5A** — backend-only additive stored-token mode on **`POST /api/trello/boards`**, **`/boards/:boardId/lists`**, **`/sync`**. **`classifyTrelloCredentialMode`**: no `apiKey`/`token` keys → stored mode (server **`TRELLO_API_KEY`** + **`decryptTokenForUser(req.user.id)`**); both keys → manual (unchanged, even when connected); one key → **`VALIDATION_ERROR`** / 400; stored mode, not connected → **`TRELLO_NOT_CONNECTED`** / 400. New **`trello-credentials.resolver.js`**; **`trello.controller.js`** delegates credential resolution. **Supervisor Review: Approved** (after test-wiring fix). **Security Review: PASS.** **`npm test` 442 pass; `npm run lint` pass.** **Frontend A5B NOT implemented:** **`/trello`** UI still sends manual apiKey/token — **only active UI sync path**. **A5B** required for connected-account sync UX. **A5C** deferred: wrong-account manual sync hardening while connected. Docs: **DOCS-TRELLO-OAUTH-A5A**.
