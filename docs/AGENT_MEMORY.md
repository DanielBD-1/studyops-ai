...
**FLASHCARD-STABILIZATION-A1 (2026-06-07):** Phase **FLASHCARD-STABILIZATION-A1** — frontend-only dashboard stats invalidation after saved DB flashcard review. **`DbFlashcardsSection`** and **`GlobalFlashcardsSection`** **`handleReviewOutcome`**: after successful **`reviewFlashcard`** and local merge (**`onFlashcardUpdated?.(data.flashcard)`** / **`setFlashcards`**), call **`refreshStats()`** — consistent with existing create/delete invalidation; **`refreshStats()`** not called before API success or on failure; success/error review UI unchanged. **No** backend/database/migration/package changes; **no** dashboard hero / **`deriveDashboardRecommendation`** / filter / URL-persisted filter changes; **no** SM-2/Anki; **no** new product features — stabilization cleanup only. **`cd frontend && npm run lint`**, **`npm test`**, **`npm run build`** passed; main CI **green**. **Supervisor Review: approved with notes.** **Security Review: not required** (frontend-only invalidation after existing authenticated API flow). Docs: **DOCS-FLASHCARD-STABILIZATION-A1**.
