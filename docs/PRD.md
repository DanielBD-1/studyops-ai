...
| Backend **`GET /api/tasks?materialId=`** / **`GET /api/courses/:id/tasks?materialId=`**; URL-persisted task filters on **`/courses/:id`** | **Deferred** (**`/tasks`** URL-persisted filters shipped in **TASK-MATERIAL-FILTERS-E1** + **`materialId=none`** in **TASK-MATERIAL-UNLINKED-A1** — **`courseId`**, **`materialId`**, **`status`**; **`materialId`** client-side only) |
