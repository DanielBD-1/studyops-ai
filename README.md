...
- **Study tasks (course UI)** — on `/courses/:id`: **All/Pending/Completed filters**, **study material** filter (**All** / material UUID / **Tasks without material** → list API **`materialId`** omitted / UUID / **`none`** — **TASK-MATERIAL-BACKEND-A1**), **Overdue/Due today/Next 7 days** deadline filters (backend-filtered — **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**); **URL-persisted filters** on course page via query params **`materialId`**, **`status`**, **`deadline`** (**TASK-COURSE-FILTERS-E1** + **FIX1** — course ID in path only; defaults omitted; Back/Forward; deep links); backend list order (**TASK-DUE-SORT-A1** — **Pending**/deadline: earliest **`due_date`** first; **Completed**/All: recency); list, **create**, **edit pending** tasks (`PATCH`), optional **link/unlink** study materials, optional **due date** on create/edit (**DASHBOARD-DEPTH-P0**), mark complete, delete (Bearer JWT)
- **Study tasks (global UI)** — on **`/tasks`**: protected cross-course list; **course + status + material + deadline filters** (**TASK-MATERIAL-BACKEND-A1** — backend **`materialId`** on list fetch; **TASK-MATERIAL-FILTERS-E1** + **UNLINKED-A1** — URL-persisted **`materialId`** UUID or **`none`**; **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1** — URL-persisted **`deadline=overdue|due_today|next_7_days`**); **create**, **edit pending**, optional **due date**, mark complete, delete; **`TaskCard`** due-date presentation when set; backend list order (**TASK-DUE-SORT-A1** — same contract as course list; frontend renders API order — **no** sort selector); includes tasks created manually or **imported from a generated plan**
...
- **Dashboard** — protected **`/dashboard`** displays real user-owned aggregate stats from **`GET /api/dashboard/stats`** (Overview, Tasks, Focus, Learning assets, Trello sync count, per-course breakdown with links); **deadline-aware aggregates** — **`overduePendingTasks`**, **`dueTodayPendingTasks`**, **`dueNext7DaysPendingTasks`**, **`deadlineReferenceDate`** with optional **`referenceDate`** and browser-local date alignment (**DASHBOARD-DEPTH-A2** + **TASK-UPCOMING-FILTERS-A1**); **Last 7 days** focus snapshot in Study pulse (**DASHBOARD-DEPTH-A1**); rule-based recommendation priority including overdue/due-today/**due-next-7-days** tasks; **At a glance** **Overdue** / **Due today** / **Due next 7 days** and deadline recommendation heroes link to backend-filtered task lists — **`/tasks?status=pending&deadline=overdue`**, **`/tasks?status=pending&deadline=due_today`**, and **`/tasks?status=pending&deadline=next_7_days`** when counts > 0 (**TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**); read-only; fetch on mount + **Try again** + **Refresh stats**; when the dashboard is open, stats refresh silently after stat-changing actions elsewhere (**5B** backend + **5C** UI + **5C.1** invalidation-only refresh — **no** backend/API changes in **5C.1**)
