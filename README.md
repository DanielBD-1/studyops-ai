...
**Current status:** Phases **1A–1G**, **2A–2G**, **2L-a/b/c/d**, and **3A-a/b/c/c.1/c.2/c.3/d/e/f** plus **3B-a/b/c/d/e/f/g** are implemented — auth, courses, study materials, Gemini document-service, backend generate + saved-plan APIs, frontend load/clear/**import plan tasks** UI, **saved DB flashcards** on material detail with **manual create/edit/delete** (3B-e) + **import plan flashcards** to library (3B-d), **global `/flashcards`** page with **create**, list/filter/study/edit/delete (3B-f + **3B-g**), **flashcard study UI** from generated plan (3B-a), **`public.flashcards` table** (3B-b), **flashcards backend REST API** (3B-c), **`study_tasks` backend API**, **course-level** and **global** task management. See **[docs/IMPLEMENTATION_STATUS.md](docs/IMPLEMENTATION_STATUS.md)** for routes and boundaries. Phase history: **[docs/AGENT_MEMORY.md](docs/AGENT_MEMORY.md)**. Future work (focus sessions, material navigation/filtering, course-level flashcard management, spaced repetition, known/unknown, Anki, AI flashcard generation, Trello, dashboard, deployment) requires explicit human approval — see **[CONTRIBUTING.md](CONTRIBUTING.md)**.
