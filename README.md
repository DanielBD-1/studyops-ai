...
**Current status:** Phases **1A–1G**, **2A–2G**, **2L-a/b/c/d**, and **3A-a/b/c/c.1/c.2/c.3/d/e/f** plus **3B-a/b/c/d** are implemented — auth, courses, study materials, Gemini document-service, backend generate + saved-plan APIs, frontend load/clear/**import plan tasks** UI, **saved DB flashcards** on material detail + **import plan flashcards** to library (3B-d), **flashcard study UI** from generated plan (3B-a), **`public.flashcards` table** (3B-b), **flashcards backend REST API** (3B-c), **`study_tasks` backend API**, **course-level** and **global** task management. See **[docs/IMPLEMENTATION_STATUS.md](docs/IMPLEMENTATION_STATUS.md)** for routes and boundaries. Phase history: **[docs/AGENT_MEMORY.md](docs/AGENT_MEMORY.md)**. Future work (focus sessions, material navigation/filtering, global **`/flashcards`** page, flashcard manual create/edit/delete UI, Trello, dashboard, deployment) requires explicit human approval — see **[CONTRIBUTING.md](CONTRIBUTING.md)**.
...
- **Saved DB flashcards (material detail)** — on `/study-materials/:materialId`, **Saved flashcards** loads rows via `GET /api/flashcards?materialId=` and reuses flip/reveal study UI
...
- **Flashcards backend API** — `GET /api/flashcards`, `POST /api/courses/:id/flashcards`, `PATCH` / `DELETE /api/flashcards/:flashcardId` (auth + ownership; frontend uses list/create on material detail; edit/delete UI not built yet)
