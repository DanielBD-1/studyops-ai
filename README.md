...
- **Trello OAuth account connection (A3 + A4-STATE + A4-FRONTEND)** — backend connect routes + signed state + frontend Connect/Disconnect on **`/trello`** and protected **`/trello/connect/callback`** (token from hash, state from query; URL cleared before complete POST; reviewed); **stored-token sync not implemented**
- **Trello not implemented (stored-token sync)** — **A5:** sync/boards/lists using stored token; remove manual sync credentials; board/list persistence; Trello card update/delete; force re-sync — **manual apiKey/token sync remains the live path**
