...
- **Study tasks (course UI)** — on `/courses/:id`: **All/Pending/Completed filters**, list, **create**, **edit pending** tasks (`PATCH`), optional **link/unlink** study materials, optional **due date** on create/edit (**DASHBOARD-DEPTH-P0**), mark complete, delete (Bearer JWT)
- **Study tasks (global UI)** — on **`/tasks`**: protected cross-course list; **course + status filters** (in-memory); **create**, **edit pending**, optional **due date**, mark complete, delete; **`TaskCard`** due-date presentation when set; includes tasks created manually or **imported from a generated plan**
