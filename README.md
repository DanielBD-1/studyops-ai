...
**Current status:** Phases **1A–1G**, **2A–2G**, **2L-a/b/c/d**, and **3A-a/b/c/c.1/c.2/c.3/d** are implemented — auth, courses, study materials, Gemini document-service, backend generate + saved-plan APIs, frontend load/clear UI, **`study_tasks` backend API**, **course-level manual task UI** on `/courses/:id` (including create), and a protected **All study tasks** page at **`/tasks`** (global list with course/status filters, edit/complete/delete — **task creation stays on course pages**). See **[docs/IMPLEMENTATION_STATUS.md](docs/IMPLEMENTATION_STATUS.md)** for routes and boundaries. Phase history: **[docs/AGENT_MEMORY.md](docs/AGENT_MEMORY.md)**. Future work (create on `/tasks`, focus sessions, material navigation/filtering, generated-plan import, flashcards **table/UI**, Trello, dashboard, deployment) requires explicit human approval — see **[CONTRIBUTING.md](CONTRIBUTING.md)**.
...
- **Study tasks (global UI)** — on **`/tasks`**: protected cross-course list; **course + status filters** (in-memory); **edit pending** (incl. `materialId` via lazy materials load), mark complete, delete; course name links to `/courses/:id`; **no create** here — add tasks from a course page; **no** import from generated plan JSON
