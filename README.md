...
- **Study tasks (course UI)** — on `/courses/:id`: **All/Pending/Completed filters**, **study material** filter (**All** / material UUID / **Tasks without material** → list API **`materialId`** omitted / UUID / **`none`** — **TASK-MATERIAL-BACKEND-A1**), **Overdue/Due today/Next 7 days** deadline filters (in-memory; backend-filtered — **TASK-DUE-FILTERS-A1** + **TASK-UPCOMING-FILTERS-A1**); backend list order (**TASK-DUE-SORT-A1** — **Pending**/deadline: earliest **`due_date`** first; **Completed**/All: recency); list, **create**, **edit pending** tasks (`PATCH`), optional **link/unlink** study materials, optional **due date** on create/edit (**DASHBOARD-DEPTH-P0**), mark complete, delete (Bearer JWT)
