...
- **Trello OAuth backend stored-token mode (A5A)** — **`POST /api/trello/boards`**, **`/lists`**, **`/sync`** support stored-token mode when body omits `apiKey`/`token`; manual mode unchanged; **frontend still manual until A5B**
- **Trello not implemented (frontend stored-token sync)** — **A5B:** frontend switch to stored-token sync; remove manual sync credential fields when connected; board/list persistence; Trello card update/delete; force re-sync — **manual apiKey/token sync remains the live UI path**
