...
- **Admin authorization (backend only, 6A-1)** — **`GET /api/admin/access-check`** protected by **`requireAuth`** + **`requireAdmin`**; verifies **`profiles.role === 'admin'`** from DB (not JWT/client); returns **`{ admin: true }`** only; **no** frontend **`/admin`** page, admin stats, or logs yet
