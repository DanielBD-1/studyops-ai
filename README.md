...
- **Trello OAuth foundation (A2, not live)** — encrypted `trello_connections` storage + backend crypto/repository in repo; **OAuth, Connect UI, and connect/callback routes are not implemented** — boards/lists/sync still use manual apiKey/token until later phases (see ADR 006, `docs/security/trello-oauth-foundation.md`)
- **Trello not implemented** — OAuth / Connect Trello UI; connect/disconnect HTTP routes; sync/boards using stored token; board/list persistence; Trello card update/delete; force re-sync
