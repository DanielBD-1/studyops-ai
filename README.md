...
- **Study tasks (course UI)** — on `/courses/:id`: **All/Pending/Completed filters**, **Overdue/Due today** deadline filters (in-memory; backend-filtered — **TASK-DUE-FILTERS-A1**); backend list order (**TASK-DUE-SORT-A1** — **Pending**/deadline: earliest **`due_date`** first; **Completed**/All: recency); list, **create**, **edit pending** tasks (`PATCH`), optional **link/unlink** study materials, optional **due date** on create/edit (**DASHBOARD-DEPTH-P0**), mark complete, delete (Bearer JWT)
- **Study tasks (global UI)** — on **`/tasks`**: protected cross-course list; **course + status + deadline filters** (**TASK-DUE-FILTERS-A1** — URL-persisted **`deadline=overdue|due_today`** on **`/tasks`**; composes with **`courseId`**, **`materialId`**, **`status`**); **create**, **edit pending**, optional **due date**, mark complete, delete; **`TaskCard`** due-date presentation when set; backend list order (**TASK-DUE-SORT-A1** — same contract as course list; frontend renders API order — **no** sort selector); includes tasks created manually or **imported from a generated plan**
